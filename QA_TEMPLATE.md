...
| 10.2.12 | Limit place card IA | Open `/limits` or `/trade` Limit tab | `/limits`: Rate → % chips → Pay → Receive → Expiry. `/trade` Limit: price → Pay → Receive; expiry / chips / pre-submit under Advanced ([#693](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/693)); no instructional paragraphs ([#488](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/488)/[#489](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/489)) | | |
...
| 10.2.18 | Protocol leftovers — 24h liquidity + dense x-axis | View `/protocol` at 390 / 820 / 1280, light + dark; switch Hourly / Daily / Monthly | Total liquidity shows **one** `24h` Δ% chip (no `30d` in that tile). Last 30d vol / fees stay. UTC volume labels every bar or every second bar; first + last period visible. ([#677](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/677)) | | |
| 10.2.19 | Trade ticket flatten | View `/trade/:pair` dark + light at ~375px + ~1280px | Fresh ticket is **Market** (compact underlined tabs, not full-width pills). Heading has token logo + non-orange wash. No Side / Top buy / Top sell / mode card. Slippage only in Advanced. Footer docked. ([#693](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/693)) | | |
| 10.2.20 | Protocol Monthly phone x-axis | View `/protocol` at **390×844**, Monthly, then Liquidity / Fees; also 820 and 1280 | Phone shows last **6–12** UTC months; ticks are horizontal `YY-MM` and do **not** overlap or rotate. Tooltip / bar label keep `YYYY-MM`. Desktop Monthly may still show 24. Subtitle stays `UTC calendar month`. ([#703](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/703)) | | |
