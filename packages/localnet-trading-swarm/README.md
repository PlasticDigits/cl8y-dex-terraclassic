...
| `SWARM_ULUNA_TOPUP` / `SWARM_UUSD_TOPUP` / `SWARM_CW20_MINT_TOPUP` / `SWARM_MIN_CW20_BALANCE` / `SWARM_MINT_SLEEP_MS` | Funding tuning (see `src/funding.ts`). Defaults match **10×** genesis `test1` on LocalTerra (GitLab #372): 20M LUNC / 10M USTC top-up per bot, `1e13` CW20 floor. |
