...
async fn clean_db_tables(pool: &PgPool) {
    ...
    sqlx::query(
        "TRUNCATE TABLE
            ...
            protocol_hourly_volume,
            protocol_monthly_volume,
            protocol_daily_liquidity,
            protocol_hourly_liquidity,
            protocol_monthly_liquidity,
            protocol_daily_fees,
            protocol_hourly_fees,
            protocol_monthly_fees,
            pair_reserves,
            hub_prices,
            economic_token_marks,
...
pub async fn seed_db(pool: &PgPool) -> SeedData {
    ...
    cl8y_dex_indexer::db::queries::volume::refresh_pair_volumes_30d(pool)
        .await
        .expect("refresh pair_volume_30d for tests");
