//! DEX hub USD API (GitLab #556). Separate from CEX `GET /api/v1/oracle/price`.
...
use crate::indexer::hub_usd::{HubTicker, HUB_TICKERS};
...
pub const HUB_PRICES_METADATA: &str = concat!(
    ...
    "Tickers: custc (1:1 USTC CEX oracle), ust1 (deepest cUSTC/UST1 pool), ",
    "ustr (deepest pair vs cUSTC or UST1). Identity is hub CW20 contract, not symbol."
);
...
fn entry_for(ticker: HubTicker, row: Option<&HubPriceRow>) -> HubPriceEntry {
    match row {
...
pub async fn get_hub_prices(
    ...
    let prices = [HubTicker::Custc, HubTicker::Ust1, HubTicker::Ustr]
        .into_iter()
        .map(|t| {
...
#[utoipa::path(
    ...
    params(
        ("ticker" = String, Path, description = "Hub ticker: custc, ust1, or ustr")
    ),
    responses(
...
pub async fn get_hub_price(
    ...
    let row = hub_prices::get_hub_price(&state.pool, ticker)
        .await
        .map_err(internal_err)?;
    Ok(Json(entry_for(ticker, row.as_ref())))
}
