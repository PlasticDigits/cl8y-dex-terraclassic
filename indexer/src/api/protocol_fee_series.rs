//! `GET /api/v1/protocol/fees/daily` — UTC Protocol fee **flow** series (GitLab #689).
//!
//! Distinct from `GET /protocol/fees?window=` (trailing 24h/7d/30d breakdown).
//! `grain` + capped `limit`; `from` / `to` / `window` / `days` / `metric` / `ticker` → **400**.
//! 60s cache keyed by allowlisted `(grain, limit)` only. Reads grain tables — never
//! `protocol_fee_events` / `swap_events` / Llama. Idle `"0"`; all-unpriced `null`.
...
use crate::db::queries::protocol_fee_series as fee_q;
...
struct FeeSeriesCacheKey {
    grain: vol_q::VolumeGrain,
    limit: i32,
}

fn fee_series_cache()
-> &'static Mutex<HashMap<FeeSeriesCacheKey, (ProtocolFeeSeriesResponse, Instant)>> {
    static CACHE: OnceLock<Mutex<HashMap<FeeSeriesCacheKey, (ProtocolFeeSeriesResponse, Instant)>>> =
        OnceLock::new();
    CACHE.get_or_init(|| Mutex::new(HashMap::new()))
}

#[derive(Debug, Deserialize)]
pub struct ProtocolFeeSeriesQuery {
...
pub struct ProtocolFeeSeriesPoint {
    ...
    /// `"0"` idle; JSON `null` when the bucket has events but no priced USD.
    pub fees_usd: Option<String>,
    pub event_count: i64,
}

#[derive(Serialize, ToSchema, Clone)]
pub struct ProtocolFeeSeriesResponse {
    pub grain: String,
    pub limit: i32,
    pub timezone: String,
    /// Protocol catalog treasury fees (PFee). Not DeFiLlama gem-exclude.
    pub methodology: String,
    pub series: Vec<ProtocolFeeSeriesPoint>,
}

/// Drop the 60s fee-series response cache (tests).
pub fn reset_protocol_fee_series_cache() {
    if let Ok(mut guard) = fee_series_cache().lock() {
        guard.clear();
    }
}
...
fn reject_disallowed(q: &ProtocolFeeSeriesQuery) -> Result<(), (StatusCode, String)> {
...
#[utoipa::path(
    get,
    path = "/api/v1/protocol/fees/daily",
    params(
    ...
    responses(
        (status = 200, description = "UTC Protocol fee flow series", body = ProtocolFeeSeriesResponse),
        (status = 400, description = "Invalid grain / limit / disallowed query"),
        (status = 500, description = "Internal server error"),
    ),
    tag = "Overview"
)]
pub async fn get_protocol_fees_daily(
    State(state): State<AppState>,
    Query(q): Query<ProtocolFeeSeriesQuery>,
) -> Result<Json<ProtocolFeeSeriesResponse>, (StatusCode, String)> {
    reject_disallowed(&q)?;
    ...
    let key = FeeSeriesCacheKey { grain, limit };

    if let Ok(guard) = fee_series_cache().lock() {
...
fn wrap(
    grain: vol_q::VolumeGrain,
    limit: i32,
    series: Vec<ProtocolFeeSeriesPoint>,
...
async fn build_daily_series(
    ...
    grain: vol_q::VolumeGrain,
    limit: i32,
) -> Result<ProtocolFeeSeriesResponse, (StatusCode, String)> {
    let today = utc_day_start(Utc::now()).date_naive();
    let from = today - ChronoDuration::days(i64::from(limit) - 1);
    let rows = fee_q::get_daily_rows(pool, from, today)
        .await
    ...
    for offset in (0..limit).rev() {
        ...
        let point = match by_day.get(&day) {
            Some(row) => ProtocolFeeSeriesPoint {
                utc_hour: None,
                utc_day: Some(day.to_string()),
                utc_month: None,
                fees_usd: row.fees_usd.as_ref().map(|v| v.to_string()),
                event_count: row.event_count,
            },
            None => ProtocolFeeSeriesPoint {
                utc_hour: None,
                utc_day: Some(day.to_string()),
                utc_month: None,
                fees_usd: idle_zero(),
...
async fn build_hourly_series(
    ...
    let rows = fee_q::get_hourly_rows(pool, from, now_hour)
        .await
        .map_err(internal_err)?;
...
async fn build_monthly_series(
    ...
    let rows = fee_q::get_monthly_rows(pool, from, this_month)
        .await
        .map_err(internal_err)?;
