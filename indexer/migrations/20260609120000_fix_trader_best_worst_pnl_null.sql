...
-- Multi-trade rows stuck at 0: backfill from aggregates (total = worst → one close; total < worst → max loss).
UPDATE traders SET best_trade_pnl = worst_trade_pnl
WHERE total_trades > 1
  AND best_trade_pnl = 0
  AND total_realized_pnl < 0
  AND total_realized_pnl = worst_trade_pnl;
UPDATE traders SET best_trade_pnl = total_realized_pnl - worst_trade_pnl
WHERE total_trades > 1
  AND best_trade_pnl = 0
  AND total_realized_pnl < 0
  AND total_realized_pnl < worst_trade_pnl;
