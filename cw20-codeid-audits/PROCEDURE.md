...
6. **Automated suite**
   ...
   - **Layer B (DEX + limits)** — same harness re-runs **P1**, **P2**, **P3**, **P4**/**P10**, **R1–R4**, **C4**, **L1–L3**, **L6**, **L10**, **L11**, sweep on a pair whose one asset is the candidate (other side 10184 analogue). Compare to 10184/10184 control. LocalTerra store of the pinned wasm is `LAYER_B_LT=1` via [`scripts/layer-b-lt.sh`](scripts/layer-b-lt.sh): whitelist **local** store id, `CreatePair`, provide **P2**, round-trip **Send** swap **B7**, limit Send escrow **L1**, SendFrom. Stub PASS is forbidden ([#590](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/590)). B-lt stays **tax-off** (no `RegisterListedPair`). Community-tax extra-debit / router `trader` / AutoLP floor is the named suite [`scripts/layer-b-tax-on.sh`](scripts/layer-b-tax-on.sh) (`make verify-issue-623`, [#623](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/623)).
