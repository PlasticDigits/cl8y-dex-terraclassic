...
6. **Automated suite**
   - **Layer A (token-only)** — `cw-multi-test` parameterized tests in `smartcontracts/tests/src/cw20_codeid_harness.rs` against mintable (10184 analogue) and the [mutant library](../smartcontracts/tests/src/cw20_mutants.rs). For the **candidate LCD wasm**, instantiate on LocalTerra (Terra `requires_terra` capability) or skip instantiate only if init is exotic and a live instance snapshot is used — document which.
   - **Layer B (DEX + limits)** — same harness re-runs **P1**, **P2**, **P3**, **P4**/**P10**, **R1–R4**, **C4**, **L1–L3**, **L6**, **L10**, **L11**, sweep on a pair whose one asset is the candidate (other side 10184 analogue). Compare to 10184/10184 control. LocalTerra store of the pinned wasm is `LAYER_B_LT=1`. A silent skip of Layer B is forbidden (**C5**): either run it or print `SKIP Layer B-lt: make has-localterra` (multi-test Layer B still runs).
