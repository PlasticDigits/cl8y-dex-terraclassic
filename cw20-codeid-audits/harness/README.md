...
| **A-lcd** | Pinned LCD wasm stored on **LocalTerra** (Terra `requires_terra` / stargate / iterator capabilities) | `CODE_ID=<id>` after fetch. Columbus-5 templates often export `requires_terra`; stock `cosmwasm-vm` without that capability is not a substitute. |
...
| **B-lt** | `StoreCode` of the **pinned LCD wasm** on LocalTerra, whitelist **only in the test factory**, `CreatePair` vs 10184 counter-asset | `LAYER_B_LT=1` after `make has-localterra`. Silent skip is forbidden (**C5**). |
