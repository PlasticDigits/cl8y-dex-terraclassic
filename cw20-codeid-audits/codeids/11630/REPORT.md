# REPORT: CW20 code ID 11630 (`cl8y-community-tax-token` #633 manager-role skip)

**Date:** 2026-08-25  
**Operator:** ops / #635 intake  
**LCD:** `https://terra-classic-lcd.publicnode.com`  
**Procedure:** [`../../PROCEDURE.md`](../../PROCEDURE.md) ([#589](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/589))  
**Issues:** [#635](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/635) leftover of [#633](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/633) · crate [#592](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/592)

The inspected artifact is a **decompilation / string fingerprint of LCD wasm**. It is **not** redistributable source. F6 pin does not replace this report. Adopt pin stays [`../11626/REPORT.md`](../11626/REPORT.md). Listed rotate pin stays [`../11619/REPORT.md`](../11619/REPORT.md). Honest pin stays [`../11611/REPORT.md`](../11611/REPORT.md). Pre-store stub: [`../community-tax-token/REPORT.md`](../community-tax-token/REPORT.md).

## Verdict

**GO** for factory `AddWhitelistedCodeId 11630` as the **named T592 exception** (inbound pair/router/escrow/AutoLP credit stays **1:1**; sell tax is extra-debit; buy tax is outbound split). This pin is the **#633** same-crate bump of **11626**: `config.manager` skips buy/sell/transfer without a paid ExemptionDirectory row (launch guards stay on). Columbus-5 **stored 2026-08-25** (height **30100763**). Live whitelist at intake is still **`[6036, 8266, 10184, 11626]`**. Do **not** whitelist a LocalTerra store id. Do **not** whitelist ALPHA **8654**. Do **not** whitelist launcher **11612** / **11614** / **11620** / **11622**, AutoLP **11613** / **11621**, or factory stores **11628** / **11629** (not pair-asset CW20s). Do **not** append columbus-5 code **3**.

- [x] GO — LCD pin matches; decomp + fingerprint; crate multitest covers T592 / option-2 / #608 / #609 / #626 adopt / #633 manager skip; harness known-bad stays red; A-lcd/B-lt executed; catalogue filled
- [ ] NO-GO — inbound FoT / 8654 class / missing pin

**Reason:** SHA-256 of LCD bytes equals `CodeInfo.data_hash` (`E60A6E31…`). cw2 `crates.io:cl8y-community-tax-token` / `1.0.0`. Pair credit on sell is `amount` (extra-debit on the trader), not inbound FoT. **Fingerprint `tax_map` / `cw20_taxed` are true** because `adopt.rs` names those leftover namespaces / cw2 allowlist for the S4 wipe — **`UpdateTaxMap` is false**; there is no live `tax_map` execute. **8654** / FoT mutants must stay red.

**Layer A-lcd / B-lt (O601-1):** executed 2026-08-25 on pinned LCD bytes (`CODE_ID=11630 LAYER_B_LT=1`). A-lcd retries community-tax `InstantiateMsg` (cw20-base init missing `manager`); 11630 then rejects SKU-gated `launch_guards` unless that feature is on — harness retries without those fields. `balance_at` is A29 N/A. B-lt does **not** `RegisterListedPair` — inbound 1:1 / provide / round-trip / limit escrow hold. **Tax-on suite** ([#623](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/623) / [`layer-b-tax-on.sh`](../../scripts/layer-b-tax-on.sh)): extra-debit / outbound / router `trader` / AutoLP floor after register. Do **not** merge that into B-lt.

Re-run: `CODE_ID=11630 LAYER_B_LT=1 make verify-issue-589` · `make verify-issue-592` · `make verify-issue-633` · `make verify-issue-626` · `make verify-issue-607` · `make verify-issue-608` · `make verify-issue-609`.
...
|-------|--------|
| `code_id` | **11630** |
| `data_hash` (LCD) | `E60A6E31BB0261505170208BBF2B0E17CDB4C6258D701E2E6D22E362CA872B96` |
...
| Approximate instantiate count | **0** tokens (2026-08-25). Canonical launcher: `terra126pr5323xkhwas7y03azv48sqr2fy3fxxg0sxu8xhmjdxr8v5tzqahzwze` (code **11622**; `token_code_id` **11626** until UpdateConfig) |
| Wasm size | 581709 bytes |
| Store tx | [`11763CB747BEE51231871F580A36862A47330D4C9EBF220FC779A0E16DC99E48`](https://finder.terraclassic.community/columbus-5/tx/11763CB747BEE51231871F580A36862A47330D4C9EBF220FC779A0E16DC99E48) height **30100763** |
| `meta.json` | [`meta.json`](meta.json) |

Sister stores (not listable as pair assets): factory **11628** / **11629** (same hash `64ABBCCB…`; live factory instance is **11629** / cw2 **1.10.0**). Canonical launcher instance `terra126pr5…ahzwze` is code **11622**. Unused first launcher **11612** at `terra1af9xm63mev4hnf4z0nmmcsnd9f4lpac2vs205rmaeg3kdqlqudhq894lyz`. AutoLP launcher pin **11621**. Adopt pin remains [`../11626/REPORT.md`](../11626/REPORT.md).

## Fetch

- Endpoint: columbus-5 LCD `https://terra-classic-lcd.publicnode.com` (`fetch-lcd-wasm.sh 11630`). Secondary LCD not required this run (`LCD_URL_SECONDARY` unset — **G4** residual).
...
**#633 vs 11626:** LCD strings include `Cannot add a protocol address as a manager exemption`, `is_protocol_exempt`, `remove_exempt`, and `manager` on `GetConfig`. Role skip is `addr == config.manager || MANAGER_EXEMPT` (crate **R633-1**). Adopt / option-2 / #608 / #609 strings from 11626 remain (`hop_trader` / `hop_trader_debit`, `migrate-adopt`, `GetMigrateOrigin`, leftover `tax_map` wipe).

**Declared / debit / credit / tax** strings present (`TaxPreview`). Sell extra-debit is **trader-side**; pair inbound credit stays `amount`.

## Decompile

`decomp/` generated (`decompile-wasm.sh 11630`, wabt 1.0.36). `fingerprint.json` is the committed pin; wat/c/objdump gitignored. Unreadable regions: stripped names; CFG not proven byte-identical to a rebuild (rebuild is appendix-only).

## Catalogue

Legend: **static-pass** = LCD strings/dump; **crate** = `cl8y-community-tax-token` multitest / `make verify-issue-592` / `626` / `633`; **control** = `cw20_codeid_harness` / mutants (not executed on 11630 bytes); **named-exception** = T592 extra-debit / outbound split (not inbound FoT); **lt-pass** = `LAYER_B_LT=1` on pinned wasm; **N/A** = no analogue or out of listing veto.
...
| A2 | **named-exception** + crate; **in these bytes** | Directional **sell** extra-debit / **buy** outbound split on **factory-registered** listed pairs. Official-router hops tax authenticated `Swap.trader` (**T592-13** / #607). **#633:** `config.manager` skips buy/sell/transfer without a directory row. Wallet↔wallet honest unless TransferTax SKU. Must **not** match inbound-tax mutant |
...
| A4 | static-pass (wipe strings) + crate; control | **No `UpdateTaxMap`.** Fingerprint `tax_map` / `cw20_taxed` are adopt **wipe** leftovers (S4). 8654 remains known-bad |
| A5 | static-pass | Plain `Transfer` does not dispatch a recipient `WasmMsg`. `Send` is CW20 Receive only |
| A6 | control; lt-pass pair path | Pair reentrancy tests exist; 11630 as offer token executed on LocalTerra (B-lt swap / limit) |
...
| A9 | crate (T592-11 / H608); **in these bytes** | `trading_enabled=false` blocks **both** buy and sell (launch guard SKU), not sell-only honeypot. Sell to listed pair bypasses `max_wallet`. Cooldown is per user wallet. Manager role skip does **not** make the manager protocol-exempt |
...
| B9 | control | F6 freeze if instance migrates off a listed id (11626 / 11630 after ops) |
...
| C2 | pass | Decomp ran with wabt 1.0.36 |
| C3 | crate + A-mt/B-mt; **A-lcd/B-lt executed** | Pinned 11630 bytes on LocalTerra 2026-08-25 (`layer-a-lcd.json` / `layer-b-lt.json`; last local store id **25** is host-ephemeral — never list it) |
| C4 | pass | Known-bad FoT 1:1 / P2 stay red |
| C5 | pass (executed) | `LAYER_B_LT=1` ran A-lcd + B-lt on pinned 11630 (not a stub) |
...
| D9 | residual | Manager ≠ wasm admin. CMM migrate residual = A14. **#633** manager role skip is tax-only |
...
| catalog F1–F8 | residual / #594 | Indexer must key on listed `code_id` (**11626** today; **11630** after whitelist + Coolify flip), CMM admin, launcher origin **or** `GetMigrateOrigin` attest. Event vs extra-debit debit is a display concern, not inbound P2 |
...
- **Crate:** `cd smartcontracts && cargo test -p cl8y-community-tax-token` / `make verify-issue-592` · `make verify-issue-633` · `make verify-issue-626` — inbound 1:1, extra-debit sell, outbound buy, manager role skip, foreign adopt, 50 UST1 invoice fail-closed.
- **#607 / #608 / #609 / #633 source:** option-2 hop trader, LaunchGuards per-wallet cooldown, manager-directory skip, and **manager role skip** are **in these bytes**.
- **A-mt:** `cw20_codeid_harness` mintable + mutants via `make verify-issue-589`.
- **A-lcd:** [`../../scripts/layer-a-lcd.sh`](../../scripts/layer-a-lcd.sh) **executed** 2026-08-25 — Transfer / TransferFrom 1:1, unauthorized mint/burn_from rejected, idle stable. `balance_at` A29 N/A. Last local token `terra1facc…rfxvz` (do not list this address). Last local store id **25**.

## Layer B (DEX + limits)

- **B-mt:** harness P1/P2/P3/donation/flash/honeypot/limit/same-asset; FoT **P2** red.
- **B-lt:** [`../../scripts/layer-b-lt.sh`](../../scripts/layer-b-lt.sh) **executed** 2026-08-25 — provide 1:1, P2 reserves, round-trip swap, limit escrow 1:1, SendFrom. Whitelist only the **local** store id **25** (never treat that id as columbus-5 11630). B-lt stays **tax-off** (no `RegisterListedPair`). Last pair `terra1f8flu…sewcg`.
...
Approving **11630** admits **every** instantiate of this wasm, including rogue `--admin` and non-launcher creates. That is accepted for the template: dApp (#593) and indexer (#594) **must** filter `ContractInfo.admin == CMM` and (`GetLauncherOrigin` **or** allowlisted `GetMigrateOrigin`). Pair-create fee remains **100 LUNC**. **0** instances of 11611 / 11619 / 11626 / 11630 — no CMM migrate and no Refresh. Factory list is **`[6036, 8266, 10184, 11630]`** (11626 removed). Duplicate store **11631** is the same bytes — do not list it.

Launcher is **11632**. `GetConfig` is `token_code_id` **11630** / `autolp_code_id` **11633**. Coolify/indexer bake **11630**.

## Instance admin / migrate residual (F6)

Launcher stamps CosmWasm admin = `cmm_governance` (CMM `terra16j5u6ey7a84g40sr3gd94nzg5w5fm45046k9s2347qhfpwm5fr6sem3lr2`). Retail adopt is `MsgMigrateContract` then `MsgUpdateAdmin` → CMM. Manager cannot migrate. F6 freezes writes if an instance leaves a listed id. Honest upgrade completed 2026-08-25: GO → whitelist 11630 → launcher `UpdateConfig` → Coolify/indexer pin → `RemoveWhitelistedCodeId 11626`. Zero instances; no CMM migrate.
...
- In-repo crate: `smartcontracts/contracts/community-tax-token` (workspace 1.0.0, `tax.rs` `is_manager_exempt`, `adopt.rs`). Optimizer artifact hash matched LCD; **not** a required rebuild proof.
- Policy: [`docs/runbooks/cw20-whitelist-policy.md`](../../../docs/runbooks/cw20-whitelist-policy.md) named exception.
- Playbook: [`skills/AGENTS_COMMUNITY_TAX_CW20.md`](../../../skills/AGENTS_COMMUNITY_TAX_CW20.md) **T592-1–T592-13**; autoregister [`skills/AGENTS_COMMUNITY_TAX_AUTOREGISTER.md`](../../../skills/AGENTS_COMMUNITY_TAX_AUTOREGISTER.md) **R633**; migrate [`skills/AGENTS_FRONTEND_TOKEN_MIGRATE.md`](../../../skills/AGENTS_FRONTEND_TOKEN_MIGRATE.md) **M626**.
- After GO, list + launcher leftover on #635:

```bash
# listed 11630; launcher 11632 GetConfig token=11630 autolp=11633
# Coolify (baked 2026-08-25):
#   VITE_COMMUNITY_TAX_CODE_ID=11630
#   COMMUNITY_TAX_CODE_ID=11630
#   COMMUNITY_TAX_OPTION2_CODE_IDS=11630
#   VITE_COMMUNITY_MIGRATE_CODE_IDS=6036,10184,8266,8654
# Never AddWhitelistedCodeId 8654 / 11621 / 11622 / 11628 / 11629 / 11631 / 11632 / 11633.
```
