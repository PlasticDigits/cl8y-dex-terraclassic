...
**Date:** 2026-08-24  
**Operator:** ops / #611 intake  
**LCD:** `https://terra-classic-lcd.publicnode.com`  
**Procedure:** [`../../PROCEDURE.md`](../../PROCEDURE.md) ([#589](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/589))  
**Issues:** [#611](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/611) / [#612](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/612) / [#616](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/616) · crate [#592](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/592)

The inspected artifact is a **decompilation / string fingerprint of LCD wasm**. It is **not** redistributable source. F6 pin does not replace this report. Listed pin stays [`../11611/REPORT.md`](../11611/REPORT.md). Pre-store stub: [`../community-tax-token/REPORT.md`](../community-tax-token/REPORT.md).
...
- Endpoint: columbus-5 LCD `https://terra-classic-lcd.publicnode.com` (`fetch-lcd-wasm.sh 11619`). Secondary LCD not required this run (`LCD_URL_SECONDARY` unset — **G4** residual).
...
**Declared / debit / credit / tax** strings present (`TaxPreview`). Sell extra-debit is **trader-side**; pair inbound credit stays `amount`. Option-2 strings present: `hop_trader`, `hop_trader_debit`, `Official router hop requires a trusted non-exempt trader`. SKU / #608 / #609 strings present: `initial_exempt`, `exemption_directory`, `variable_rates`, `launch_guards`, `enable_feature`, `Cannot remove a protocol exemption`.

## Decompile

`decomp/` generated (`decompile-wasm.sh 11619`, wabt). `fingerprint.json` is the committed pin; wat/c/objdump gitignored. Unreadable regions: stripped names; CFG not proven byte-identical to a rebuild (rebuild is appendix-only).

## Catalogue

Legend: **static-pass** = LCD strings/dump; **crate** = `cl8y-community-tax-token` multitest / `make verify-issue-592`; **control** = `cw20_codeid_harness` / mutants (not executed on 11619 bytes); **named-exception** = T592 extra-debit / outbound split (not inbound FoT); **lt-pass** = `LAYER_B_LT=1` on pinned wasm; **N/A** = no analogue or out of listing veto.
...
| A6 | control; lt-pass pair path | Pair reentrancy tests exist; 11619 as offer token executed on LocalTerra (B-lt swap / limit) |
...
| A9 | crate (T592-11 / H608); **in these bytes** | `trading_enabled=false` blocks **both** buy and sell (launch guard SKU), not sell-only honeypot. Sell to listed pair bypasses `max_wallet`. Cooldown is per user wallet; `max_wallet` skips protocol / listed pair as `to` (crate + LCD strings). Not “needs migrate” — this is the post-#608 pin |
...
| A11 | residual (SKU); crate #608; **in these bytes** | `max_wallet` is launch-guard SKU; sell-to-listed-pair **and** protocol/`to` (provide) skip the cap. User Buy / Transfer still capped. Not a silent post-list shrink of pair balance |
...
| B9 | control | F6 freeze if instance migrates off a listed id (11611 and, after ops, 11619) |
...
| C3 | crate + A-mt/B-mt; **A-lcd/B-lt executed** | Pinned 11619 bytes on LocalTerra 2026-08-24 (`layer-a-lcd.json` / `layer-b-lt.json`; local store id is host-ephemeral — never list it) |
| C4 | pass | Known-bad FoT 1:1 / P2 stay red |
| C5 | pass (executed) | `LAYER_B_LT=1` ran A-lcd + B-lt on pinned 11619 (not a stub) |
...
| D11 | residual (SKU); crate #608; **in these bytes** | **Same-sender** in-block batch / ladder can still cooldown (**CH3** / **H608-8**). Pair-wide halt (H-3: pair `LAST_TRADE_BLOCK` blocking other wallets) is **fixed in this pin** |
...
| catalog F1–F8 | residual / #594 | Indexer must key on listed `code_id` (11611 today; 11619 after whitelist), CMM admin, launcher origin. Event vs extra-debit debit is a display concern, not inbound P2 |
...
- **#607 / #608 / #609 source:** option-2 hop trader, LaunchGuards per-wallet cooldown, manager-directory skip are **in these bytes** (`make verify-issue-607` · `608` · `609`).
- **A-mt:** `cw20_codeid_harness` mintable + mutants via `make verify-issue-589`.
- **A-lcd:** [`../../scripts/layer-a-lcd.sh`](../../scripts/layer-a-lcd.sh) **executed** 2026-08-24 — Transfer / TransferFrom 1:1, unauthorized mint/burn_from rejected, idle stable. `balance_at` A29 N/A. Local token `terra1zznq6…p7qkuuyqg` (do not list this address).
...
Launcher stamps CosmWasm admin = `cmm_governance` (CMM `terra16j5u6ey7a84g40sr3gd94nzg5w5fm45046k9s2347qhfpwm5fr6sem3lr2`). Manager cannot migrate. F6 freezes writes if an instance leaves a listed id. Honest upgrade: this report **GO** → whitelist 11619 → CMM migrate instances → Refresh. Zero instances at intake; no CMM migrate required to list.
