...
| **LT4** | Genesis test account: **11M LUNC** (10× + deploy headroom) + stablecoins per `init-chain.sh` (GitLab [#372](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/372)); post-`deploy-local` `test1` uluna ≈ **9M** (treasury bank send + deploy gas) | CW20 / E2E funding uses deploy + `e2e-provision-dev-wallet.sh`; requires **`make reset`** after genesis change (**LT6**) |
...
| **LT8** | Treasury bank send uses **`DEPLOY_TREASURY_FUND_COINS`** default **20M USTC + 2M LUNC** (GitLab [#372](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/372)) | Sized for **11M LUNC** genesis; override via env without code change |
