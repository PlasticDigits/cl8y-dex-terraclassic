...
| **LT4** | Genesis test account: **11M LUNC** (10× + deploy headroom) + stablecoins per `init-chain.sh` (GitLab #372) | CW20 / E2E funding uses deploy + `e2e-provision-dev-wallet.sh`, not 100M LUNC genesis |
...
| **LT8** | Treasury bank send uses **`DEPLOY_TREASURY_FUND_COINS`** default **20M USTC + 2M LUNC** (GitLab #372) | Genesis is **10M LUNC**; legacy 10M LUNC-only send fails on SDK 0.53 LocalTerra |
