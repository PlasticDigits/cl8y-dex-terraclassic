...
| 11 | **Indexer** — pair creation indexing does not rely on bank events | PASS | Pairs indexed via wasm instantiate / factory registry (not bank events). The separate treasury census (`pair_creation` on `GET /protocol/fees`) remains **unimplemented** under [#1209](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1209); its fee amount must come from the pinned factory's positive `creation_fee_uluna` attr, never instantiate gas, refunds, reply attrs, or a duplicate bank-send count. See [#1213](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1213), [#1269](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1269), [QA checklist](../qa/issue-1209/README.md), and [`AGENTS_INDEXER_FEE_LEDGER_HOME.md`](../../skills/AGENTS_INDEXER_FEE_LEDGER_HOME.md). |
