...
| Indexer `/health` git SHA + auto-deploy leftover | [#1276](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1276) | ADR [`0006-indexer-health-git-sha.md`](./adr/0006-indexer-health-git-sha.md); skill [`AGENTS_INDEXER_HEALTH_GIT_SHA.md`](../skills/AGENTS_INDEXER_HEALTH_GIT_SHA.md); `make verify-issue-1276` lands in the code slice | **H1276-1–H1276-8** — additive `git_sha`; empty/`GIT_SHA` whitespace falls through to `SOURCE_COMMIT`; never copy Coolify `HEAD` into `GIT_SHA`; ARG/ENV on **runtime** stage **after** `COPY --from=builder`; Coolify checkbox is operator leftover (#297), not inferred from HTTP. Live leftover: `VERIFY1276_REQUIRE_LIVE=1` (or `VERIFY1276_IID=1276`) — jq-parse `GET https://indexer.dex.cl8y.com/health` 200, `status=ok`, hex `git_sha`; omitted field is FAIL; leftover glance that should close #1276 **must** set `VERIFY1276_EXPECT_SHA` (prefix-match; IID-only / regex-only is bake presence). Leftover-complete = checkbox **and** tip-match. Unreachable host SKIP unless that flag. Ok-only tests `remove_var` both keys. Watch paths off until leftover closed. No LocalTerra. Do not scrape Coolify |
