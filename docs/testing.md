...
| Indexer `/health` git SHA + auto-deploy leftover | [#1276](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1276) | ADR [`0006-indexer-health-git-sha.md`](./adr/0006-indexer-health-git-sha.md); skill [`AGENTS_INDEXER_HEALTH_GIT_SHA.md`](../skills/AGENTS_INDEXER_HEALTH_GIT_SHA.md); `make verify-issue-1276` lands in the code slice | **H1276-1–H1276-8** — additive `git_sha`; empty `GIT_SHA` falls through to `SOURCE_COMMIT`; ARG/ENV on **runtime** stage; Coolify checkbox is operator leftover (#297). Live leftover: `VERIFY1276_REQUIRE_LIVE=1` (or `VERIFY1276_IID=1276`) — `GET https://indexer.dex.cl8y.com/health` 200, `status=ok`, `git_sha` `^[0-9a-f]{7,40}$`; omitted field is FAIL; unreachable host SKIP unless that flag. No LocalTerra. Do not scrape Coolify. Do not assert the checkbox from HTTP |
