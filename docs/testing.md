...
| Indexer `/health` git SHA + auto-deploy leftover | [#1276](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1276) | ADR [`0006-indexer-health-git-sha.md`](./adr/0006-indexer-health-git-sha.md); skill [`AGENTS_INDEXER_HEALTH_GIT_SHA.md`](../skills/AGENTS_INDEXER_HEALTH_GIT_SHA.md); `make verify-issue-1276` lands in the code slice | **H1276-1–H1276-8** — additive `git_sha`; empty/`GIT_SHA` whitespace falls through to `SOURCE_COMMIT`; never copy Coolify `HEAD`/`main` into `GIT_SHA`; ARG/ENV on **runtime** stage **after** `COPY --from=builder`; Coolify checkbox is operator leftover (#297), not inferred from HTTP. **Bake presence:** `VERIFY1276_REQUIRE_LIVE=1` without IID/`EXPECT_SHA` may PASS hex `git_sha`. **Leftover-complete:** `VERIFY1276_IID=1276` or `VERIFY1276_LEFTOVER_COMPLETE=1` **FAIL** if `VERIFY1276_EXPECT_SHA` unset (do not close on bake presence under the IID). Rollout leftover: `VERIFY1276_REQUIRE_LIVE=1 VERIFY1276_EXPECT_SHA=$(git rev-parse HEAD) make verify-issue-1276`. Issue AC: keep checkbox; strengthen baked-commit to tip-match; leftover does not HTTP-attest the Vite app. Ok-only tests `remove_var` both keys (`generic_health_unchanged`). Watch paths off until leftover closed. No LocalTerra. Do not scrape Coolify |
