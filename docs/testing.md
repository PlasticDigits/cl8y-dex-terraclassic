...
| Trader rolling raw volume type | [#1277](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1277) | [`make verify-issue-1277`](../Makefile) (target may land with the implement MR): `cd indexer && cargo test --test indexer_volume_window_decay --test volume_usd_catalog --test api_traders --test indexer_trader_rolling_volume_numeric -- --test-threads=1`; ADR [`0005-trader-rolling-volume-numeric.md`](./adr/0005-trader-rolling-volume-numeric.md); skill [`AGENTS_INDEXER_TRADER_ROLLING_VOLUME.md`](../skills/AGENTS_INDEXER_TRADER_ROLLING_VOLUME.md) | **V1277-1–V1277-8** — `NUMERIC(38, 0)` + `LEAST(10^38-1)` + heal missing/stale/leftover-lifetime senders from `swap_events` before **D5** refresh (one tx including leftover-USD); `bd_plain_string` unscoped + pair-scoped; canonical I10 temp overflow + leftover heal (missing sender + register-then-overflow + leftover-lifetime ghost; registered zeros do not trip; no live `ALTER TABLE traders`); **A1**. Postgres only. Design: [`design/1277-trader-rolling-volume-numeric.md`](../design/1277-trader-rolling-volume-numeric.md) |
