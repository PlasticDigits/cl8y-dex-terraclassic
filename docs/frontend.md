...
On-card copy stays short: labels ≤ ~5 words, blocking errors ≤ 1 sentence, optional **Docs** link for depth — no instructional paragraphs on primary trade cards ([#489](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/489)). Shared terminology: [`design-system.md` § Terminology glossary](./design-system.md#terminology-glossary). Agent playbook: [`skills/AGENTS_FRONTEND_COPY_COGNITIVE_LOAD.md`](../skills/AGENTS_FRONTEND_COPY_COGNITIVE_LOAD.md). Required risk ack, footer NFA, and trust-boundary warnings stay visible.
...
| Header menus above ribbon | Inside `.app-top-sticky` (`isolation: isolate`), `.app-header-shell` stacks **above** `.app-env-ribbon` so open **More** (`.app-menu`) and **wallet** (`.wallet-menu`) paint over the ribbon band — not under it. Do not “fix” by shrinking the content-column ribbon or hiding it when menus open ([GitLab **#486**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/486)). |
...
| **Rate label + chips (#488/#489)** | Primary card uses **When 1 {token} is worth** (token0 symbol). **% chips** (0/+1/+5/+10 vs ref) sit under the rate field. No on-card instructional essay and no **Place limit ⓘ** tooltip; invalid-direction errors stay inline. |
...
| **Button copy** | **`/trade`** and **`/limits`** both use **Buy {base}** / **Sell {base}** (token0 display symbol) via [`tradeDirectionSideLabels.ts`](../frontend-dapp/src/utils/tradeDirectionSideLabels.ts) ([#412](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/412), [#489](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/489)). Historical retail copy **Bid (escrow …)** / **Ask (escrow …)** on the place card is retired; **bid**/**ask** remain on-chain enums and order-book column titles only. |
...
**Calculating… / slow multihop ([#484](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/484), [#485](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/485)):** Periodic sim refresh uses [`simQuoteRefetchInterval`](../frontend-dapp/src/utils/quoteDebounce.ts) so React Query does **not** cancel/restart an in-flight quote every 10s (default `cancelRefetch`). Swap **You Receive** shows Calculating only via [`shouldShowSimReceiveCalculating`](../frontend-dapp/src/utils/quoteDebounce.ts) (`isFetching && !simData`) — background refetch keeps the prior amount. Indexer `GET`/`POST` `/route/solve` use a **45s** client timeout (`INDEXER_ROUTE_SOLVE_TIMEOUT_MS`) and accept an AbortSignal from the sim `queryFn`. **#485:** while the first CW20 multihop quote is in flight, Swap polls `GET /api/v1/route/solve/progress` (~1 Hz) and may replace static Calculating with indexer `label` (e.g. Searching 2 of 5 paths…) after ~500ms; `aria-live="polite"` for screen readers. Agent checklists: [`skills/AGENTS_FRONTEND_SWAP_QUOTE_REFETCH.md`](../skills/AGENTS_FRONTEND_SWAP_QUOTE_REFETCH.md), [`skills/AGENTS_INDEXER_ROUTE_SOLVE_PROGRESS.md`](../skills/AGENTS_INDEXER_ROUTE_SOLVE_PROGRESS.md).
...
| **Receive stale-OK (#484)** | Prior receive amount stays visible during background refetch; Calculating is for first load / no settled quote only. |
