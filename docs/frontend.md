...
| **P1317-1–P1317-8** CMM LP | After Vol/LP: **CMM v2 LP**, **30d fee bps**, **Vol/CMM LP**. Time-weighted CMM shares at current pool USD per share. Fee bps exclude wrap and UST1 window. Volume multiple uses `formatVolumePerTvl` (`2.469` → `2.47×`). Missing keys → **—**. `whitespace-nowrap` inside the existing horizontal scroll. No farm / APR. [#1317](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1317) / [ADR 0012](./adr/0012-cmm-held-v2-lp-fee-bps.md). |
