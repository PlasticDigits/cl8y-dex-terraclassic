...
| **P1263-1–P1263-8** Top pairs | At most five factory pairs by trailing 30d priced USD (`GET /api/v1/protocol/top-pairs`). Columns: Pair \| 30d vol \| v2 LP \| Vol/LP (`N×` or **—**). Gems out. Ratio is server-side; missing TVL → **—**. Pair links via `chartsPairHref`. Flat table, no per-row `card-glass`. Hide on 404. Not `/pool` 30d, not a vote/farm. CMM columns are **P1317** and do not replace Vol/LP. |
| **P1317-1–P1317-8** CMM LP | After Vol/LP: **CMM v2 LP**, **30d fee bps**, **Vol/CMM LP**. Time-weighted CMM shares at current pool USD per share. The first LCD anchor stays fixed. Fee bps exclude wrap and UST1 window. Volume multiple uses `formatVolumePerTvl` (`2.469` → `2.47×`). Missing keys → **—**. `whitespace-nowrap` inside the existing horizontal scroll. No farm / APR. [#1317](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1317) / [ADR 0012](./adr/0012-cmm-held-v2-lp-fee-bps.md). |
