...
| Production build guard | `vite.config.ts` throws if `VITE_DEV_MNEMONIC` is present for any `vite build` unless `mode === 'development'` or `VITE_ALLOW_DEV_MNEMONIC=local-only` (prevents inlining a real seed into `dist/`). Tracked in [GitLab #118](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/118), [#378](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/378). |
