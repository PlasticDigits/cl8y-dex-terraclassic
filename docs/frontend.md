...
- **Slippage tolerance is the on-chain guard:** **Slippage tolerance** (`max_spread` on pair/router messages) is the primary contract-level protection against sandwich and front-running losses. Keep it tight for large trades. The Swap Settings **retail** panel exposes slippage presets and a **High slippage increases front-running risk** warning when tolerance exceeds 5% ([#413](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/413)).
