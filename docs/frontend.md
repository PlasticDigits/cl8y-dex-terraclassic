...
| **C543-9** | Empty state when there is no positive USD bar **and** no positive `*_human` bar. Neither-catalog rows use **Price ({quote} per {base})** ([#1315](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1315)). A catalog pair with no positive USD stays on the empty state. No `NaN` / `Infinity` / negative axis. |
