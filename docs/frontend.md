...
| **Station extension + LocalTerra fee steps** | With **`VITE_NETWORK=local`**, [`wallet.ts`](../frontend-dapp/src/services/terraclassic/wallet.ts) calls **`window.station.keplr.experimentalSuggestChain`** using the same chain suggestion as Keplr (**`feeCurrencies[].gasPriceStep` = 28.325 / 28.325 / 50**). Station’s built-in LocalTerra entry can keep a **stale low minimum gas price**; the extension may then **recompute** `Fee.amount` as `gas_wanted × ~0.015` (~3000 uluna on a 200k gas tx) despite `preferNoSetFee` + correct dApp math — [GitLab #127 follow-up](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/127). Approve the chain update in Station if prompted; reconnect after changing RPC/LCD env. **QA:** exercise limit/bid (and allowance-first flows) with **both Station and Terra Classic Keplr** on LocalTerra — behavior can differ by extension even when fee math in-app matches. |
