...
| **M626** Migrate Token | `/token/migrate` is free (no invoice). Source gate is `VITE_COMMUNITY_MIGRATE_CODE_IDS` (default 6036,10184,8266,8654) — not factory whitelist. Create Token links **Migrate here** and stays code-id-free (**M628-7**). Retail lead is one sentence (address stays); no env-var / 50 UST1 / cw2 essay on the card. Launcher stays CMM-only. Post-merge leftovers: [#628](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/628) / `make verify-issue-628`. |
