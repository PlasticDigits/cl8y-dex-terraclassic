...
Regression coverage: [`frontend-dapp/src/viteConfig.build.test.ts`](../frontend-dapp/src/viteConfig.build.test.ts) (loads `vite.config.ts` via `loadConfigFromFile`; production-allow cases pin `VITE_DEV_MODE` so LocalTerra `.env.local` cannot re-inject `true` — [#698](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/698)).
...
[`/charts`](../frontend-dapp/src/pages/ChartsPage.tsx) **Trader leaderboard** is the shared [`TraderLeaderboard`](../frontend-dapp/src/components/trader/TraderLeaderboard.tsx) with `pairAddress` ([GitLab **#666**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/666) **CS-14**): `GET /api/v1/traders/leaderboard?pair={addr}&sort=…`. Volume tab is **USD-only** ([GitLab **#553**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/553)). The **Trader** cell is 4/6 + blockie ([#656](#trader-identity)). Hide **Best Trade**. Empty copy: “No traders on this pair yet”. Unscoped `GET /leaderboard` (no `pair`) stays DEX-wide for `/trader` / ops ([#657](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/657)). Agent playbooks: [`skills/AGENTS_FRONTEND_TRADER_LEADERBOARD.md`](../skills/AGENTS_FRONTEND_TRADER_LEADERBOARD.md), [`skills/AGENTS_FRONTEND_TRADER_VOLUME_USD.md`](../skills/AGENTS_FRONTEND_TRADER_VOLUME_USD.md), [`skills/AGENTS_FRONTEND_TRADER_IDENTITY.md`](../skills/AGENTS_FRONTEND_TRADER_IDENTITY.md), [`skills/AGENTS_FRONTEND_CHARTS_PAIR_SCOPED.md`](../skills/AGENTS_FRONTEND_CHARTS_PAIR_SCOPED.md).
...
| Production `VITE_DEV_MODE` reject | `vite build --mode production` **throws** if `VITE_DEV_MODE=true`. Unset it in Coolify / `.env.production`. `VITE_ALLOW_DEV_MNEMONIC=local-only` does **not** permit the flag. Development / LocalTerra / Playwright and `vite build --mode staging` may keep it for Simulated Wallet ([#695](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/695) / FE-01). Vitest `loadConfigFromFile` must pin `process.env.VITE_DEV_MODE` to `''` or `'false'` so `.env.local` cannot re-inject `true` ([#698](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/698)). |
...
**Third-party / agent context:** [`skills/AGENTS_BUNDLE_DEV_WALLET.md`](../skills/AGENTS_BUNDLE_DEV_WALLET.md) · [`skills/AGENTS_FRONTEND_DEV_MODE_GUARD.md`](../skills/AGENTS_FRONTEND_DEV_MODE_GUARD.md) ([#695](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/695)). Regression: `make verify-issue-695`.
...
| Build guards | No `VITE_DEV_MNEMONIC` in non-dev builds; `VITE_WC_PROJECT_ID` required for production `vite build`; no `VITE_DEV_MODE=true` in production mode ([#695](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/695)) | [`vite.config.ts`](../frontend-dapp/vite.config.ts), [`viteConfig.build.test.ts`](../frontend-dapp/src/viteConfig.build.test.ts) |
...
Limit placement, ladder placement, and price relink include a `25_000 × max_adjust_steps` allowance in the gas limit; batch placement sums the per-rung step caps because skipped rungs may retry a bounded head walk before a cursor exists. The same selected caps feed the fee estimate, native LUNC preflight, Max reserve, and ladder summary (**#1329**). The dApp derives these from the execute message and its preset state; query-string `gas` / `credit` fields are not parsed or signed. Hybrid fill remains on the separate quote-driven maker/scan envelope. See [`docs/limit-orders.md § Place / cancel`](./limit-orders.md#place--cancel-limit-gitlab-206) and [`skills/AGENTS_HYBRID_LIMIT_GAS.md`](../skills/AGENTS_HYBRID_LIMIT_GAS.md).
...
**Measured CL8Y → UST1 exception ([#1328](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1328), [ADR 0013](./adr/0013-cl8y-ust1-gas-floor.md)):** the current Columbus-5 quote is a two-hop pool-only router path. Forward LCD simulation used **2,643,979–2,643,980** gas; the generic envelope was **1,910,000**. `getGasLimitForTx` raises only this continuous two-hop pool-only endpoint pair to **3,000,000**. The middle token may change; hybrid/book, other two-hop routes, and wrap+2hop limits stay on their existing paths. Verify: `make verify-issue-1328`; the signed forward swap and no-toast check are tracked as operator QA.

Hybrid hops use quote-driven limits in [`hybridSwapGas.ts`](../frontend-dapp/src/services/terraclassic/hybridSwapGas.ts) (sum per hop for router paths; flat **15M** only when `max_maker_fills` is unknown **and no hop has parseable params** — [GitLab **#249**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/249), mixed-hop rule [#679](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/679)). Hops **without** `hybrid` (or `book_input = 0`) use the pool-only router floor — **not** 15M per empty hop. When `book_input > 0`, **`bookWalkScanOverheadGas`** budgets worst-case **`MAX_SCAN_STEPS`** (500) and **`MAX_EXPIRED_PARKS_PER_SWAP`** (15) offline ([#260](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/260), [#254](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/254), [#262](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/262)); see [`hybridBookWalkLimits.ts`](../frontend-dapp/src/services/terraclassic/hybridBookWalkLimits.ts). Single-hop with a known pair must not use the router ([`swapRouting.ts`](../frontend-dapp/src/services/terraclassic/swapRouting.ts)).
...
| Wrap + ≥2hop combo ([#587](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/587) / [#1264](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1264), closed) | Combined `wrap_deposit` + CW20 `send`→`execute_swap_operations` (N≥2) adds **`WRAP_ROUTER_COMBO_OVERHEAD_GAS` (400k)** in `totalGasLimitForExecuteMsgs`. Wrap+2hop = **2,710,000** (~76.76 LUNC at 28.325) for **LUNC→USTR and USTC→USTR**. USTC→USTR columbus-5 AC1 used **2,630,228** under the 2,710,000 envelope; keep the envelope unchanged. Wrap+1hop stays **1.8M** (#353). Malformed `send.msg` **throws** (`SendHookGasDecodeError`) instead of silent 600k. Native path stays **pool-only** (no hybrid / `book_input`). **G1264-4:** Swap Max / LUNC gas-gate treat only `uluna` as fee-paying native — `uusd` wraps but the Network-fee reserve is bank LUNC (`nativeWrapSwapHints.ts`). Fixture `wrap_plus_send_2hop_ustc`. Verify: `make verify-issue-587` · `make verify-issue-1264`. Measurement: [#1264 AC1](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1264). |
...
| Mixed hybrid + pool router hops ([#679](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/679)) | **G-AUTO-1–G-AUTO-7:** hops without `hybrid` (or `book_input = 0`) use the pool-only router floor (**950k**/hop), **not** `HYBRID_SWAP_GAS_LIMIT` (15M). `gasLimitForHybridParams(undefined)` is a **tx-level** last resort when **no** hop has parseable params. Envelope is `max(router N-hop floor, sum of per-hop pieces)` plus **`MIXED_HYBRID_ROUTER_HEADROOM_GAS` (2,150,000)** when the path is mixed. Live columbus-5 cLUNC→USTR 4-hop (hop1 book/8 makers, hops 2–4 pool-only): `gas_used` **5,026,176**, envelope **6,785,500** (~192.20 LUNC at 28.325) — hash `AB8BE4F75E051837BB01C364DEDE6611727E47F0F857AADF04B17C39F360446D`. Must stay **> 5,026,176** and **< 15M** (not 46.8M). Hybrid-on-every-hop 2-hop stays **3,058,600**. Inventory: `send_4hop_hybrid_first_pool_rest`. Swap **Network fee** uses the same indexer ops as submit (`cw20RouterOperations`). Native wrap+N-hop stays pool-only (**H596-7**). Swarm `gas.ts` lockstep. Verify: `make verify-issue-679`. |
| Swap Network fee row ([#587](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/587) / [#679](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/679) / [#1264](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1264)) | When a quote is ready, Swap shows **Network fee (est.) ~X LUNC** (`swap-network-fee`) from the same envelope as broadcast. Fee denom is **LUNC / `uluna` only** — 0 USTC does not block a LUNC-funded swap. AMM **Fee** row stays pool `fee_bps`. Optional `gas × price` internals only inside **Trade details**. CW20 multi-hop must pass indexer `operations` (per-hop `hybrid`) so the hint cannot sit on the pool-only 4-hop **3.81M / ~108 LUNC** while broadcast would OOG or over-charge. Native LUNC **Max** uses the same wrap+N-hop hints (`hopCount` defaults to **2** until the route is known). Native **USTC** Max stays full `uusd` (**G1264-4**); hopCount still defaults to **2** for wrap+swap (**G1264-5**). Submit preflight: bank LUNC ≥ pay+fee (uluna pay) or fee (CW20 / USTC). |
| Station mobile / WC auto-gas ([#679](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/679) **G-AUTO-8**) | Station **extension** on columbus-5: `preferNoSetFee` + amino `fee.gas` / `fee.amount` — when honored, wallet Tx Fee matches the dApp envelope. Station **WalletConnect / mobile `post`** stays atomic (`isAtomicWalletConnectPost`); that app may still LCD-simulate CosmWasm and show ~**4,158 LUNC** (~147M gas) on a 4-hop book walk. **Residual:** do not treat LCD simulate as the envelope; dApp **Network fee (est.)** is the source of truth (~200 LUNC class). Trade details one-liner (`swap-wallet-fee-note`). Do **not** enable mainnet SEC-E08 (#429). Station LocalTerra QA is not the gate (#235). |
...
| **C593-9** Extra-debit Max | Swap/Trade sell max reduced by sell tax on **pair-direct and router-hop** sells (**T592-13**). Manager-directory wallets skip extra-debit (**#609** / **E609-7**); unknown exempt stays fail-closed. Typed 100% / reverse offer still must not submit when `TaxPreview.debit > balance` (**S1267**, [#1267](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1267) / [`AGENTS_FRONTEND_EXTRA_DEBIT_SELL.md`](../skills/AGENTS_FRONTEND_EXTRA_DEBIT_SELL.md); `make verify-issue-1267`). |
| **S1267-1** LCD debit is a floor | When `sell_bps > 0`, debit is `max(TaxPreview.debit, local extraDebitFromDeclared)`. Honest LCD (`debit === declared`) cannot enable a 100% tax sell. Classic `amount > balance` stays Insufficient Balance. No pair/router FoT. |
| **S1267-2** Max round-trip | Max declared stays `debit ≤ balance` after `fromRawAmount` → `toRawAmount`. |
| **S1267-3** Pin ≠ detector | Catalog `VITE_COMMUNITY_TAX_CODE_ID` is Create/Manage only. Sell detection is live `GetConfig.sell_bps`. |
| **S1267-4** Unknown exempt | Manager skip only when `true`. Loading/error keep extra-debit. |
| **S1267-5** Pair + router | Same extra-debit gate; preview `to` is pair or official router. |
...
| **C605-3** AutoLP create | Auto liquidity at create instantiates+binds the sister when launcher `autolp_code_id` is set. Unset → create blocked for that SKU (no 50 UST1 no-op). Manage includes `autolp` only on a sister `GetConfig` delta (no blank→`1` / wallet default) (**T592-4**, [#1237](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1237)). `SkimToLp` stays permissionless and is never called from `Transfer`/`Send` (**T592-10**). Pair must be this token’s factory-listed CL8Y pool; skim has a spread floor (**M610**, [#610](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/610)). |
...
[`ProtocolPage.tsx`](../frontend-dapp/src/pages/ProtocolPage.tsx) is the DEX census + reference-oracle surface ([GitLab **#550**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/550) / [**#556**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/556) / [**#569**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/569) / [**#570**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/570) / [**#586**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/586) / [**#652**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/652) / [**#667**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/667) / [**#1263**](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1263)). `/charts` does **not** duplicate this census ([#666](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/666)). Page order: title → **Global stats** (`protocol-global-stats`, tiles + optional UTC-day volume chart) → **Top pairs (30d)** (`protocol-top-pairs`, [#1263](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1263)) → **Protocol fees** (`protocol-fee-stats`) → **DEX hub prices** (`protocol-dex-hub-prices`) → **one** CEX oracle card (`protocol-oracle`) → on-chain contracts (audit, [#378](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/378)) → hook events. Global stats / fees / oracle **stat chips** use `StatBox variant="flat"` ([#653](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/653)). Oracle sources/history and hook events may keep **one** table well.

| Invariant | Meaning |
|-----------|---------|
| **P550-1 Order** | Stats → **Top pairs (30d)** (`protocol-top-pairs`, [#1263](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1263); pending extension [#1317](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1317)) → **fees** (`protocol-fee-stats`, #586) → **DEX hub card** (#556 / #570) → CEX oracle. Do not merge factory/router into stats or fees. Do not clone `AddressRow` onto Swap confirmation. Do not add USTR as a fourth CEX tab. |
| **P1263-1–P1263-8** Top pairs | At most five factory pairs by trailing 30d priced USD (`GET /api/v1/protocol/top-pairs`). Columns: Pair \| 30d vol \| v2 LP \| Vol/LP (`N×` or **—**). Gems out. Ratio is server-side; missing TVL → **—**. Pair links via `chartsPairHref`. Flat table, no per-row `card-glass`. Hide on 404. Not `/pool` 30d, not a vote/farm. |
| **P1317-1–P1317-5** CMM top-pairs extension (pending) | The current UI/API still has only the four #1263 columns. Keep those cells and ranking unchanged. When #1317 is implemented, add CMM LP, 30d trading-fee bps, and Vol/CMM LP as `×`; show missing values as **—**, retain horizontal scroll on phone widths, and explain that the fee numerator excludes wrap/window fees. No APR/farm claim. Backend contract: [`skills/AGENTS_INDEXER_PROTOCOL_TOP_PAIRS.md`](../skills/AGENTS_INDEXER_PROTOCOL_TOP_PAIRS.md) (**I1317**). |
| **PFee-1–PFee-12** / **PFee-13** / **PFee-14** Fees | Trailing 24h/7d/30d treasury USD with **inline** flow Δ% in the same tile ([#652](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/652)). Chips sit with that tile’s USD, not under the next fee label ([#667](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/667)). Source + token tables from `GET /api/v1/protocol/fees`. Retail labels include **UST1 mint** / **UST1 redeem** when `ust1_window_configured` ([#614](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/614) / **I614**). Idle `$0`; unpriced `—`; never `Infinity`. Not `traders.total_fees_paid`. Wrap/Unwrap rows appear when ingest sees mapper `notify_deposit` / `unwrap` `fee` ([GitLab #613](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/613)). CL8Y / listed-economic ask-side fees show human + `$` when the indexer stamps `fee_usd` ([#683](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/683)). Testids: `protocol-fee-stats`, `protocol-stat-fees-24h` / `7d` / `30d` (Δ% child `-chg`), `protocol-fees-by-source`, `protocol-fees-by-token`. Flat `StatBox` — no nested `card-glass`. |
...
| **P569-1 Pool TVL** | **Total liquidity** is humanized AMM `pair_reserves` USD (`total_liquidity_usd`), not volume, not CG `liquidity_in_usd`, not book escrow. One cell: USD + **one** inline 24h snapshot Δ% (`protocol-stat-liquidity`; child testid `protocol-stat-liquidity-24h`). Do **not** render a 30d chip on this tile ([#677](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/677)). Δ% is visually grouped with the `$` (`justify-start` / wrap), not `justify-between` to the next column ([#667](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/667) **P667-1**). `liquidity_change_30d_pct` stays on `GET /overview`. |
| **P569-2 Δ%** | 24h liquidity is vs indexer snapshots. Missing / `null` / non-finite → em-dash (`formatProtocolPct`), never `0%` / `Infinity`. |
...
Volume boxes use **Last 24h / 7d / 30d vol** plus static trailing-window `title` / `aria-label` — not calendar buckets and not a lecture in the lead paragraph ([#576](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/576), [`trailingWindowCopy.ts`](../frontend-dapp/src/utils/trailingWindowCopy.ts)). Each volume tile also shows prior-window flow Δ% (`volume_change_*_pct`). The Δ% chip sits immediately after (or wrapped under) that tile’s USD — not `justify-between` into the next column’s gutter ([#667](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/667) **P667-1–P667-4**). Census tiles (tokens / pairs / trades) are integer locale counts (`14`, not `14.00`). A UTC **Hourly / Daily / Monthly** bar chart (`protocol-volume-daily-chart`, default Daily) with a **Volume / Liquidity / Fees** metric toggle (default Volume; GitLab [#689](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/689) **P689-1–P689-8**) and a visible USD value axis and pointer/keyboard tooltip is additive and must not be read as the trailing tiles ([#652](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/652) **P652-1–P652-7**, [#668](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/668) **P668-1–P668-9**). The plot SVG **fills the Global stats panel width** (viewBox tracks `ResizeObserver`; do not letterbox a 320×128 canvas). Liquidity bars are **stock** (last snapshot in the UTC bucket; missing → outlined / em-dash, not `$0`). Fees bars are **flow** treasury USD. X-axis labels every bar or every second bar (Daily/Monthly step 1 or 2; Hourly may widen when step 2 collides) ([#677](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/677)). Phone Monthly requests the last **6–12** UTC calendar months (desktop up to **24**) with horizontal `YY-MM` ticks — do not rotate labels ([#703](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/703) **P703-1–P703-8**). Hide the chart when `GET /api/v1/protocol/volume/daily` is missing (old indexer). Missing liquidity/fees sibling routes hide that metric’s plot only. Do not call `GET /defillama/daily` from this page. Do not mount `PriceChart`. Bar count follows plot width (hourly ≤ 168, daily ≤ 90, monthly ≤ 24; phone monthly ≤ 12).
Regression: `make verify-issue-1263` · `make verify-issue-1240` · `make verify-issue-1290` · `make verify-issue-1302` · `make verify-issue-1306` · `make verify-issue-703` · `make verify-issue-689` · `make verify-issue-677` · `make verify-issue-668` · `make verify-issue-667` · `make verify-issue-652` · `make verify-issue-550` · `make verify-issue-569` · `make verify-issue-586` · `make verify-issue-614` · `make verify-issue-570` · `make verify-issue-576` · `make verify-issue-571`. Playbook: [`skills/AGENTS_FRONTEND_PROTOCOL_STATS.md`](../skills/AGENTS_FRONTEND_PROTOCOL_STATS.md), [`skills/AGENTS_INDEXER_PROTOCOL_TOP_PAIRS.md`](../skills/AGENTS_INDEXER_PROTOCOL_TOP_PAIRS.md), [`skills/AGENTS_INDEXER_UST1_WINDOW_FEES.md`](../skills/AGENTS_INDEXER_UST1_WINDOW_FEES.md), [`skills/AGENTS_FRONTEND_PROTOCOL_HUB.md`](../skills/AGENTS_FRONTEND_PROTOCOL_HUB.md), [`skills/AGENTS_FRONTEND_TRAILING_WINDOW.md`](../skills/AGENTS_FRONTEND_TRAILING_WINDOW.md). Oracle API: [`runbooks/indexer-external-oracle.md`](./runbooks/indexer-external-oracle.md). Venus redeem: [`skills/AGENTS_INDEXER_VENUS_VFDUSD.md`](../skills/AGENTS_INDEXER_VENUS_VFDUSD.md). Overview TVL / fee / volume-Δ% rollup: [`runbooks/overview-global-stats-brin.md`](./runbooks/overview-global-stats-brin.md).
...
| **Positions API** | `GET /api/v1/traders/{addr}/positions` — returns `[]` for flat/unknown traders; not on-chain balances. Mixed 6/18 raw storage + `trade_count` parity with `/trades` is [#676](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/676) (**P676-1–P676-8**). |
| **P676-1** | Indexer `trader_positions` is `NUMERIC(78, 18)`. Do not assume a missing 18-dec pair means the wallet is flat — compare `/trades`. |
...
| **P&amp;L semantics** | **Realized** is closed quote sales (indexer `realized_pnl`). **Mark** is current hub USD of remaining quote; **unrealized** is that mark minus on-DEX cost (holdings vs cost — [#675](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/675) **P675-1–P675-8**). Remaining quote with zero cost/avg shows **No cost basis**, not an invented figure. Not wallet balances. Per-pair amounts are **human token units** with the token symbol; cross-pair header totals are **USD** from `GET /api/v1/hub-prices` (UST1/USTR/cUSTC; never `$1` / `2.5×`) or **—** when units differ / unpriced ([#551](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/551) **P551-1–P551-5**, [#560](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/560) **P560-1–P560-6**). Best/Worst is **—**, not `N/A`. |
| **Hide test-gem performance (#674)** | Default Open Positions, header realized/unrealized P&amp;L (from those visible rows), and Recent activity **omit** soft-launch gems. **Show test pairs** reveals them under a **Test pairs** divider. See [P674](#portfolio-hide-test-gems). |
...
**Tests:** [`PortfolioPage.test.tsx`](../frontend-dapp/src/pages/PortfolioPage.test.tsx), [`portfolioPerformanceFilter.test.ts`](../frontend-dapp/src/utils/__tests__/portfolioPerformanceFilter.test.ts), [`traderPositionDisplay.test.ts`](../frontend-dapp/src/utils/__tests__/traderPositionDisplay.test.ts), [`TraderPositionsTable.test.tsx`](../frontend-dapp/src/components/trader/TraderPositionsTable.test.tsx), [`TraderSummaryStats.test.tsx`](../frontend-dapp/src/components/trader/TraderSummaryStats.test.tsx), [`usePortfolioLpBalances.test.ts`](../frontend-dapp/src/hooks/__tests__/usePortfolioLpBalances.test.ts), [`client.test.ts`](../frontend-dapp/src/services/indexer/__tests__/client.test.ts) (`getTraderPositions`, `getTraderLimitPlacements`), [`e2e/portfolio.spec.ts`](../frontend-dapp/e2e/portfolio.spec.ts), indexer [`api_traders.rs`](../indexer/tests/api_traders.rs), [`position_tracker_18dec.rs`](../indexer/tests/position_tracker_18dec.rs). Regression: `make verify-issue-674`, `make verify-issue-551`, `make verify-issue-560`, `make verify-issue-675`, `make verify-issue-665`, `make verify-issue-676`.

**Third-party / agent context:** [`skills/AGENTS_FRONTEND_PORTFOLIO.md`](../skills/AGENTS_FRONTEND_PORTFOLIO.md), [`skills/AGENTS_FRONTEND_PORTFOLIO_TEST_PAIRS.md`](../skills/AGENTS_FRONTEND_PORTFOLIO_TEST_PAIRS.md), [`skills/AGENTS_FRONTEND_PORTFOLIO_PNL.md`](../skills/AGENTS_FRONTEND_PORTFOLIO_PNL.md), [`skills/AGENTS_FRONTEND_HUB_PNL.md`](../skills/AGENTS_FRONTEND_HUB_PNL.md), [`skills/AGENTS_FRONTEND_PORTFOLIO_UNREALIZED.md`](../skills/AGENTS_FRONTEND_PORTFOLIO_UNREALIZED.md), [`skills/AGENTS_INDEXER_TRADER_POSITIONS_DECIMALS.md`](../skills/AGENTS_INDEXER_TRADER_POSITIONS_DECIMALS.md), [`skills/AGENTS_FRONTEND_SHARE_LINK.md`](../skills/AGENTS_FRONTEND_SHARE_LINK.md), [`skills/AGENTS_FRONTEND_TRADER_IDENTITY.md`](../skills/AGENTS_FRONTEND_TRADER_IDENTITY.md).

#### Hide test-gem performance {#portfolio-hide-test-gems}

Soft-launch gems are not economic P&amp;L ([GitLab **#674**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/674)). Overlay in the dApp; indexer JSON stays raw. Classifier is the same `#534` set used by pickers.

| ID | Rule |
|----|------|
| **P674-1** | Reuse `isTestPair` / `isGemTokenId` / `GEM_SYMBOLS` + `COLUMBUS5_GEM_ADDRESSES`. No second gem list. Address in the gem set wins over a spoofed `symbol=UST1`. |
| **P674-2** | Default `/portfolio` Open Positions, header realized P&amp;L USD (visible rows only), and Recent activity omit test gems. Economic assets stay. |
| **P674-3** | **Show test pairs** appears when any hidden test position or recent-activity trade exists. Default off. Not `?showGems=`. |
| **P674-4** | Toggle on: economic rows first, then a **Test pairs** divider, then gems. Header P&amp;L includes gem rows while shown. |
| **P674-5** | `/trader` still lists every pair. Open limits and LP overview stay unfiltered. |
| **P674-6** | UST1 / CL8Y / wrap / vFDUSD are never gems (**U6** / **P534-8**). Missing metadata (`symbol=—`) is not a gem. |
| **P674-7** | `traders.total_volume_usd` stays indexer-wide. Do not invent a client gem subtract. Fees / best / worst stay **—**. |
| **P674-8** | No always-on lecture banner. Helper: [`portfolioPerformanceFilter.ts`](../frontend-dapp/src/utils/portfolioPerformanceFilter.ts). |

Regression: `make verify-issue-674`. Playbook: [`skills/AGENTS_FRONTEND_PORTFOLIO_TEST_PAIRS.md`](../skills/AGENTS_FRONTEND_PORTFOLIO_TEST_PAIRS.md). Production discovery hide remains [#562](#production-hide-test-tokens).
...
| **Liquidity badge** | Options show indexed 24h **USD** volume (`volume_usd_24h`, `formatPairListVolumeUsd`) when priced — never `formatNum(raw)` on quote (**P534-4** / [#692](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/692)). Hide the badge when the field is missing / **—**. |
...
| **P534-3** | **Volume within a group** prefers list JSON **`volume_usd_24h`** when present ([#692](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/692)). Else human quote volume `raw / 10^quoteDecimals` (mixed 6/18-dec safe). Do not sort 18-dec USTR raw against 6-dec cUSTC raw. |
| **P534-4** | **Vol badge / `/pool` Vol** render compact **USD** from `volume_usd_24h` (`formatPairListVolumeUsd` / `formatIndexedVolumeUsd`). Missing / invalid → **—**. Never `formatNum(raw)` on USTR (`19,297,048T`). Indexer `volume_quote_24h` stays a **raw** integer for integrators. |
...
`GET /api/v1/pairs?sort=volume_24h` remains raw-quote for API clients. The dApp overlays catalog rank on pickers **and** on the `/pool` table **default** ([#547](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/547) **P547-3**). User-chosen `/pool` column sorts (USD Vol, **v2 LP USD**, fee, created, name) use indexer `sort`/`order` and are **not** re-ranked by catalog. On production those pages still **omit gems** ([#562](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/562) **P562-3**). **v2 LP USD** is indexer `liquidity_usd` ([#655](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/655)). **24h Vol USD** is indexer `volume_usd_24h` ([#692](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/692)).
...
| **P562-7** | **Balances / exit.** `/trader`, `/limits` history, and LP rows still render gem symbols and amounts. `/portfolio` Open Positions / header realized P&amp;L / Recent activity **hide** gems by default with **Show test pairs** ([#674](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/674) **P674**). Deep link `/trade/<gem-pair-addr>` may resolve; optional static copy: **Legacy noneconomic market.** Do not put hidden gems back in global browse. Factory token universe remains the Swap gate (#481) — filter gems *inside* that set. |
...
| **Interval / timeframe** | `PriceChartLightweightCanvas` is created **once per mount**; interval changes call **`setData`** on the existing series then **`fitContent()`** once ([#705](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/705)). Stale async inits are dropped via **`chartInitIdRef`** ([GitLab **#148**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/148)). Pressed interval uses **`.price-chart-interval.tab-glass-active`** (stronger blue than generic `tab-glass-active`). |
| **Time-scale zoom / scroll (#336 / #705)** | Background candle refetches (`refetchInterval: 30_000`) must **not** reset user zoom/scroll. **`timeScale().fitContent()`** runs on initial mount, indicator toggle, and **interval switch** after the new series `setData` ([#705](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/705)) — not on routine data refresh or a sliding newest-N window at the same interval. Live updates use **`series.update()`** only for the **last** bar and/or appended bars via [`priceChartLightweightSeriesSync.ts`](../frontend-dapp/src/components/charts/priceChartLightweightSeriesSync.ts). **`setData`** is used for first load, interval switch, truncated history, or a rewrite of any historical bar (pair invert **#524** / USD invert **#543** keeps times and rewrites OHLC — `update` on an older bar throws `Cannot update oldest data`). Regression: [`priceChartLightweightSeriesSync.test.ts`](../frontend-dapp/src/components/charts/__tests__/priceChartLightweightSeriesSync.test.ts), [`PriceChartLightweightCanvas.test.tsx`](../frontend-dapp/src/components/charts/__tests__/PriceChartLightweightCanvas.test.tsx) — *background data refresh does not call fitContent*; *interval setData calls fitContent*. |
...
**USD invert (headline + candles, [#543](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/543)):** for a factory USD-of-`asset_0` series, displayed Last **and** each candle OHLC = factory `price_usd` when not inverted; when inverted = `invertUsd(price_usd, human_quote_per_base)` = `price_usd / price` **per bar**. A `usd_leg=asset_1` series is not that path: the wire omits `open/high/low/close`, the pane plots `subject_*` or `subject × H`, and `/charts` defaults both the pane and the pill to `asset_1` when there is no `?price=` and no Charts session flag (`defaultChartsDisplayInverted()` stays false for every other pair). **Never** `1 / price_usd` on a USD series. **Never** plot human quote-per-base on a control labeled **Price (USD)** (**P522-5**). Bars with NULL / non-positive USD are dropped from the USD series (no human fallback on that axis). A pair with no catalog leg plots `*_human` under **Price ({quote} per {base})** ([#1315](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1315), [ADR 0012](./adr/0012-alpha-pair-price-candles.md)). A pair that has a catalog leg stays on the empty state when USD is missing; it does not switch to that human heading. `usd_leg=asset_1` is already USD of `asset_1`: Trade `inverted` still drives the ticket and labels, and the chart plots stored OHLC or `stored × H` (high with human low). Do not send that series through `applyChartDisplayInvert`. 24h OHLC, TWAP, and tape Price stay on swap `price_usd`. SMA/RSI run on the **plotted** series. Volume stays quote/base volume. Indexer candle USD is **as-of** ingest or idle mark ([#568](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/568)); the dApp must **not** overlay CoinGecko or `GET /oracle/history` onto pair charts.
...
| **C543-9** | A catalog pair with no positive USD stays on the empty state even when `*_human` is present. Neither-catalog rows use **Price ({quote} per {base})** ([#1315](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1315)). Empty when that human series is also missing. No `NaN` / `Infinity` / negative axis. |
...
| **Order ticket — type tabs** | **Limit** vs **Market** compact text tabs on `/trade` (`TradeOrderTicket`, `.trade-order-text-tab`). Fresh mount defaults **Market** ([#693](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/693)). Market uses global slippage (`useDexStore`); quotes via indexer **`GET /route/solve`** (solver-optimized pool/book split, same as Swap — [#501](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/501), always-on [#596](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/596)); Advanced manual book leg uses `POST`; **no hybrid-off control**. Default path shows expected receive; min after slippage + Route extras live under Advanced ([GitLab **#152**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/152), [#414](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/414), [#693](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/693)). |
...
| **Market quote — route preview (#302)** | When a market amount is quoted, [`TradeMarketOrderPanel`](../frontend-dapp/src/components/trade/TradeMarketOrderPanel.tsx) shows a single **Route** row (`data-testid="trade-market-route-summary"`) under **Advanced** (`trade-market-quote-extras`), using the same `computeSwapRouteDisplay` helper and indexer-op precedence as Swap ([#158](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/158), [#693](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/693)). The default path keeps one expected-receive line (`trade-market-quote`). Row renders only when `marketRouteLine` is truthy; multihop paths appear when hybrid quoting returns indexer `router_operations`. Agent checklist: [`skills/AGENTS_FRONTEND_SWAP_ROUTE_DISPLAY.md`](../skills/AGENTS_FRONTEND_SWAP_ROUTE_DISPLAY.md). |
...
| **Market slippage presets** | Shared [`SlippageProtectionPresets`](../frontend-dapp/src/components/common/SlippageProtectionPresets.tsx): label above a `role="group"` 3-up grid (`data-testid="trade-market-slippage-presets"`). On `/trade` Market the group lives under **Advanced** ([#693](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/693)); closed Advanced still applies store default **5%**. Chips use `min-h-11` (~44px); `data-testid="trade-market-slippage-preset-{pct}"`. Do not wrap 0.5% onto the label row ([#528](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/528)). |
...
| **T563-6 Shared + blue CTAs** | Same [`LimitOrderBidAskSideSelector`](../frontend-dapp/src/components/trade/LimitOrderBidAskSideSelector.tsx) on `/trade` and `/limits`. `/trade` Limit/Market tabs are compact underlined text (`.trade-order-text-tab`, [#693](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/693)) — not `tab-glass*` pills. Place/Market CTAs stay `btn-primary`. Book Bid/Ask columns unchanged. |
...
**Third-party / agent context:** [`skills/AGENTS_FRONTEND_TRADE_TICKET_HEADING.md`](../skills/AGENTS_FRONTEND_TRADE_TICKET_HEADING.md). Ticket flatten / Market default: [`skills/AGENTS_FRONTEND_TRADE_TICKET_FLATTEN.md`](../skills/AGENTS_FRONTEND_TRADE_TICKET_FLATTEN.md) (`#693`). Footer dock: [`skills/AGENTS_FRONTEND_TRADE_TICKET_CTA_DOCK.md`](../skills/AGENTS_FRONTEND_TRADE_TICKET_CTA_DOCK.md) (`#527`).

### Trade page — ticket flatten (Market default, compact tabs, logo wash) {#trade-page-ticket-flatten}

`/trade` order ticket defaults to **Market**, uses compact underlined Market/Limit text (not full-width `tab-glass` pills), shows a **TokenLogo** + clamped token wash on the heading, unwraps Side / mode cards, and moves slippage plus non-blocking amount chrome into **Advanced** ([GitLab **#693**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/693)). Styling / IA only — escrow, quotes, hybrid always-on, and footer dock stay.

| Invariant | Meaning |
|-----------|---------|
| **T693-1 Default Market** | Fresh `/trade/:pair` (`terra1…`): `trade-order-tab-market` `aria-selected="true"`; Market pay field + footer Market CTA; no `limit-order-price-input` until Limit. Book **Edit** / limit draft still select Limit (**L561-11**). **Place another** stays Limit (#161). Pair remount → Market again. |
| **T693-2 Compact text tabs** | `.trade-order-text-tabs` / `.trade-order-text-tab`: small type, selected underline + `aria-selected`, idle muted, intrinsic width. Not `tab-glass*` and not a full-width 2-col bordered toolbar. `:focus-visible` (#144). Testids `trade-order-tab-limit` / `trade-order-tab-market` unchanged. |
| **T693-3 Logo + wash** | Heading row: sibling allowlisted [`TokenLogo`](../frontend-dapp/src/components/ui/TokenLogo.tsx) (or blockie) for the **displayed** base + `{verb} {displayBase}` text nodes. `Select a pair` has no logo. Wash from displayed-base id hash or local canvas sample — **never** `rgba(251, 146, 60, …)`. Invert updates logo + verb together. Buy/Sell and money CTAs are not painted with the wash. |
| **T693-4 Flatten** | No `TicketSection` / `TicketStat`. No **Side** title. No **Top buy** / **Top sell**. Buy/Sell radiogroup is a direct child of `trade-order-ticket-scroll`. One **Market** + Docs or **Limit** + Docs line (`trade-order-mode-docs`). No inner `<h3>Market</h3>`. |
| **T693-5 Advanced Market** | Default path: Pay + Max + one expected-receive line (`trade-market-quote` typography, not a nested `card-glass`) + blocking errors. Slippage chips (`trade-market-slippage-*`, #528 geometry when open), min-after-slippage / Route extras / pre-submit live under **Advanced**. Closed Advanced still applies store default **5%** to `max_spread`. |
| **T693-6 Advanced Limit** | Default path: price + Pay + Receive + invalid buy/sell. Expiry, % chips + ref/USD, pre-submit fee card under Advanced. `/limits` stays Limit-only with chips beside the price input. |
| **T693-7 Behavior** | Bid still escrows token1; ask token0. Click Buy still `onSideChange('bid')`. GET `/route/solve` (typed book → POST). Hybrid always on (#596). Footer dock (#527). One ticket mount (#178). Hidden ticket `inert`. |
| **T693-8 Docs** | This section + [`design-system.md`](./design-system.md) + [`AGENTS_FRONTEND_TRADE_TICKET_FLATTEN.md`](../skills/AGENTS_FRONTEND_TRADE_TICKET_FLATTEN.md). Verify: `make verify-issue-693`. Coolify leftover: [#702](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/702). |

**Code:** [`TradeOrderTicket.tsx`](../frontend-dapp/src/components/trade/TradeOrderTicket.tsx), [`TradeMarketOrderPanel.tsx`](../frontend-dapp/src/components/trade/TradeMarketOrderPanel.tsx), [`tokenHeadingWash.ts`](../frontend-dapp/src/utils/tokenHeadingWash.ts), [`index.css`](../frontend-dapp/src/index.css) (`.trade-order-text-tab`).

**Third-party / agent context:** [`skills/AGENTS_FRONTEND_TRADE_TICKET_FLATTEN.md`](../skills/AGENTS_FRONTEND_TRADE_TICKET_FLATTEN.md).
...
| Invariant | Meaning |
|-----------|---------|
| **T527-1 Dock, do not float** | At ticket scroll-top on Chromium, the money CTA **bottom** aligns with `trade-order-ticket-card` **bottom** (≤ 8px). It must not sit between Pay % chips and Expiry, and must not overlap **visible** Pay / Receive / Expiry / Advanced inside `trade-order-ticket-scroll`. After [#693](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/693), Limit expiry lives under Advanced; at ~1280×720 those controls may overflow the scrollport so their layout boxes overlap the sibling footer in document coordinates — clip intersection to the scrollport (leftover [#702](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/702)). P6 still requires expiry `scrollIntoView` to sit above the footer. |
...
| Invariant | Meaning |
|-----------|---------|
| **Control type** | Limit **side** is a WAI-ARIA **`radiogroup`** with two **`role="radio"`** `<button type="button">` controls (`side-control` + `side-buy-*` / `side-sell-*` semantic fills, [GitLab **#563**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/563)), not native `<input type="radio">`, so the active side updates in the same React commit as `onSideChange` without browser-native controlled-radio timing quirks ([GitLab **#153**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/153)). `/trade` Limit vs Market tabs are compact underlined text ([#693](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/693)), not **`tab-glass*`**. |
...
| **P547-4** | Column sort uses indexer keys (`symbol`, `volume_usd_24h`, `liquidity_usd`, `fee`, `created`) with **no** catalog overlay. Vol uses `formatPairListVolumeUsd` (`volume_usd_24h`) — compact `$` or **—**, never quote-token `48.2M` / `19.3`. Visible header stays **Vol**; `title` discloses priced-USD trailing 24h ([#576](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/576) **U6**, [#692](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/692) **PVol**). Catalog fetch still uses `sort=volume_24h` for the 500-row window (**P547-3**). Created cells show relative age from list JSON `created_at` ([#662](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/662) **P662-3**); `—` only when the field is missing or unparsable. Sort `created` orders by indexer first-seen `pairs.created_at` (default **desc**), not `created_at_block`. After `--fresh` / DB rebuild every pair looks recent — that caveat lives in this table, not a page lecture ([#489](#retail-copy-cognitive-load)). **v2 LP USD** is [#655](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/655) (**P655-1–P655-8**). |
...
**Invariants (dApp + indexer, PVol-1–PVol-8 — `/pool` 24h Vol USD, leftover [#544](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/544) / [#692](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/692)):**

| ID | Meaning |
|----|---------|
| **PVol-1** | `/pool` **Vol** cells are compact **USD** (`$` + `formatPairListVolumeUsd`) or **—**. Never raw 18-dec, never quote-token `48.2M` / `19.3` without `$`. |
...
| **PVol-5** | Priced pair list USD matches `GET /pairs/{addr}/stats` `volume_usd` within rollup lag (~5 min). Header `title` is trailing 24h USD, not midnight reset, not “quote-side.” |
...
**Code:** `frontend-dapp/src/pages/PoolPage.tsx`, `PoolPairsTable.tsx`, `PoolAdvancedManage.tsx`, `poolListQuery.ts`, `formatDate.ts`. Issues: [glab#692](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/692), [glab#655](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/655), [glab#547](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/547), [glab#662](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/662), [glab#112](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/112).

**Verify:** `make verify-issue-692` · `make verify-issue-655` · `make verify-issue-547` · `make verify-issue-662`. Coolify leftover: `make verify-issue-701`. Playbooks: [`AGENTS_FRONTEND_POOL_TABLE.md`](../skills/AGENTS_FRONTEND_POOL_TABLE.md), [`AGENTS_INDEXER_PAIR_VOLUME_USD.md`](../skills/AGENTS_INDEXER_PAIR_VOLUME_USD.md), [`AGENTS_INDEXER_PAIR_LIQUIDITY_USD.md`](../skills/AGENTS_INDEXER_PAIR_LIQUIDITY_USD.md), [`AGENTS_FRONTEND_POOL_CREATED.md`](../skills/AGENTS_FRONTEND_POOL_CREATED.md), [`AGENTS_POST_MERGE_OPS_701.md`](../skills/AGENTS_POST_MERGE_OPS_701.md).
...
**Route preview (GitLab [#158](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/158), Forgejo [#1218](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1218)):** The **Route** line lives in the same trade-summary card as **Price impact** and **Min received** (no separate “quote source” strip, no paired `Route (indexer)` / `Route` labels). The displayed token path follows the same precedence as submit: indexer-shaped `router_operations` when present (including wrap-enter after client `uluna`→cLUNC / `uusd`→cUSTC map), otherwise the client BFS route, native wrap path, or a direct `from → to`. Direct 1:1 wrap/unwrap never calls `/route/solve`. Code: [`swapRouteDisplay.ts`](../frontend-dapp/src/utils/swapRouteDisplay.ts), [`nativeWrapRouteSolve.ts`](../frontend-dapp/src/utils/nativeWrapRouteSolve.ts). Agent checklist: [`skills/AGENTS_FRONTEND_SWAP_ROUTE_DISPLAY.md`](../skills/AGENTS_FRONTEND_SWAP_ROUTE_DISPLAY.md), [`skills/AGENTS_FRONTEND_WRAP_ENTER_ROUTE_SOLVE.md`](../skills/AGENTS_FRONTEND_WRAP_ENTER_ROUTE_SOLVE.md).
...
| **S528-6 Chips stay in the ticket body** | Do not move presets into the [#527](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/527) money-CTA footer. On `/trade` Market they live under **Advanced** ([#693](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/693)); when the section is open, `elementFromPoint` on a chip must hit that chip. |
