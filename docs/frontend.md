...
| **Address row** | Connected menu uses [`AddressRow`](../frontend-dapp/src/components/ui/AddressRow.tsx) for full bech32 + copy + explorer ([#188](#addressrow-primitive)); chip trigger network label: [#186](#connected-wallet-chip-network-mobile). |
...
| **Out of scope (#140 B)** | Sibling issues: [#185](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/185) extra dropdown menu items. Remaining AddressRow surfaces: pair chips, `TxResultAlert` tx copy — [#188](#addressrow-primitive) (wallet dropdown done). Address explorer URLs: [#184](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/184) — see [Terra Classic block explorer URLs](#terra-classic-block-explorer-urls). Chip network label + mobile layout: [#186](#connected-wallet-chip-network-mobile) — done. |
...
| NFA footer on navigation | [`RouteContentReadyProvider`](../frontend-dapp/src/contexts/RouteContentReadyContext.tsx) exposes ready only when `readyForPath === pathname` (stale paths never satisfy a new route), then [`RouteContentReadyMarker`](../frontend-dapp/src/components/common/RouteContentReadyMarker.tsx) sets it via context (not `window` events — child `useEffect` runs before parent listeners and previously dropped the signal; [GitLab #138](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/138)). **Do not** use render-phase `setState` in this provider — it can break React Router tab clicks ([GitLab #182](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/182)). Regression: [`RouteContentReadyContext.test.tsx`](../frontend-dapp/src/contexts/__tests__/RouteContentReadyContext.test.tsx), E2E navigation NFA-after-route-change. |
...
| **Station extension + LocalTerra fee steps** | With **`VITE_NETWORK=local`**, [`wallet.ts`](../frontend-dapp/src/services/terraclassic/wallet.ts) sets **`station.keplr.defaultOptions.sign.preferNoSetFee`** and calls **`experimentalSuggestChain`** **before and after** `StationController.connect` (**`gasPriceStep` = 28.325**). Patched **`KeplrExtension`**: per-sign **`preferNoSetFee`**, post-sign fee guards, and **one amino re-sign retry** on LocalTerra when the first signed fee undershoots. **`StationController`**: LocalTerra (**case-insensitive `chainId`**) or Ledger → **amino**. Run **`npm ci`** ([`cosmesPatch127.test.ts`](../frontend-dapp/src/services/terraclassic/__tests__/cosmesPatch127.test.ts)). **QA:** Station + Keplr on LocalTerra. [GitLab #127](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/127). |
...
| Loading vs empty | **First** load (no candle payload yet): full-panel **Loading chart…**. **Interval switch** while refetching: keep `PriceChartLightweightCanvas` mounted (`placeholderData: keepPreviousData`), overlay **`data-testid="price-chart-interval-loading"`** — do **not** unmount the plot (async `createChart` races froze the selector after ~5 switches — [GitLab **#148**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/148)). Empty state applies only when the request **succeeded**, fetch settled, and there are zero valid points. |
