...
| **M626** Migrate Token | `/token/migrate` is free (no invoice). Source gate is `VITE_COMMUNITY_MIGRATE_CODE_IDS` (default 6036,10184,8266,8654) — not factory whitelist. **Do not append 3** ([#627](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/627)). Create Token links **Migrate here**. Retail lead is one sentence (address stays); no env-var / 50 UST1 / cw2 essay on the card. Launcher 11619 stays CMM-only. |
