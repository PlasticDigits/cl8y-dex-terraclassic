...
| Wrap + ≥2hop combo ([#587](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/587) / [#1264](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1264)) | Combined `wrap_deposit` + CW20 `send`→`execute_swap_operations` (N≥2) adds **`WRAP_ROUTER_COMBO_OVERHEAD_GAS` (400k)** in `totalGasLimitForExecuteMsgs`. Wrap+2hop = **2,710,000** (~76.76 LUNC at 28.325) for **LUNC→USTR and USTC→USTR**. Columbus-5 USTC→USTR `gas_used` is still unmeasured — do not raise the combo here. Wrap+1hop stays **1.8M** (#353). Malformed `send.msg` **throws** (`SendHookGasDecodeError`) instead of silent 600k. Native path stays **pool-only** (no hybrid / `book_input`). **G1264-4:** Swap Max / LUNC gas-gate treat only `uluna` as fee-paying native — `uusd` wraps but the Network-fee reserve is bank LUNC (`nativeWrapSwapHints.ts`). Fixture `wrap_plus_send_2hop_ustc`. Verify: `make verify-issue-587` · `make verify-issue-1264`. Optional LCD: `VERIFY1264_COLUMBUS_TX`. |
