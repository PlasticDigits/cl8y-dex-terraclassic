...
**Calculating… / slow multihop ([#484](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/484)):** Periodic sim refresh uses [`simQuoteRefetchInterval`](../frontend-dapp/src/utils/quoteDebounce.ts) so React Query does **not** cancel/restart an in-flight quote every 10s (default `cancelRefetch`). Swap **You Receive** shows Calculating only via [`shouldShowSimReceiveCalculating`](../frontend-dapp/src/utils/quoteDebounce.ts) (`isFetching && !simData`) — background refetch keeps the prior amount. Indexer `GET`/`POST` `/route/solve` use a **45s** client timeout (`INDEXER_ROUTE_SOLVE_TIMEOUT_MS`) and accept an AbortSignal from the sim `queryFn`. Agent checklist: [`skills/AGENTS_FRONTEND_SWAP_QUOTE_REFETCH.md`](../skills/AGENTS_FRONTEND_SWAP_QUOTE_REFETCH.md).
