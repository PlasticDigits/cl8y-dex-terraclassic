...
| I14 | **CreatePair inherits factory registry ([#536](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/536)):** Factory `Config.discount_registry` (serde default `None`) is copied into pair instantiate. `SetDiscountRegistryAll` / `SetDiscountRegistryBatch` write or clear that pointer; single-pair `set_discount_registry` does **not**. `UpdateConfig { discount_registry }` sets the pointer without touching indexed pairs. Pair query `GetDiscountRegistry` returns stored `Option<Addr>`. Existing listings are **not** auto-wired (that is [#535](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/535)). Fail-closed registry errors stay **I10**. Factory **1.8.0** + pair **1.14.0**. Verify: `make verify-issue-536`. Playbook: [`skills/AGENTS_FACTORY_DISCOUNT_REGISTRY.md`](../../skills/AGENTS_FACTORY_DISCOUNT_REGISTRY.md). |
