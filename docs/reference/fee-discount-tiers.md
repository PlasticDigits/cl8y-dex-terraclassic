...
| I14 | **CreatePair inherits factory registry ([#536](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/536)); dApp chrome gated on pair registry ([#537](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/537)):** Factory `Config.discount_registry` (serde default `None`) is copied into pair instantiate. `SetDiscountRegistryAll` / `SetDiscountRegistryBatch` write or clear that pointer; single-pair `set_discount_registry` does **not**. `UpdateConfig { discount_registry }` sets the pointer without touching indexed pairs. Pair query `GetDiscountRegistry` returns stored `Option<Addr>`. Existing listings are **not** auto-wired (that is [#535](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/535)). Fail-closed registry errors stay **I10**. Factory **1.8.0** + pair **1.14.0**. Advertise a CL8Y tier discount (strikethrough `fee_bps`, “your discount”, maker place bps) only when that pair’s `DISCOUNT_REGISTRY` is set **and** equals `VITE_FEE_DISCOUNT_ADDRESS`. Unwired pairs (`None`) charge full `fee_bps` / `maker_fee_bps(fee_bps)` on-chain — do not invent a client-side discount. Probe: LCD raw key `discount_registry` (`ZGlzY291bnRfcmVnaXN0cnk=`) until pair `GetDiscountRegistry` ships; probe failure is fail-closed (full fee in chrome). Hybrid `trader` quotes stay the execute-aligned source of truth. Frontend: [`pairDiscountRegistry.ts`](../../frontend-dapp/src/utils/pairDiscountRegistry.ts). Verify: `make verify-issue-536` / `make verify-issue-537`. Playbook: [`skills/AGENTS_FACTORY_DISCOUNT_REGISTRY.md`](../../skills/AGENTS_FACTORY_DISCOUNT_REGISTRY.md). |
