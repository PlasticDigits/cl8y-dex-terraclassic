...
| I14 | **Pair registry gate for dApp fee-tier chrome ([#537](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/537)):** Advertise a CL8Y tier discount (strikethrough `fee_bps`, “your discount”, maker place bps) only when that pair’s `DISCOUNT_REGISTRY` is set **and** equals `VITE_FEE_DISCOUNT_ADDRESS`. Unwired pairs (`None`) charge full `fee_bps` / `maker_fee_bps(fee_bps)` on-chain — do not invent a client-side discount. Probe: LCD raw key `discount_registry` (`ZGlzY291bnRfcmVnaXN0cnk=`) until pair `GetDiscountRegistry` ships; probe failure is fail-closed (full fee in chrome). Hybrid `trader` quotes stay the execute-aligned source of truth. Frontend: [`pairDiscountRegistry.ts`](../../frontend-dapp/src/utils/pairDiscountRegistry.ts). On-chain wiring of economic pairs is [#535](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/535) (out of scope here). Verify: `make verify-issue-537`. |
