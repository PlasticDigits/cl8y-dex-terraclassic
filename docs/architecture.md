...
## Anonymous UTC-day activity digest {#indexer-evidence-digest}

`GET /api/v1/evidence/digest?day=YYYY-MM-DD` is one public JSON cover sheet for a single UTC calendar day: protocol **counts** (swap, LP add/remove, limit place/cancel/fill, wrap/unwrap fee events, UST1 window fee events) plus treasury **fee-path mix** (`FeeSource::ALL` order, capped `by_token`). No wallets, no `tx_hash`, no `actor_hash`, no unique-trader census. `counts.wrap` repeats wrap/unwrap **fee-event** counts; it is not wrap principal and must not be added to swap count. `counts.window` keeps `ust1_mint` / `ust1_redeem` off the wrap surface. Gems count here; DeFiLlama daily still gem-excludes and can **404**. Trailing `/protocol/fees` stays a different clock and a different USD rule (that rollup can omit unconfigured families).

`complete` means the UTC date is before today, computed when the response is built, not that ingest has reached chain tip. Today is **200** with `complete: false`. The only query key is `day` (`deny_unknown_fields`). The route sits on the global governor. Counts and fees cache for 60s; `complete` does not. It is not on the LCD-heavy router. Decision, SQL bounds, BRIN migration, tests, rollout, and rollback: [ADR 0010](./adr/0010-anonymous-utc-day-activity-digest.md) ([#1206](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1206)). Invariants: [`indexer-invariants.md`](./indexer-invariants.md) **Anonymous UTC-day digest (#1206)**. Unredacted incident SQL stays in [`suspicious-activity-queries.md`](./runbooks/suspicious-activity-queries.md).
