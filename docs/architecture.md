...
Sister **[#1300](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1300) / Q21** (unpublished ADR **0008** on `origin/cac-design-issue-1300`): attest sqlx **`20260921120000` then `20260921120001`**; HTTP **`protocol-top-pairs` / `Top pairs (30d)`**. Implement **inserts** that paragraph when #1300 lands — do **not** whole-file checkout that tip (would drop Q23 below). Hub wrap Coolify visual stays **#1305 leftover-1**.

After PRs **1302–1304**, **Q23 / [#1305](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1305)** is ordinary leftover: attest sqlx **`20260921130000_traders_lifetime_heal_from_swaps` only** (Observability **SELECT** on Coolify DB / indexer `DATABASE_URL`, not `postgres-psql.sh`). Heal is **startup-before-D5** (once in `poller.rs`, then D5 `refresh_all_volume_windows`; not in `run_volume_refresh_loop`): indexer-log needle `traders_healed lifetime totals from swap_events (GitLab #1277)`, not `DATABASE_URL`. While the [#1276](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1276) checkbox is off, leftover 2 is a **manual** Coolify indexer deploy of `729b097f+`. Healthy `GET /health` is **not** `_sqlx_migrations` evidence ([ADR 0006](./adr/0006-indexer-health-git-sha.md)). Rebuild frontend `729b097f+`. **Leftover-1 is operator glance** hub **`cUSTC / USD`** + **`cLUNC / USD`** vs CEX **`USTC`** / **`LUNC`** / **`vFDUSD`** / click **vFDUSD** selected not **`VFDUSD`** (idle `/protocol` is **USTC**). Hashed-chunk HTTP pins unquoted **`cLUNC wrap`**, **`cLUNC`**, **`cUSTC`**, **`vFDUSD`** (any quote style; never require source `'`; **all four supporting rebuild-presence — none prove leftover 1**; live HTTP must not treat **`cLUNC`** as leftover-1 complete) — do **not** grep concatenated **`cLUNC / USD`** in hashed chunks (assembled at render) and do **not** grep **`protocol-top-pairs`** (that is [#1300](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1300)). Keep the heal gate trades+raw only. Dismiss CODEOWNERS self-request before `fj pr merge` (no `force_merge`). Child Playwright / #703 `YY-MM` ticks are not leftover-complete. `make verify-issue-1305` **(implement)** is children + optional HTTP, not leftover-complete. Leftover-complete close comment (ADR 0010 / playbook): sqlx SELECT + heal needle + leftover-1 glance including click **vFDUSD**. Forbidden in that comment: `GET /health`, `protocol-top-pairs`, hashed-chunk `cLUNC` PASS, green make. If **#1305** is still closed on the 06:46 comment, reopen it as a leftover precondition (not leftover-complete). That is **not** the indexer auto-deploy checkbox ([ADR 0006](./adr/0006-indexer-health-git-sha.md) / [agent-control #297](https://git.cl8y.com/PlasticDigits/cl8y-agent-control/issues/297)).
