...
After PRs **1302–1304**, **Q23 / [#1305](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1305)** is ordinary leftover: attest sqlx **`20260921130000_traders_lifetime_heal_from_swaps` only** (operator Coolify DB / indexer `DATABASE_URL`, not `postgres-psql.sh`; D5 heal no-op or one log), rebuild frontend `729b097f+` and glance hub **`cUSTC / USD`** + **`cLUNC / USD`** vs CEX tabs **`USTC` / `LUNC` / `vFDUSD`** (hashed-chunk HTTP marker **`cLUNC / USD`**; do **not** grep **`protocol-top-pairs`** — that is [#1300](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1300)), keep the heal gate trades+raw only (USD-only skew must not re-trip), and dismiss CODEOWNERS self-request before `fj pr merge` (no `force_merge`). That is **not** the indexer auto-deploy checkbox ([ADR 0006](./adr/0006-indexer-health-git-sha.md) / [agent-control #297](https://git.cl8y.com/PlasticDigits/cl8y-agent-control/issues/297)).
