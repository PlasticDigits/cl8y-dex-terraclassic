...
Factory candles today require a catalog USD for `asset_1`. When that leg is ALPHA or CL8Y, ingest writes nothing and `/charts` shows the empty price pane. [#1315](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1315) keeps both-catalog candles as USD of `asset_0`. A single catalog leg prices **1 human unit of the other leg** (USD of ALPHA, either slot) and stores that leg as `usd_leg` at write. Neither leg catalog (CL8Y/ALPHA) stores human quote-per-base only; the chart heading is **Price ({quote} per {base})**. A missing catalog print skips the whole candle. Gap fill runs only for pairs with `se.price > 0` and zero candle rows. Trade `inverted` still drives the ticket; an `asset_1` series is plotted as stored OHLC or `stored × H`, not through `invertUsd`. 24h OHLC, TWAP, and tape Price stay on swap `price_usd`. No ALPHA/CL8Y peg and no new `quote_usd_kind` arm. Tape `price_usd`, volume, and idle marks stay on the existing catalog rules. GET `/candles` omits `open/high/low/close` when `usd_leg=asset_1` and sends `subject_*`, so a cached dApp drops the bar until the new client is running. On `/charts` that series defaults both the price pane and the pill to `asset_1`. A failed gap fill does not set the done marker.
