...
The custodian is the existing `CMM_GOVERNANCE_ADDR` pin. Provide credits wasm `receiver` by `share` (`sender` only when `receiver` is absent). Withdraw debits `sender` by `withdrawn_share`. LP-token `transfer` / `transfer_from` / `send` write `transfer_out` / `transfer_in` from `from`, `to`, and `amount`, idempotent on `(tx_hash, pair_id, kind, holder, event_index)`. The first LCD balance is a fixed anchor (`observed_at` is that block time and stays put). Later custodian reads, at most once per 24 hours, store `lcd_balance − (anchor.shares + deltas after the anchor)` as one reconciliation delta. They do not replace the 30-day integral with today’s balance: 100 shares for 20 days then 500 for 10 days stays `100×20/30 + 500×10/30` after a day-29 refresh. The denominator is those share-seconds marked at current `pair_liquidity_usd / total_supply`. The fee numerator is `swap_amm` + `book_take` + `limit_place` only. GET does not scan events or the chain. Revert keeps the lowest `book_take` / `limit_place` id per `(tx_hash, source, ordinal)` before clearing `pair_id`. Decision: [ADR 0012](./adr/0012-cmm-held-v2-lp-fee-bps.md). Playbook: [`AGENTS_INDEXER_CMM_LP_CENSUS.md`](../skills/AGENTS_INDEXER_CMM_LP_CENSUS.md).
