...
Sibling leftover scripts (`VERIFY*_IID`) fail-close on **unreachable**; #1276 **adds** `EXPECT_SHA` under IID/`LEFTOVER_COMPLETE` (bare IID without `EXPECT_SHA` is an intentional FAIL before curl, not a #701 bug). Probe: `VERIFY1276_REQUIRE_LIVE=1` without IID and without `EXPECT_SHA` may PASS bake presence. Leave watch paths off until leftover is closed if glance uses repo `HEAD`. Boot still runs `sqlx::migrate!()` (no production `set_ignore_missing`). Unique `YYYYMMDDHHMMSS` prefixes required; colliding same-timestamp files keep the **first-applied** checksum (`20260916120000_usdt_quote_usd_null_backfill.sql`) and bump the later sibling. Same rule for [#1277](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1277) `20260921120000_traders_rolling_volume_numeric_38_0` then [#1263](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1263) `20260921120001_pair_volume_30d` ([ADR 0008](./adr/0008-post-merge-leftover-1287-1298.md) / **Q21**). Coolify-era indexer rollback is **three-way** (**H1276-7**): **2(a)** unchanged vs prior Coolify Deploys SHA (`git ls-tree --name-only <sha> indexer/migrations/`) → restore; **2(b)** keep schema + hotfix that still ships N **even if** a paired `down.sql` exists — **no Stop** only for clean ahead; when `idx_reclass` is **2(b)**, **start only the hotfix image**; dirty → **2(a)**: start **only** the restored prior image (already stopped); dirty → **2(c)**: stay stopped through downs / version `DELETE`, then start **only** the prior image. **2(c)** **iff** restoring the prior image is required **and** `indexer/migrations/revert/<version>_*.down.sql` exists for **every** successful `_sqlx_migrations.version` newer than that baseline (historical `revert/` files do **not** select 2(c); if **any** ahead version has no pair → 2(b); **Partial suffix revert** is still 2(b)) — [`rollback-decision.md`](./runbooks/rollback-decision.md) § Auto-deploy era. Dirty is a **sequential gate** (any `success=false`?: auto-deploy **off** → Coolify **Stop** / scale-to-zero → snapshot → DELETE every success=false → idx_reclass; auto-deploy off is **not** a process stop; never `UPDATE success`). Mixed dirty+ahead uses the same every-ahead-version **2(c)** gate after reclassify. **Stop** is a hard pre-step for dirty `DELETE` and **2(c)** `down.sql` + ledger `DELETE`; do not `DELETE` `_sqlx_migrations` while the N-shipping image can still boot. Inspect prod schema via Coolify DB / indexer `DATABASE_URL`, not `postgres-psql.sh`. Historical **#573** coupled leftover is unchanged. Decision, parser, bake ARG, slices, leftover glance, and close-comment template: [ADR 0006](./adr/0006-indexer-health-git-sha.md). Invariants: [`indexer-invariants.md`](./indexer-invariants.md) **Health git SHA (#1276)**. Playbook: [`AGENTS_INDEXER_HEALTH_GIT_SHA.md`](../skills/AGENTS_INDEXER_HEALTH_GIT_SHA.md).

## Post-merge leftover ops {#post-merge-leftover-ops}

Stacked leftover live (Coolify migrate/rebuild, columbus-5 wasm attest, LocalTerra/manual, ops-bot) is the **Q6–Q21** registry in [`qa-invariants.md`](./qa-invariants.md), not a second architecture. After PRs 1287–1298, **Q21 / [#1300](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1300)** is ordinary leftover: leftover-complete is **operator + leftover-*ops*** (Coolify migrate/rebuild **and** LCD cw2 **1.17.0** probe **and** four walks); tracker-close comments on #1300 do **not** waive the walks; leftover-*ops* **comments the closed tracker** with the four walk notes (reopen is optional noise vs sister leftover playbooks); Forgejo closed ≠ leftover-complete; green `make verify-issue-1300` never means close. **Attest `20260921120000` then `20260921120001` only** (operator Coolify DB / indexer `DATABASE_URL`, not `postgres-psql.sh`; leftover `make verify-issue-1300` live is HTTP only — **`GET /api/v1/protocol/top-pairs`** plus frontend **`protocol-top-pairs` / `Top pairs (30d)`** in #673/#686 `chunk("ProtocolPage")`, not #701 `PoolPage|chartsOverviewStats|pairCatalog`, not #1276 `EXPECT_SHA`, not hub `cLUNC / USD`), rebuild frontend `6d34da13+`, pair leftover-complete is **probe-attest** LCD cw2 **1.17.0 / 11672** via `UPGRADE582_PROBE_ONLY=1` (store/2-of-3 operator-only if probe is still 1.16.0; leftover agents never `terrad tx`; do not reopen #1300 for wasm). `VERIFY1300_REQUIRE_HAS_LOCALTERRA=1` is fail-closed `make has-localterra` only (no Playwright; default leftover verify probes `has-localterra` and does not execute the four walks). Use **`VERIFY1300_SKIP_HAS_LOCALTERRA` only** (short `SKIP_HAS_LOCALTERRA` never skips). Defining `VERIFY1300_REQUIRE_CHAIN` as Playwright / `run_leftover_e2e` is a leftover-implement FAIL. Close leftover only after leftover-*ops* walk evidence (the four how-tos on #1300; leftover-*ops* comments the closed tracker with the four walk notes (reopen is optional noise vs sister leftover playbooks); Forgejo closed ≠ leftover-complete; slice 7; `#1255` is LocalTerra-only: Create Pair **Custom contract** paste (**C542**) then Swap pick (**Do not paste on Swap**); leftover pass is Max of minted raw `10^18` → human `1`; unlimited mint cap is valid). `has-localterra` down / chain-probe SKIP is **not** leftover-complete for issue item 5. Slice 2 leftover implement does **not** provision LocalTerra or record walks. Optional: Cloud Agent leftover-*ops* may `make setup-cloud-localterra` when item 5 still lacks walk notes; that is **not** implement-slice close. Do **not** reopen [#1264](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1264) (AC1 measured: `53B06B65…BAC36`, `gas_used=2630228` under 2,710,000, class not-A/not-B; envelope stays). Leave [#1279](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1279) device QA to ops-bot. Same indexer boot may also apply `20260921130000`; that row is sister [#1305](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1305), not a #1300 FAIL. PR **#1302** (`92c84406`) landed on **#1305** — do not revert it; hub wrap Coolify visual is **#1305 leftover-complete**. Keep leftover ADR as [`0008-post-merge-leftover-1287-1298.md`](./adr/0008-post-merge-leftover-1287-1298.md); do not merge `origin/cac-design-issue-1302` as-is (overlap on this file / `qa-invariants.md` / README). That branch already publishes `docs/adr/0009-verify-issue-1290-hub-wrap-labels.md` / **Q22**. A later **0009** follow-up MR that inserts without taking 0008/Q21 is allowed. That is **not** the indexer auto-deploy checkbox ([ADR 0006](./adr/0006-indexer-health-git-sha.md) / [agent-control #297](https://git.cl8y.com/PlasticDigits/cl8y-agent-control/issues/297)). Do not treat `cac-design-issue-1276` / `1277` as this stack. Decision, slices, rollback: [ADR 0008](./adr/0008-post-merge-leftover-1287-1298.md). Playbook: [`AGENTS_POST_MERGE_OPS_1300.md`](../skills/AGENTS_POST_MERGE_OPS_1300.md).
