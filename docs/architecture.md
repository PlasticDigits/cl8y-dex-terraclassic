...
## DEX hub wrap vs CEX labels {#dex-hub-wrap-labels}

`/protocol` keeps wrap identity and CEX identity on **separate** cards. DEX hub `<dt>` lines are **cUSTC / USD** and **cLUNC / USD** (API ticker for the LUNC wrap column stays `lunc`; no `clunc` path). CEX oracle tabs stay **USTC**, **LUNC**, **vFDUSD**. Product rules **P1240-1–P1240-8** / **H11–H16** live in [`frontend.md`](./frontend.md) and the Protocol skills — do not retitle CEX as wrap or CSS-uppercase ticker-bearing labels.

Labels shipped in [!1290](https://git.cl8y.com/code/cl8y-dex-terraclassic/pulls/1290) ([#1240](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1240)). Closed !1302 merged a weaker `verify-issue-1290` sketch (`f237a6e6`). Leftover issue is **[#1306](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1306)** (not product #1290). The #1306 MR on current `main` **inserts** this section plus ADR **0009** / **Q22** / README index, aliases `verify-issue-1302` and `verify-issue-1306`, Cloud Agent + Frontend help for **1290 / 1302 / 1306**, `free_tcp_port` 30129, and full B1290-4 greps — do not retarget closed !1302, do not merge open !1306 as-is, and do not whole-file checkout this file from the design tip (overlap with unpublished [#1300](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1300) ADR **0008**; live **Q21** on `main` is Lunc Dash payload [#1308](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1308) / !1310). Leftover-complete is that wiring MR, not green skip-E2E. Production `/protocol` glance is [#1305](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1305) leftover 1. Bundle: `make verify-issue-1290` (aliases `verify-issue-1302` / `verify-issue-1306`) → child `verify-issue-1240`, `hubPriceTicker` Vitest, optional Playwright `protocol-page` at 5 workers. Decision: [ADR 0009](./adr/0009-verify-issue-1290-hub-wrap-labels.md). QA row: [`qa-invariants.md`](./qa-invariants.md) **Q22**. No indexer JSON change.

## Post-merge leftover after PRs 1306/1309/1310 {#post-merge-leftover-ops-1311}

After PRs **1306 / 1309 / 1310** (`0d6eaeee`), leftover **[#1311](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1311) / Q24** is ordinary ops: Coolify **frontend** cut so `dex.cl8y.com` serves the !1310 `payload` query helper (`ead2ffe4+`), then device **AC2 / AC3 / AC7** (closed [#1308](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1308)) via cl8y-pm inbox **1.5.1 / 1.5.1a / 1.5.1b / 1.5.1c** + **1.5.6 / 1.5.7** (**AC3** = **1.5.1c**). `make verify-issue-1308` is pre-check; `VERIFY1308_DEVICE_COMPLETE=1` / `VERIFY1311_DEVICE_COMPLETE=1` / `VERIFY1311_IID=1311` must FAIL. Hashed-chunk unquoted **`wallet_connect?payload=`** is supporting rebuild-presence, not leftover-complete. Do **not** reopen [#1279](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1279). Production `/protocol` glance and indexer migrate stay [#1305](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1305). Do **not** use architecture `{#post-merge-leftover-ops}` here — that anchor / ADR **0010** / **Q23** stay reserved on unpublished `origin/cac-design-issue-1305`. Do **not** merge `cac-design-issue-1300` / `1302` / `1305` / `1306` as-is. Decision: [ADR 0011](./adr/0011-post-merge-leftover-1306-1309-1310.md). Playbook: [`AGENTS_POST_MERGE_OPS_1311.md`](../skills/AGENTS_POST_MERGE_OPS_1311.md). Not [agent-control #297](https://git.cl8y.com/PlasticDigits/cl8y-agent-control/issues/297) checkbox flip.
