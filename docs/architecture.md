...
Labels shipped in [!1290](https://git.cl8y.com/code/cl8y-dex-terraclassic/pulls/1290) ([#1240](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1240)). Leftover issue is **#1302** (not product #1290). Closed !1302 merged a weaker sketch (`f237a6e6`); the follow-up on current `main` **inserts** this section plus ADR **0009** / **Q22** / README index, alias `verify-issue-1302`, Cloud Agent + Frontend help for **both** `verify-issue-1290` and `verify-issue-1302`, `free_tcp_port` 30129, and full B1290-4 greps — do not retarget that closed PR, and do not whole-file checkout this file from the design tip (overlap with [#1300](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1300) ADR **0008** / **Q21**). Leftover-complete is that wiring MR, not green skip-E2E. Production `/protocol` glance is [#1305](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1305) leftover 1. Bundle: `make verify-issue-1290` (alias `verify-issue-1302`) → child `verify-issue-1240`, `hubPriceTicker` Vitest, optional Playwright `protocol-page` at 5 workers. Decision: [ADR 0009](./adr/0009-verify-issue-1290-hub-wrap-labels.md). QA row: [`qa-invariants.md`](./qa-invariants.md) **Q22**. No indexer JSON change.
