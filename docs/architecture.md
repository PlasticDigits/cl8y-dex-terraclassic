...
Two Coolify apps share this Forgejo repo: **frontend** (`dex.cl8y.com`, auto-deploy on) and **indexer** (`indexer.dex.cl8y.com`, Dockerfile [`docker/indexer/Dockerfile`](../docker/indexer/Dockerfile)). CAC grouped drain maps **one** UUID per `owner/repo` and is not the indexer redeploy path. Product attest is indexer `GET /health`: always `{"status":"ok"}`, plus optional `"git_sha"` when baked `GIT_SHA` (only if **non-empty after trim**) or else `SOURCE_COMMIT` parses as lowercase hex 7–40. Empty `GIT_SHA=""` (Dockerfile ARG default) **falls through** to `SOURCE_COMMIT`; a non-empty reject (`HEAD`, secrets) does not. ARG/ENV are re-declared on the **runtime** stage, not the builder. CAC `/health` stays SHA-free. Fee-discount health stays LCD-only.

Auto-deploy on the indexer app is an operator Coolify protected-branch flag ([#1276](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1276) leftover, [agent-control #297](https://git.cl8y.com/PlasticDigits/cl8y-agent-control/issues/297)). Boot still runs `sqlx::migrate!()`; schema-ahead rollback is forward-fix. Vite vs Rust rebuild skew is expected. Historical **#573** coupled leftover is unchanged. Decision, parser, bake ARG, slices, and leftover glance: [ADR 0006](./adr/0006-indexer-health-git-sha.md). Invariants: [`indexer-invariants.md`](./indexer-invariants.md) **Health git SHA (#1276)**. Playbook: [`AGENTS_INDEXER_HEALTH_GIT_SHA.md`](../skills/AGENTS_INDEXER_HEALTH_GIT_SHA.md).
