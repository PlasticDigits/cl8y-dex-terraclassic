...
## Scroll layout invariants {#scroll-layout-invariants}

Four frontend rules must stay true while the user scrolls: a portaled picker tracks its trigger, `.app-top-sticky` stays opaque, the trade-ticket money CTA stays a flex sibling of `trade-order-ticket-scroll`, and a zoomed price chart does not `fitContent()` on a same-interval candle refresh. Mechanisms already live in [`frontend.md`](./frontend.md). [#1316](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1316) adds one regression bundle and fixes only the rule that fails. No swap, route, wallet, contract, or indexer change. Decision: [ADR 0012](./adr/0012-scroll-layout-invariants.md). QA row: [`qa-invariants.md`](./qa-invariants.md) **Q25** (**S1316-1–S1316-8**). Gate: `make verify-issue-1316` (Playwright at 5 workers, `PLAYWRIGHT_SKIP_CHAIN=1`). ADR **0010** / **Q23** and ADR **0011** / **Q24** stay with unpublished [#1305](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1305) and [#1311](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1311).
