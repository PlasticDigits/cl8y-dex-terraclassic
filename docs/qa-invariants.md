...
| **B1290-3** | Playwright default-on, **5 workers**, port **30129**, `PLAYWRIGHT_SKIP_CHAIN=1`. Copy `free_tcp_port` from `verify-issue-703.sh` (function only — not 703’s `ln -sfn` bootstrap); free 30129 first; port + `PLAYWRIGHT_BASE_URL` only in that step’s subshell. Skip only `VERIFY_ISSUE_1290_SKIP_E2E=1` or missing Playwright package. Installed package + crash (Chromium / NSS) = FAIL. Missing nvm / `with-node.sh` bootstrap = FAIL (not skip). |
| **B1290-4** | Crosslinks: testing, AGENTS, PROTOCOL_STATS, PROTOCOL_HUB, frontend.md, ADR 0009, architecture `#dex-hub-wrap-labels`, this Q22, **and Makefile `verify-issue-1302` / `verify-issue-1306`**. #1306 **inserts** those slice-0 sections (do not whole-file checkout overlap files from the design tip); do not shrink greps to the closed-!1302 sketch or to eight docs without Makefile. |
