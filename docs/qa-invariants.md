...
| Invariant | Check | On failure |
| --------- | ----- | ---------- |
| **Q21** Post-merge PRs 1287–1298 leftover live is checked (Coolify `20260921120000` then `20260921120001` only — `20260921130000` is #1305; columbus-5 pair LCD cw2 1.17.0 / 11672 **probe** required to close — store only if still 1.16.0; LocalTerra/manual four walks as leftover-*ops* evidence — tracker-close comments do not waive them; `has-localterra` down is not leftover-complete, keep #1300 open; leftover-complete is **operator + leftover-*ops***, green `make verify-issue-1300` never means close; `VERIFY1300_REQUIRE_HAS_LOCALTERRA=1` is fail-closed `has-localterra` only; **`VERIFY1300_SKIP_HAS_LOCALTERRA` only** (not `SKIP_HAS_LOCALTERRA`); defining `VERIFY1300_REQUIRE_CHAIN` as Playwright/`run_leftover_e2e` is leftover-implement FAIL; frontend `protocol-top-pairs` / `Top pairs (30d)` in #673/#686 `chunk("ProtocolPage")` — not #701 PoolPage regex; #1264 closed (AC1 measured; do not reopen; envelope 2.71M); #1279 ops-bot; #1302 landed on #1305 — do not revert; do not merge `cac-design-issue-1302` as-is; later 0009 without taking 0008/Q21 is allowed) | **`make verify-issue-1300`** → children **1287, 1234, 1265, 1240, 1279, 1277, 1264, 1285, 1255, 1219, 1218, 1263** plus leftover probes (**M1300-1–M1300-8**) | Non-zero exit; fix the failing child or Coolify/columbus-5 leftover; do not treat green verify or a tracker-close comment as leftover-complete; do not reopen #1264; do not treat green #1279 make as leftover-complete; do not treat chain-probe SKIP as item-5 leftover-complete; do not revert #1302; do not FAIL on `20260921130000` |
...
| **M1300-3** | Coolify frontend rebuild from `6d34da13+`. HTTP marker: **`protocol-top-pairs` / `Top pairs (30d)`** in hashed Vite **`chunk("ProtocolPage")`** (#673/#686; `App.tsx` lazy `ProtocolPage`). Do **not** copy #701 `PoolPage\|chartsOverviewStats\|pairCatalog`. Do not grep hub `cLUNC / USD` (#1305). Keep issue item 5 as leftover-complete leftover-*ops* evidence: close #1300 only after the four walks (tracker-close comments do **not** waive them). **#1218** Pay LUNC / Receive USTR **or** JADE/RUBY E7 stand-in, wrap prefix + solver hops — do not require Route `cUSTC` or `USTR`. **#1255 LocalTerra only** — `mint_control` is a 50 UST1 SKU; a `dex.cl8y.com` Create Token is spend + live CW20; set **Decimals** to **18**, enable **Minting** at create; mint cap empty (unlimited — **valid**) or human `≥1`; type Swap `1`; Manage Token Mint raw **`1000000000000000000`** not `1`; do not mix mint cap with the raw Mint field. **#1219** LocalTerra in-tree 1.17.0 dust ladder copy / Place disabled, no popup — **not columbus-5 wasm leftover** (issue leftover 3 is `#1264`). **#1263** LocalTerra `/protocol` **Top pairs (30d)** renders with indexer-from-this-tip-up, ≤5 rows — Coolify-only `?limit=6` → 400. `has-localterra` down is **not** leftover-complete — keep #1300 open. Cloud Agent leftover-*ops* may `make setup-cloud-localterra` when item 5 is still open; that is not implement-slice close. Chain-probe SKIP/PASS is not item-5 evidence. Child Vitest is not the walk. Green `make verify-issue-1300` never means close. `VERIFY1300_REQUIRE_HAS_LOCALTERRA=1` is fail-closed `has-localterra` only. **`VERIFY1300_SKIP_HAS_LOCALTERRA` only.** |
| **M1300-4** | columbus-5 pair leftover-complete is LCD cw2 **1.17.0** (`UPGRADE582_PROBE_ONLY=1`). Leftover-ops recorded **1.17.0 / 11672**. A second store is a leftover defect. Store/2-of-3 / `UpdateConfig { pair_code_id }` only if probe is still **1.16.0** (`UPGRADE582_SKIP_FACTORY_MIGRATE=1`; factory already **1.10.0 / 11629**). Versions: [`AGENTS_CW20_CODE_ID_PIN.md`](../skills/AGENTS_CW20_CODE_ID_PIN.md). Do not follow `cw20-code-id-ops.md` 1.15.0 RAN. Leftover agents never `terrad tx`. `verify-issue-1234` must not claim `verify-issue-582` unless it runs it. |
