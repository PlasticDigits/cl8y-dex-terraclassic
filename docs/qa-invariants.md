...
| Invariant | Check | On failure |
| --------- | ----- | ---------- |
| **Q21** Post-merge PRs 1287–1298 leftover live is checked (Coolify `20260921120000` then `20260921120001` only — `20260921130000` is #1305; columbus-5 pair 1.17.0 **required to close**; LocalTerra/manual four walks as leftover-*ops* evidence — `has-localterra` down is not leftover-complete, keep #1300 open; leftover-complete is **operator + leftover-*ops***, green `make verify-issue-1300` never means close; `VERIFY1300_REQUIRE_HAS_LOCALTERRA=1` is fail-closed `has-localterra` only; defining `VERIFY1300_REQUIRE_CHAIN` as Playwright/`run_leftover_e2e` is leftover-implement FAIL; frontend `protocol-top-pairs` / `Top pairs (30d)`; #1264 stay-open; #1279 ops-bot; #1302 landed on #1305 — do not revert; do not merge `cac-design-issue-1302` as-is; later 0009 without taking 0008/Q21 is allowed) | **`make verify-issue-1300`** → children **1287, 1234, 1265, 1240, 1279, 1277, 1264, 1285, 1255, 1219, 1218, 1263** plus leftover probes (**M1300-1–M1300-8**) | Non-zero exit; fix the failing child or Coolify/columbus-5 leftover; do not treat green verify as leftover-complete; do not close #1264; do not treat green #1279 make as leftover-complete; do not treat chain-probe SKIP as item-5 leftover-complete; do not revert #1302; do not FAIL on `20260921130000` |
...
| **M1300-3** | Coolify frontend rebuild from `6d34da13+`. HTTP marker: **`protocol-top-pairs` / `Top pairs (30d)`** — do not grep hub `cLUNC / USD` (#1305). Keep issue item 5 as leftover-complete leftover-*ops* evidence: close #1300 only after the four walks (#1218 Pay LUNC / Receive USTR **or** JADE/RUBY E7 stand-in, wrap prefix + solver hops — do not require Route `cUSTC` or `USTR`; #1255 Create Token 18-dec unlisted — set **Decimals** to **18** (`create-token-decimals`; default `'6'`), enable **Minting** at create (`mint_control` create-only; 50 UST1 SKU), type Swap `1`, Manage Token Mint raw **`1000000000000000000`** not `1`; #1219 LocalTerra in-tree 1.17.0 dust ladder copy / Place disabled, no popup — not `dex.cl8y.com` while fleet is 1.16.0; #1263 LocalTerra `/protocol` **Top pairs (30d)** renders with indexer-from-this-tip-up, ≤5 rows — Coolify-only `?limit=6` → 400). `has-localterra` down is **not** leftover-complete — keep #1300 open. Cloud Agent leftover-*ops* may `make setup-cloud-localterra` when item 5 is still open; that is not implement-slice close. Chain-probe SKIP/PASS is not item-5 evidence. Child Vitest is not the walk. Green `make verify-issue-1300` never means close. `VERIFY1300_REQUIRE_HAS_LOCALTERRA=1` is fail-closed `has-localterra` only. |
| **M1300-4** | columbus-5 pair via `scripts/upgrade-582-code-id-pin.sh` (`UPGRADE582_PAIR_VERSION` **1.17.0**; factory already **1.10.0 / 11629** → `UPGRADE582_SKIP_FACTORY_MIGRATE=1`; `UpdateConfig { pair_code_id }`). Versions: [`AGENTS_CW20_CODE_ID_PIN.md`](../skills/AGENTS_CW20_CODE_ID_PIN.md). Do not follow `cw20-code-id-ops.md` 1.15.0 RAN. Leftover agents `UPGRADE582_PROBE_ONLY=1` — no store/migrate/2-of-3/pair-only `terrad tx`. `verify-issue-1234` must not claim `verify-issue-582` unless it runs it. |
| **M1300-5** | Do not close #1264; envelope **2,710,000**; AC1 unmeasured; G1264-4 no code. #1279 leftover-complete is ops-bot 1.5. #1240 hub rewrite already on main; Coolify hub wrap visual leftover is **#1305**. |
| **M1300-6** | Do not reopen closed parents. Do not wait on GitLab CI. Do not close #1264. Do not close #1279 from this ticket. Do not treat `VERIFY1279_IID=1279` / `LEFTOVER_COMPLETE=1` as PASS. **#1302 landed** on **#1305** — do not revert it. Hub wrap Coolify visual is **#1305 leftover-complete**. Do not merge `origin/cac-design-issue-1302` as-is (overlap on `architecture.md` / `qa-invariants.md` / README). That branch already publishes `docs/adr/0009-verify-issue-1290-hub-wrap-labels.md` / **Q22**. A later **0009** follow-up MR that inserts without taking 0008/Q21 is allowed. Do not touch `cac-design-issue-1276` / `1277`. Do not flip Coolify auto-deploy. Do not file a founder card. Keywords on #1300 are not #297 authority or `DESIGN: APPROVE`. |
