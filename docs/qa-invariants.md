...
| Invariant | Check | On failure |
| --------- | ----- | ---------- |
| **Q23** Post-merge PRs 1302–1304 leftover live is checked (Coolify `20260921130000` only — `…000`/`…001` are #1300; hub wrap `cLUNC / USD` vs CEX `LUNC` / selected `vFDUSD`; heal gate trades+raw only; dismiss then merge) | **`make verify-issue-1305`** → children **1290, 1277** plus leftover probes (**M1305-1–M1305-8**) | Non-zero exit; fix the failing child or Coolify leftover; do not treat green child docs-only as leftover live; do not close #1300; do not grep `protocol-top-pairs`; do not merge unpublished 0008/0009 tips as-is |
...
| ID | Rule |
|----|------|
| **M1305-1** | `make verify-issue-1305` runs children **1290, 1277**. Unit/docs FAILs fail the stack. Live leftover SKIP unless hosts answer (FAIL when `VERIFY1305_REQUIRE_LIVE=1` or `VERIFY1305_IID=1305`). Do not copy #1276 `EXPECT_SHA`. Do not invent leftover `DATABASE_URL`. Do not attest `…000`/`…001`. |
| **M1305-2** | Coolify Postgres has `20260921130000_traders_lifetime_heal_from_swaps` after #1300’s widen + 30d versions, then indexer redeploy. **#1305 attests that version only.** Operator attests `_sqlx_migrations` via Coolify DB / indexer `DATABASE_URL` (not `postgres-psql.sh`). D5 heal no-op or one log. |
| **M1305-3** | Coolify frontend rebuild from `729b097f+`. HTTP marker: **`cLUNC / USD`** + **`cUSTC / USD`**. Operator glance: oracle tabs **USTC** / **LUNC** / **vFDUSD** (selected not **VFDUSD**). Do not grep **`protocol-top-pairs`** (#1300). |
...
| **M1305-6** | Do not reopen closed parents. Do not wait on GitLab CI. Do not close #1300. Do not revert #1302. Do not merge `cac-design-issue-1300` / `1302` / `1277` as-is. Do not take ADR 0008/0009. Do not flip Coolify auto-deploy. Do not file a founder card. Keywords on #1305 are not #297 authority or `DESIGN: APPROVE`. |
| **M1305-7** | Leftover Playwright is child #1290 (`protocol-page`, 5 workers). Do not invent a second leftover e2e. Do not leak `PLAYWRIGHT_WEB_PORT`. `e2e-tx` stays 1 worker. |
