...
## Post-merge PRs 1302–1304 leftover verify (invariant Q23) {#post-merge-ops-1305}

**Q21** `{#post-merge-ops-1300}` and **Q22** `{#ops-hub-wrap-1302}` are reserved by unpublished sister designs ([#1300](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1300) ADR **0008**, hub-wrap bundle ADR **0009**). This leftover is **Q23**.

| Invariant | Check | On failure |
| --------- | ----- | ---------- |
| **Q23** Post-merge PRs 1302–1304 leftover live is checked (Coolify `20260921130000` only — `…000`/`…001` are #1300; leftover-1 is operator glance hub `cUSTC / USD` + `cLUNC / USD` vs CEX `USTC` / `LUNC` / `vFDUSD` / click **vFDUSD** selected not `VFDUSD` (idle `/protocol` is USTC); HTTP unquoted `cLUNC wrap` / `cLUNC` / `cUSTC` / `vFDUSD` are supporting rebuild-presence, not leftover-1; startup-before-D5 heal; heal gate trades+raw only; dismiss then merge) | **Make (implement):** `make verify-issue-1305` → children **1290** (`VERIFY_ISSUE_1290_SKIP_E2E=1`), **1277** + optional quote-agnostic HTTP. Make does **not** cover leftover-complete (no sqlx, no heal logs, no leftover-1 glance; HTTP is not leftover-1). **Leftover-complete (operator):** paste ADR 0010 / playbook close-comment template — sqlx SELECT `20260921130000` `success=true` + heal needle `traders_healed lifetime totals from swap_events (GitLab #1277)` + leftover-1 glance hub **`cUSTC / USD`** + **`cLUNC / USD`** vs CEX **`USTC`** / **`LUNC`** / **`vFDUSD`** / click **vFDUSD** selected not **`VFDUSD`**. Forbidden in that comment: `GET /health`, `protocol-top-pairs`, hashed-chunk `cLUNC` PASS, green make. | Non-zero make: fix the failing child or HTTP probe. Do not treat green make or HTTP PASS as leftover-complete or leftover-1. Leftover-1 is operator DOM glance (hub + CEX + selected vFDUSD). Do not grep concatenated `cLUNC / USD` in hashed chunks or `protocol-top-pairs`; never require source `'`; do not close #1300; do not merge unpublished 0008/0009 tips as-is |

**M1305** (Forgejo **#1305** — [`skills/AGENTS_POST_MERGE_OPS_1305.md`](../skills/AGENTS_POST_MERGE_OPS_1305.md); design [`docs/adr/0010-post-merge-leftover-1302-1304.md`](./adr/0010-post-merge-leftover-1302-1304.md)):

| ID | Rule |
|----|------|
| **M1305-1** | `make verify-issue-1305` **(implement)** runs children **1290** (`VERIFY_ISSUE_1290_SKIP_E2E=1` by default: docs + `hubPriceTicker` + `verify-issue-1240`) and **1277**. Unit/docs FAILs fail the stack. Live leftover SKIP unless hosts answer (FAIL when `VERIFY1305_REQUIRE_LIVE=1` or `VERIFY1305_IID=1305`). Live make is HTTP only (supporting pins). Make does **not** cover leftover-complete. Do not invent `VERIFY1305_LEFTOVER_COMPLETE`. HTTP PASS is not leftover-1 and not sqlx leftover-complete. Do not copy #1276 `EXPECT_SHA`. Do not invent leftover `DATABASE_URL`. Do not attest `…000`/`…001`. |
| **M1305-2** | Coolify Postgres has `20260921130000_traders_lifetime_heal_from_swaps` after #1300’s widen + 30d versions. **#1305 attests that version only** via Coolify leftovers **SELECT** (`success=true` — Coolify DB / indexer `DATABASE_URL`; not `postgres-psql.sh`). If the #1276 checkbox is off, leftover 2 is a **manual** Coolify indexer deploy of `729b097f+`. Healthy `GET /health` is not `_sqlx_migrations` evidence. Heal is **startup-before-D5**, not D5, not in `run_volume_refresh_loop`. Heal attest needle (indexer logs, not `DATABASE_URL`): `traders_healed lifetime totals from swap_events (GitLab #1277)` — one on first start after migrate, then silence. |
| **M1305-3** | Coolify frontend rebuild from `729b097f+`. **Leftover-1 is operator DOM glance:** hub **`cUSTC / USD`** + **`cLUNC / USD`** vs CEX **`USTC`** / **`LUNC`** / **`vFDUSD`** / click **vFDUSD** selected not **`VFDUSD`**. Idle `/protocol` selects **USTC**. **Selected not `VFDUSD`** is the #1240 CSS guard (`textTransform: 'none'`). Operator **must click the vFDUSD tab**. HTTP pins unquoted **`cLUNC wrap`**, **`cLUNC`**, **`cUSTC`**, **`vFDUSD`** (any quote style; never require source `'`) are supporting rebuild-presence — **none prove leftover 1**. Unquoted **`cLUNC`** is a substring of pre-1302 **`cLUNC wrap`**; implement live HTTP must not treat it as leftover-1 complete. Do not grep concatenated **`cLUNC / USD`** in hashed chunks. Do not grep **`protocol-top-pairs`** (#1300). |
| **M1305-4** | Optional **I11**: USD-only lifetime skew does not make `trader_lifetime_diverges_from_swaps` true (trades+raw only). Do not add `total_volume_usd` to the EXISTS. Do not name this test I10 (I10 is missing-trader / ghost-zero). |
| **M1305-5** | Dismiss CODEOWNERS self-request then `fj pr merge`. Do not `force_merge`. #1302/#1303 force_merge commits are ancestors of `main`. |
| **M1305-6** | Do not reopen closed parents (#1290 / #1240 / #1277). Do not wait on GitLab CI. Do not close #1300. Do not revert #1302. Do not merge `cac-design-issue-1300` / `1302` / `1277` as-is. Do not take ADR 0008/0009. Do not flip Coolify auto-deploy. Do not file a founder card. Do not infer leftover-complete from the 06:46 close comment. If **#1305** is still closed on that comment, reopen it as a leftover precondition (not leftover-complete). Keywords on #1305 are not #297 authority or `DESIGN: APPROVE`. |
| **M1305-7** | Child #1290 Playwright is not leftover-complete. Slice 1 defaults `VERIFY_ISSUE_1290_SKIP_E2E=1`. Optional leftover e2e stays explicit (5 workers, child’s port; no extra `PLAYWRIGHT_WEB_PORT`). Do not wait leftover-complete on #703 `YY-MM` ticks. `e2e-tx` stays 1 worker. |
| **M1305-8** | Playbook + this Q23 + ADR 0010 + child skills stay crosslinked. GitLab CI quota is not a substitute for local verify. |
...
- [`skills/AGENTS_POST_MERGE_OPS_1305.md`](../skills/AGENTS_POST_MERGE_OPS_1305.md) — post-merge PRs 1302–1304 leftover verify ([#1305](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1305), **Q23**); [ADR 0010](./adr/0010-post-merge-leftover-1302-1304.md)
