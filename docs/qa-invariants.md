...
| Invariant | Check | On failure |
| --------- | ----- | ---------- |
| **Q23** Post-merge PRs 1302–1304 leftover live is checked (Coolify `20260921130000` only — `…000`/`…001` are #1300; HTTP literals `cLUNC wrap` / `'cLUNC'` / `'cUSTC'` / `'vFDUSD'`; glance hub `cUSTC / USD` vs CEX `LUNC` / selected not `VFDUSD`; heal gate trades+raw only; dismiss then merge) | **`make verify-issue-1305`** → children **1290** (`VERIFY_ISSUE_1290_SKIP_E2E=1` by default), **1277** plus leftover probes (**M1305-1–M1305-8**) | Non-zero exit; fix the failing child or Coolify leftover; do not treat green child docs-only or HTTP PASS as sqlx leftover-complete; do not grep concatenated `cLUNC / USD` or `protocol-top-pairs`; do not close #1300; do not merge unpublished 0008/0009 tips as-is |
...
| **M1305-2** | Coolify Postgres has `20260921130000_traders_lifetime_heal_from_swaps` after #1300’s widen + 30d versions. **#1305 attests that version only.** If the #1276 checkbox is off, leftover 2 is a **manual** Coolify indexer deploy of `729b097f+`. Healthy `GET /health` is not `_sqlx_migrations` evidence. Operator attests `_sqlx_migrations` via Coolify DB / indexer `DATABASE_URL` (not `postgres-psql.sh`). D5 heal no-op or one log. |
| **M1305-3** | Coolify frontend rebuild from `729b097f+`. HTTP markers (minify-safe literals): **`cLUNC wrap`**, `'cLUNC'`, `'cUSTC'`, `'vFDUSD'`. Operator glance: hub **`cUSTC / USD`** vs CEX **`LUNC`** / selected not **`VFDUSD`**. Do not grep concatenated **`cLUNC / USD`**. Do not grep **`protocol-top-pairs`** (#1300). |
| **M1305-4** | Optional I10: USD-only lifetime skew does not re-trip the poller gate (trades+raw only). Do not add `total_volume_usd` to the EXISTS. |
...
| **M1305-6** | Do not reopen closed parents. Do not wait on GitLab CI. Do not close #1300. Do not revert #1302. Do not merge `cac-design-issue-1300` / `1302` / `1277` as-is. Do not take ADR 0008/0009. Do not flip Coolify auto-deploy. Do not file a founder card. Do not infer leftover-complete from the 06:46 close comment. Keywords on #1305 are not #297 authority or `DESIGN: APPROVE`. |
