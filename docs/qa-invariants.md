...
| Invariant | Check | On failure |
| --------- | ----- | ---------- |
| **Q22** Post-merge hub wrap **cLUNC** vs CEX **LUNC** has an agent `make` bundle; leftover-complete is the **#1306** wiring MR, not green skip-E2E | **`make verify-issue-1290`** (aliases **`verify-issue-1302`** / **`verify-issue-1306`**) → child **1240** + `hubPriceTicker` Vitest + optional Playwright `protocol-page` at 5 workers (**B1290-1–B1290-8**). Related #1302 / !1290 / #1240. Closed !1302 is a weaker sketch; continue open !1306 or a new branch on current `main`, do not retarget. Do not merge `2d4e8b64` as-is. ADR **0008** / **Q21** stay with leftover-ops [#1300](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1300). | Non-zero exit; fix the failing child or docs. `VERIFY_ISSUE_1290_SKIP_E2E=1` is a required implement check, **not** leftover-complete — leftover-complete is the #1306 wiring MR (aliases, `free_tcp_port`, full B1290-4 **including recipe-anchored Makefile alias recipes**, leftover **#1306** naming, slice-0 docs, policy-clean messages, help `run_step` pinning `@echo` lines). Land leftover evidence on a **provisioned Cloud Agent** (Node 24 via `scripts/setup-cloud-agent-toolchain.sh` / `scripts/with-node.sh`; Playwright Chromium via `scripts/setup-cloud-agent-localterra.sh` `_ensure_playwright_browsers` or the equivalent `playwright install chromium`; do **not** treat `scripts/setup-browser-cloud-agent.sh` as the Chromium install): skip-E2E **and** full Playwright. Current Woodpecker is gitleaks-only and must not. A *future* CI job that actually runs `make verify-issue-1306` may substitute. A gate VM that cannot provision Node 24 via `scripts/with-node.sh` / `cloud_agent_ensure_node` is not that runner. Production `/protocol` glance is [#1305](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1305) leftover 1, not this ticket. Do not reopen #1240 / !1290. Do not `Fixes #1290`. Do not `Closes #1302`. Do not scrape production HTML. Do not land `frontend-dapp/node_modules` or hook-test FAIL residue. |

**B1290** (Forgejo **[#1306](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1306)** — [ADR 0009](./adr/0009-verify-issue-1290-hub-wrap-labels.md); first leftover sketch !1302):

| ID | Rule |
|----|------|
| **B1290-1** | Bundle runs child **1240**. Child FAIL fails the stack. `verify-issue-1302` and `verify-issue-1306` are the same target. |
| **B1290-2** | `hubPriceTicker.test.ts` is a required step. |
| **B1290-3** | Playwright default-on, **5 workers**, port **30129**, `PLAYWRIGHT_SKIP_CHAIN=1`. Copy `free_tcp_port` from `verify-issue-703.sh` (function only — not 703’s `ln -sfn` bootstrap); free 30129 first; port + `PLAYWRIGHT_BASE_URL` only in that step’s subshell. Skip only `VERIFY_ISSUE_1290_SKIP_E2E=1` or missing Playwright package. Installed package + crash (Chromium / NSS) = FAIL. Failure to provision Node 24 via `scripts/with-node.sh` / `cloud_agent_ensure_node` = FAIL (not skip). `VERIFY_ISSUE_1290_SKIP_E2E=1` skips Playwright only. |
| **B1290-4** | Crosslinks: testing, AGENTS, PROTOCOL_STATS, PROTOCOL_HUB, frontend.md, ADR 0009, architecture `#dex-hub-wrap-labels`, this Q22, **and recipe-anchored Makefile aliases** `^verify-issue-1302:[[:space:]]*verify-issue-1290[[:space:]]*$` / `^verify-issue-1306:[[:space:]]*verify-issue-1290[[:space:]]*$`. Unanchored `grep -qE "verify-issue-1306" Makefile` is **not** B1290-4 (`.PHONY` / help still hit after dropping the recipes). Cloud Agent + Frontend `help` is a **separate** `run_step` pinning `@echo` lines (not this ID). #1306 **inserts** those slice-0 sections (do not whole-file checkout overlap files from the design tip); do not shrink greps to the closed-!1302 sketch, to eight docs without Makefile, or to unanchored Makefile substring greps. |
...
| **B1290-7** | Do not add `frontend-dapp/node_modules`, hook-test FAIL residue, `.gitignore` churn, or `AGENTS_POST_MERGE_OPS_1302.md` / `AGENTS_POST_MERGE_OPS_1306.md`. Do not edit `scripts/test-commit-msg-hook.sh`. Do not take ADR 0008 / Q21 (#1300). Merge-range bodies must pass `.githooks/commit-msg`. Do not land `2d4e8b64` as-is. Never `--no-verify`. |
| **B1290-8** | Green skip-E2E ≠ leftover-complete. Leftover-complete is the **#1306** wiring MR (aliases, `free_tcp_port`, full B1290-4 including recipe-anchored Makefile, leftover **#1306** naming, slice-0 docs, policy-clean messages, help `run_step` pinning `@echo` lines). Land leftover evidence requires a **provisioned Cloud Agent** (Node 24 via `scripts/setup-cloud-agent-toolchain.sh` / `scripts/with-node.sh`; Playwright Chromium via `scripts/setup-cloud-agent-localterra.sh` `_ensure_playwright_browsers` or the equivalent `playwright install chromium`; do **not** treat `scripts/setup-browser-cloud-agent.sh` as the Chromium install): skip-E2E **and** full Playwright. Current Woodpecker is gitleaks-only and must not stand in. A *future* CI job that actually runs `make verify-issue-1306` may substitute. Production `/protocol` visual is [#1305](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1305) leftover 1 (related #1290 AC), not this ticket. No HTML scrape. Not [agent-control #297](https://git.cl8y.com/PlasticDigits/cl8y-agent-control/issues/297). |
...
- [ADR 0009](./adr/0009-verify-issue-1290-hub-wrap-labels.md) — leftover `make verify-issue-1290` hub wrap vs CEX labels ([#1306](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1306), **Q22**); ADR **0008** / **Q21** reserved for leftover-ops [#1300](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1300)
