...
| Invariant | Check | On failure |
| --------- | ----- | ---------- |
| **Q23** Post-merge PRs 1302–1304 leftover live is checked (Coolify `20260921130000` only — `…000`/`…001` are #1300; leftover-1 is operator glance hub `cUSTC / USD` + `cLUNC / USD` vs CEX `USTC` / `LUNC` / `vFDUSD`; HTTP unquoted `cLUNC wrap` / `cLUNC` / `cUSTC` / `vFDUSD` are supporting rebuild-presence, not leftover-1; startup-before-D5 heal; heal gate trades+raw only; dismiss then merge) | **Make (implement):** `make verify-issue-1305` → children **1290** (`VERIFY_ISSUE_1290_SKIP_E2E=1`), **1277** + optional quote-agnostic HTTP. Make does **not** cover leftover-complete (no sqlx, no heal logs, no leftover-1 glance; HTTP is not leftover-1). **Leftover-complete (operator):** sqlx `20260921130000` `success=true` + startup-before-D5 heal logs + hub glance **`cUSTC / USD`** + **`cLUNC / USD`**. | Non-zero make: fix the failing child or HTTP probe. Do not treat green make or HTTP PASS as leftover-complete or leftover-1. Leftover-1 is operator DOM glance. Do not grep concatenated `cLUNC / USD` in hashed chunks or `protocol-top-pairs`; never require source `'`; do not close #1300; do not merge unpublished 0008/0009 tips as-is |
...
| **M1305-2** | Coolify Postgres has `20260921130000_traders_lifetime_heal_from_swaps` after #1300’s widen + 30d versions. **#1305 attests that version only** via Coolify DB / indexer `DATABASE_URL` (not `postgres-psql.sh`). If the #1276 checkbox is off, leftover 2 is a **manual** Coolify indexer deploy of `729b097f+`. Healthy `GET /health` is not `_sqlx_migrations` evidence. Heal is **startup-before-D5**, not D5, not in `run_volume_refresh_loop`. Startup heal is a no-op or one `traders_healed` info; later process starts stay silent if the gate is false. Heal silence/one log via **indexer logs** — not queryable from `DATABASE_URL`. |
| **M1305-3** | Coolify frontend rebuild from `729b097f+`. **Leftover-1 is operator DOM glance:** hub **`cUSTC / USD`** + **`cLUNC / USD`** vs CEX **`USTC`** / **`LUNC`** / **`vFDUSD`** / selected not **`VFDUSD`**. HTTP pins unquoted **`cLUNC wrap`**, **`cLUNC`**, **`cUSTC`**, **`vFDUSD`** (any quote style; never require source `'`) are supporting rebuild-presence — **none prove leftover 1**. Unquoted **`cLUNC`** is a substring of pre-1302 **`cLUNC wrap`**; implement live HTTP must not treat it as leftover-1 complete. Do not grep concatenated **`cLUNC / USD`** in hashed chunks. Do not grep **`protocol-top-pairs`** (#1300). |
