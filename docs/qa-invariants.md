...
| Invariant | Check | On failure |
| --------- | ----- | ---------- |
| **Q21** Post-merge PRs 1287–1298 leftover live is checked (Coolify `20260921120000` then `20260921120001` only — `20260921130000` is #1305; columbus-5 pair LCD cw2 1.17.0 / 11672 **probe-attest** leftover-complete (leftover-ops recorded 11672; store only if still 1.16.0; do not reopen #1300 for wasm); LocalTerra/manual four walks as leftover-*ops* evidence — tracker-close comments do not waive them; leftover-*ops* comments the closed tracker with the four walk notes (reopen is optional noise vs sister leftover playbooks); Forgejo closed ≠ leftover-complete; `has-localterra` down is not leftover-complete; leftover-complete is **operator + leftover-*ops***, green `make verify-issue-1300` never means close; `VERIFY1300_REQUIRE_HAS_LOCALTERRA=1` is fail-closed `has-localterra` only; **`VERIFY1300_SKIP_HAS_LOCALTERRA` only** (not `SKIP_HAS_LOCALTERRA`); defining `VERIFY1300_REQUIRE_CHAIN` as Playwright/`run_leftover_e2e` is leftover-implement FAIL; frontend `protocol-top-pairs` / `Top pairs (30d)` in #673/#686 `chunk("ProtocolPage")` — not #701 PoolPage regex; #1264 closed (AC1 measured; do not reopen; envelope 2.71M); #1255 Create Pair Custom contract paste then Swap pick (Swap paste ban Q1255-4 only), leftover pass is Q1255-6 Max; #1279 ops-bot; #1302 landed on #1305 — do not revert; do not merge `cac-design-issue-1302` as-is; later 0009 without taking 0008/Q21 is allowed) | **`make verify-issue-1300`** → children **1287, 1234, 1265, 1240, 1279, 1277, 1264, 1285, 1255, 1219, 1218, 1263** plus leftover probes (**M1300-1–M1300-8**) | Non-zero exit; fix the failing child or Coolify/columbus-5 leftover; do not treat green verify, a tracker-close comment, or Forgejo closed as leftover-complete; leftover-*ops* comments or reopens the closed tracker until the four walk notes exist; do not reopen #1264; do not treat green #1279 make as leftover-complete; do not treat chain-probe SKIP as item-5 leftover-complete; do not revert #1302; do not FAIL on `20260921130000` |
...
| ID | Rule |
|----|------|
| **M1300-1** | `make verify-issue-1300` runs children **1287, 1234, 1265, 1240, 1279, 1277, 1264, 1285, 1255, 1219, 1218, 1263**. Unit/docs FAILs fail the stack. Green `make verify-issue-1300` never means leftover-complete. Live leftover SKIP unless hosts answer (FAIL when `VERIFY1300_REQUIRE_LIVE=1` or `VERIFY1300_IID=1300`). Default leftover verify probes `make has-localterra` (down → SKIP; up → PASS the probe) and does not execute the four leftover walks even when the chain is up. **`VERIFY1300_REQUIRE_HAS_LOCALTERRA=1` is fail-closed `make has-localterra` only** — no Playwright, does not execute the walks. **Slice-2 FAIL:** defining `VERIFY1300_REQUIRE_CHAIN` as Playwright / `run_leftover_e2e`. **`VERIFY1300_SKIP_HAS_LOCALTERRA=1`** skips the probe (do **not** ship `SKIP_HAS_LOCALTERRA`). Tracker-close comments on #1300 do **not** waive the four walks. The tracker is already **closed**; leftover-*ops* **comments the closed tracker** with the four walk notes (reopen is optional noise vs sister leftover playbooks). Forgejo closed ≠ leftover-complete. Walks stay leftover-complete leftover-*ops* evidence (playbook how-tos). `has-localterra` down is **not** leftover-complete. Chain-probe SKIP/PASS is not item-5 evidence. Cloud Agent leftover-*ops* (not leftover implement / slice 2) may `make setup-cloud-localterra` when item 5 still lacks walk notes; that is not implement-slice close. Slice 2 never executes the walks. Do not close on probe SKIP. Child Vitest is not the walk. Leftover stacked Playwright is SKIP until named (**M1300-7**). Do not copy #1276 `EXPECT_SHA` leftover-complete. Do not invent leftover `DATABASE_URL`. Do not add sqlx `20260921130000` to this verify. |
| **M1300-2** | Coolify Postgres has `20260921120000_traders_rolling_volume_numeric_38_0` then `20260921120001_pair_volume_30d`, then indexer redeploy. **#1300 attests those two versions only.** `20260921130000` is **#1305**, not a #1300 FAIL. Operator attests `_sqlx_migrations` via Coolify DB / indexer `DATABASE_URL` (not `postgres-psql.sh`). Live `GET /api/v1/protocol/top-pairs` ≤5 economic rows after one rollup tick; `?limit=6` → **400** (**I1263-6**). Vite `/protocol` is frontend rebuild evidence. `#1277` leftover-complete is migrate applied + aggregator still running. |
| **M1300-3** | Coolify frontend rebuild from `6d34da13+`. HTTP marker: **`protocol-top-pairs` / `Top pairs (30d)`** in hashed Vite **`chunk("ProtocolPage")`** (#673/#686; `App.tsx` lazy `ProtocolPage`). Do **not** copy #701 `PoolPage\|chartsOverviewStats\|pairCatalog`. Do not grep hub `cLUNC / USD` (#1305). Keep issue item 5 as leftover-complete leftover-*ops* evidence: leftover-complete still needs the four walks (tracker-close comments do **not** waive them; leftover-*ops* comments the closed tracker with the four walk notes (reopen is optional noise vs sister leftover playbooks); Forgejo closed ≠ leftover-complete). **#1218** Pay LUNC / Receive USTR **or** JADE/RUBY E7 stand-in, wrap prefix + solver hops — do not require Route `cUSTC` or `USTR`. **#1255 LocalTerra only** — Minting (`mint_control`) is a paid **50 UST1 SKU** (`PayWithAnyToken`); Simulated Wallet `test1` already has ≫50 UST1 after `make setup-cloud-localterra` (SmokeUST1 **L620-5**); free create cannot mint; a `dex.cl8y.com` Create Token is spend + live CW20; set **Decimals** to **18**, enable **Minting** at create; mint cap empty (unlimited — **valid**) or human `≥1`; after mint `/create` **Custom contract** paste (**C542**) + pick UST1/cUSTC/EMBER (prefer **cUSTC or EMBER** if `DEPLOY_SKIP_COMMUNITY_TAX=1`); remount Swap; Swap **pick** (**Do not paste on Swap** — **Q1255-4 / QS-3** only); leftover pass is Max of minted raw **`1000000000000000000`** → human `1` (**Q1255-6**) — drop typed `1` execute/sim unless leftover-*ops* **seeds LP**; do not mix mint cap with the raw Mint field. **#1219** LocalTerra in-tree 1.17.0 dust ladder copy / Place disabled, no popup — **not columbus-5 wasm leftover** (issue leftover 3 is `#1264`). **#1263** LocalTerra `/protocol` **Top pairs (30d)** renders with indexer-from-this-tip-up, ≤5 rows — Coolify-only `?limit=6` → 400. `has-localterra` down is **not** leftover-complete. Cloud Agent leftover-*ops* may `make setup-cloud-localterra` when item 5 still lacks walk notes; that is not implement-slice close. Chain-probe SKIP/PASS is not item-5 evidence. Child Vitest is not the walk. Green `make verify-issue-1300` never means close. `VERIFY1300_REQUIRE_HAS_LOCALTERRA=1` is fail-closed `has-localterra` only. **`VERIFY1300_SKIP_HAS_LOCALTERRA` only.** |
| **M1300-4** | columbus-5 pair leftover-complete is **probe-attest** LCD cw2 **1.17.0** (`UPGRADE582_PROBE_ONLY=1`). Leftover-ops recorded **1.17.0 / 11672**. Comment that attest on the closed tracker. A second store is a leftover defect. Store/2-of-3 / `UpdateConfig { pair_code_id }` is operator-only if probe is still **1.16.0** (`UPGRADE582_SKIP_FACTORY_MIGRATE=1`; factory already **1.10.0 / 11629**). Do not reopen #1300 for wasm. Versions: [`AGENTS_CW20_CODE_ID_PIN.md`](../skills/AGENTS_CW20_CODE_ID_PIN.md). Do not follow `cw20-code-id-ops.md` 1.15.0 RAN. Leftover agents never `terrad tx`. `verify-issue-1234` must not claim `verify-issue-582` unless it runs it. |
| **M1300-5** | Do **not** reopen #1264. Envelope **2,710,000**. Do **not** raise `WRAP_ROUTER_COMBO_OVERHEAD_GAS`. Operator AC1: tx `53B06B653D78AF3683F51A065FC640074A3A2E76CAE343A97B3E0F0BD79BAC36`, `gas_wanted=2710000`, `gas_used=2630228`, class not-A/not-B, issue **closed**. Child `make verify-issue-1264` stays in the stack. Optional `VERIFY1264_COLUMBUS_TX` may pin that hash. G1264-4 no code. #1279 leftover-complete is ops-bot 1.5. #1240 hub rewrite already on main; Coolify hub wrap visual leftover is **#1305**. |
| **M1300-6** | Do not reopen closed parents (including **#1264**). Do not wait on GitLab CI. Do not close #1279 from this ticket. Tracker-close comments on #1300 do not waive the four walks. The tracker is already closed; leftover-*ops* comments the closed tracker with the four walk notes (reopen is optional noise vs sister leftover playbooks). Forgejo closed ≠ leftover-complete. Do not treat `VERIFY1279_IID=1279` / `LEFTOVER_COMPLETE=1` as PASS. **#1302 landed** on **#1305** — do not revert it. Hub wrap Coolify visual is **#1305 leftover-complete**. Do not merge `origin/cac-design-issue-1302` as-is (overlap on `architecture.md` / `qa-invariants.md` / README). That branch already publishes `docs/adr/0009-verify-issue-1290-hub-wrap-labels.md` / **Q22**. A later **0009** follow-up MR that inserts without taking 0008/Q21 is allowed. Do not touch `cac-design-issue-1276` / `1277`. Do not flip Coolify auto-deploy. Do not file a founder card. Keywords on #1300 are not #297 authority or `DESIGN: APPROVE`. |
| **M1300-7** | Leftover stacked Playwright is **manual**; `VERIFY1300_LEFTOVER_E2E=1` **SKIP until named**. `VERIFY1300_REQUIRE_HAS_LOCALTERRA` is not that flag. Defining `VERIFY1300_REQUIRE_CHAIN` as Playwright / `run_leftover_e2e` is a leftover-implement FAIL. Child verifies already run `wrap-swap.spec.ts` / #1218 and ladder UI / #1219. `protocol-page.spec.ts` does not assert Top pairs. Do not invent five-worker leftover coverage. Do not leak a different `PLAYWRIGHT_WEB_PORT`. `e2e-tx` stays 1 worker. |
