...
**Errors:** unknown pair → **404**; LCD failure → **502**; invalid cursor / side mismatch → **400**. Sustained abuse → **429** (`Retry-After` + `x-ratelimit-*` headers): global **`RATE_LIMIT_RPS`** (default **60**) on all routes, plus **`RATE_LIMIT_LCD_HEAVY_RPS`** (default **10**) on the LCD-heavy paths above — production cannot disable either ([#363](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/363); [indexer-invariants.md](./indexer-invariants.md)).
