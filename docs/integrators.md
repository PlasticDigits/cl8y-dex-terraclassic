...
Updating only the **price** of an existing order (`ExecuteMsg::UpdateLimitOrderPrice`) re-links the order in the FIFO book **without** charging the maker placement fee again (same `order_id`, no CW20). Relink **joins the tail** of the destination equal-price run (time-priority at the quoted price — [#1227](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1227) / **L23**); it does **not** keep place-time rank via the preserved id. **F6 freeze** rejects the relink before DLL mutation ([#1234](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1234)). Cancel + new placement pays a new maker-side half. Placement uses the maker’s `limit_discount_bps` when set ([#514](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/514)); fills still charge the **taker** half of the taker’s swap `discount_bps`. See [fee-discount-tiers.md](./reference/fee-discount-tiers.md) **I13**.
...
**Limit `price` is raw token1 base units per token0 base unit.** The pair applies **[`MIN_LIMIT_PRICE`, `MAX_LIMIT_PRICE`]** to the **human-scale** value `raw × 10^(decimals0 − decimals1)` ([#529](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/529) / **L20**). For UST1 (6) / USTR (18) at ~79 human, send raw `≈ 7.9e13`, not `79`. Indexer `limit-book` prices are the same raw strings. See [limit-orders.md § Limit price band](./limit-orders.md#limit-price-band-gitlab-467) and [skills/AGENTS_LIMIT_PRICE_DECIMALS.md](../skills/AGENTS_LIMIT_PRICE_DECIMALS.md).
...
**Open liveness gap — [Forgejo #1225](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1225), invariant [L26](./contracts-security-audit.md):** the human-scale price gate in [limit_placement.rs](../smartcontracts/packages/dex-common/src/limit_placement.rs) accepts MIN_LIMIT_PRICE, while pair [match_asks](../smartcontracts/contracts/pair/src/orderbook.rs) and indexer [db_orderbook_sim](../indexer/src/api/db_orderbook_sim.rs) skip zero-cost rows without unlinking them. Default CleanLimitBook thresholds are 0/0, so a live, no-expiry minimum-price row can stay at the ask head. A book_start_hint past the row only helps that taker; it does not clear the book. The placement size floor in [#1219](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1219) does not enforce remaining × price ≥ 1. See the pair and indexer paths plus required parity/scan tests in [limit-orders.md](./limit-orders.md#unfillable-in-band-floor-orders-forgejo-1225) and [security invariant L26](./contracts-security-audit.md). The proposed omitted-parameter greedy default [#718](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/718) remains gated on resolution.
...
**Without `belief_price`:** The check compares `max_spread` to `(spread_cmp + book_shortfall) / total_gross_out` (strict `>` fails), where `spread_cmp = min(pool_spread, pool_gross)`, `total_gross_out = pool_gross + book_return_net`, and when both legs are present `book_shortfall = max(0, pool_net × book_input / pool_input − book_return_net)` using the pool’s realized **net** rate as the book reference ([#273](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/273)). Hybrid with **`declared_pool_input > 0`** and `book_input > 0` also requires **`declared_pool_input ≥ 10%` of `offer_amount`** (min 1 raw unit) and **`pool_net_return > 0`** before execute ([#307](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/307)); pair and router reject early on the declared split. **Pure-book** intent (`declared_pool_input = 0`) is not bounded by this metric — on **execute**, set `belief_price` or pair `min_return` on the swap hook (router: per-hop `min_return` on `terra_swap` when `book_input > 0`, plus optional final `minimum_receive`; see [#334](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/334)).

**With `belief_price`:** Expected output is `offer_amount / belief_price` (in ask units). `belief_price` must be strictly positive and must produce `expected_return >= 1` raw unit; otherwise the swap (and `HybridSimulation` when `bp = 0` or `1/bp` underflows) is rejected ([#1230](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1230)). Actual output used in the inequality is `book_return_net + pool_net_to_receiver + pool_commission` (pool commission to treasury counts on the “actual” side). The material-pool floor ([#307](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/307)) is skipped only when `belief_price` is **usable**; a dummy dust-floor belief does not skip L9 or #307.
...
**Invariant L8:** Pool-only quotes use `hybrid_simulation` with `book_input = 0` (helpers: `pool_only_hybrid_params`, `pool_only_hybrid_template`). Router ops with `hybrid: null` still get pool-only hybrid quotes on-chain. For book-inclusive quotes set non-zero `book_input` on the router op or pair query. **Declared hybrid is a partition of that hop’s offer** (`pool_input + book_input` via `checked_add`). Do **not** reuse hop-0 integers on later hops: execute sizes hop 1+ from realized `hop_output`. Retail **`GET /route/solve` emits declared hybrid on hop 0 only**; later hops are pool-only ([#1280](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1280) Policy A). **`POST` `hybrid_by_hop`** may still declare interior splits that partition that hop. Wrap/native execute hops omit `hybrid` ([#1264](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1264)). Native wrap-enter quotes GET after a **client** wrap-map (`uluna`→cLUNC / `uusd`→cUSTC); indexer GET still 400s `token_in=uluna` ([#1218](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1218)). Playbook: [`skills/AGENTS_HYBRID_HOP_OFFER.md`](../skills/AGENTS_HYBRID_HOP_OFFER.md) · [`skills/AGENTS_FRONTEND_WRAP_ENTER_ROUTE_SOLVE.md`](../skills/AGENTS_FRONTEND_WRAP_ENTER_ROUTE_SOLVE.md). **Fee discounts:** pass optional `trader` (and `sender` when the CW20 sender differs, e.g. trusted router) on `HybridSimulation` / router `SimulateSwapOperations` so quotes match execute for registered CL8Y tiers; omit `trader` for undiscounted (full fee) quotes ([#238](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/238)). Indexer **`GET/POST /api/v1/route/solve`** accept the same optional fields; hybrid GET cache keys include normalized `trader` when set ([#245](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/245)) and the resolved **`discount_tier`** from synced `traders.tier_id` for the discount subject (`trader` if set, else `sender`) so same-tier callers share the cache ([#283](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/283)). See [limit-orders.md](./limit-orders.md), [ADR 0001](./adr/0001-hybrid-quoting-and-routing.md), [skills/AGENTS_HYBRID_QUOTING.md](../skills/AGENTS_HYBRID_QUOTING.md), [skills/AGENTS_FEE_DISCOUNT_TIERS.md](../skills/AGENTS_FEE_DISCOUNT_TIERS.md).

**Greedy book-first ([GitLab #708](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/708) / leftovers [#709](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/709) [#710](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/710), invariants G1–G14):** opt-in `greedy: GreedySwapParams { max_maker_fills, book_start_hint }` on pair `Swap` and router `TerraSwap`. Do **not** send Pattern C `pool_input=0, book_input=offer` for this path (**G11**). Setting **both** `hybrid` and `greedy` **errors** on pair `HybridSimulation`, pair execute, and router sim/execute — same `resolve_swap_hybrid_mode` (**G7** / **A11**). Current pair/router wasm maps omitted `hybrid` + omitted `greedy` to pool-only (**G1** / **G12**); [#718](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/718) tracks the not-yet-shipped opt-out default and required pair + router migrations. Quote explicit greedy with pair `HybridSimulation` `{ hybrid: null, greedy }` (**G7**). Execute needs `belief_price` or `min_return` (**G8**). `max_maker_fills` of `0` rejects; values above 100 clamp (**G5**). Wasm `greedy_stop`: `worse_than_pool` | `max_makers` | `scan_cap` | `empty` (no maker filled) | `filled` (offer fully on book) | `remainder_to_pool` (makers filled, leftover → AMM — **not** `empty`). Pool-spot overflow is Skip, not a panic (**A7**). Official UI still uses `GET /route/solve` (**G4**). Reverse sim does not take `greedy`. Playbook: [`skills/AGENTS_GREEDY_BOOK_FIRST.md`](../skills/AGENTS_GREEDY_BOOK_FIRST.md).
...
| On-chain swap succeeds with `effective_fee_bps = fee_bps` | Full fee applied (registered or not) | Reconcile with LCD/health above — outage vs genuinely unregistered. **Also check pair `GetDiscountRegistry`:** if `registry` is `null`, the pair is unwired and full fee is correct even for a registered tier holder ([#537](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/537) / [#538](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/538), **I14**). |

Canonical tier numbers remain only in [fee-discount-tiers.md](./reference/fee-discount-tiers.md) — do not duplicate the ladder here. Agent playbook: [skills/AGENTS_FEE_DISCOUNT_TIERS.md](../skills/AGENTS_FEE_DISCOUNT_TIERS.md) § Registry outage observability. dApp fee chrome: [skills/AGENTS_FRONTEND_PAIR_FEE_DISCOUNT.md](../skills/AGENTS_FRONTEND_PAIR_FEE_DISCOUNT.md). Regression: `make verify-issue-365`. Pair-registry UI gate: `make verify-issue-537`. CreatePair inherit + smart-query-first: `make verify-issue-538`.
...
Upstream [PR #132](https://github.com/chainapsis/keplr-contract-registry/pull/132) merged on 2026-09-04, adding CL8Y, UST1, cLUNC, cUSTC, and vFDUSD; **USTR was already listed** as **USTC Repeg**. The merged JSON matches on-chain `token_info`; the local pack is still pre-merge for CL8Y / cLUNC / cUSTC, so do not re-export over upstream until synced. Recognition JSON has **no** invented price fields; optional `coinGeckoId` is `ceramicliberty-com` on CL8Y only (CoinGecko still maps that id to BNB Chain). Invariants **K629-1–K629-9**: [`listings/keplr-contract-registry/README.md`](./listings/keplr-contract-registry/README.md). Agent playbook: [`skills/AGENTS_KEPLR_CW20_REGISTRY.md`](../skills/AGENTS_KEPLR_CW20_REGISTRY.md). Verify the local snapshot: `make verify-issue-629`.

Do **not** treat `/cg/*` or `/cmc/*` as Keplr wallet metadata. Related price-path form is [#644](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/644) (child of parent listing catalog [#639](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/639)); DeFiLlama TVL/volume is a separate surface ([#631](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/631)).

## Listing venue catalog (GitLab #639) {#listing-venue-catalog-gitlab-639}

Places that can list CL8Y DEX stats, tokens, and wallet recognition (Cosmostation, Hexxagon, cosmos/chain-registry, CoinGecko/CMC forms, GeckoTerminal). The **#639 catalog** does not add indexer endpoints. Exchange forms point at `https://indexer.dex.cl8y.com/cg/` and `/cmc/`, not CoinGecko Pro v3. GeckoTerminal Non-EVM adapters are the **#646** child: `GET /gt/latest-block`, `/gt/asset`, `/gt/pair`, `/gt/events` (`make verify-issue-646`). Event `reserves` are post-event AMM state (**#684**, `make verify-issue-684`), not the live `pair_reserves` snapshot.
...
Cumulative values are **`Uint256` decimal strings** ([#1224](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1224) / [#1322](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1322), **O1322**). Parse them as arbitrary-precision integers. A sum past `2^128` is the full integer, not a wrap and not an error. A stored value that fits in `u128` is the same digit string as before. The pair cw2 1.18.0 fix is not live on affected Columbus-5 pairs until the migration in [#1324](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1324); preflight the missing-key backfill in [#1232](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1232) before broadcast.
