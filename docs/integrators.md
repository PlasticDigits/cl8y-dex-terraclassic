...
**Errors:** unknown pair → **404**; LCD failure → **502**; invalid cursor / side mismatch → **400**. Sustained abuse → **429** (global **`RATE_LIMIT_RPS`**, default **60**; LCD-heavy routes also **`RATE_LIMIT_LCD_HEAVY_RPS`**, default **10** — [`indexer-invariants.md`](./indexer-invariants.md), [`skills/AGENTS_INDEXER_API_LCD_SECURITY.md`](../skills/AGENTS_INDEXER_API_LCD_SECURITY.md)). **429** response: HTTP **429** with plain-text body and optional `Retry-After` / `x-ratelimit-*` headers from tower-governor.
