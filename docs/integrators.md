...
**With `belief_price`:** Expected output is `offer_amount / belief_price` (in ask units). Actual output used in the inequality is `book_return_net + pool_net_to_receiver + pool_commission` (pool commission to treasury counts on the “actual” side). The material-pool floor ([#307](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/307)) is skipped when `belief_price` is set.
