...
**Greedy book-first ([GitLab #708](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/708), invariants G1–G14):** opt-in `greedy: GreedySwapParams { max_maker_fills, book_start_hint }` on pair `Swap` and router `TerraSwap`. Do **not** send Pattern C `pool_input=0, book_input=offer` for this path (**G11**). `hybrid: null` stays pool-only (**G1** / **G12**). Quote with pair `HybridSimulation` `{ hybrid: null, greedy }` (**G7**). Execute needs `belief_price` or `min_return` (**G8**). `max_maker_fills` of `0` rejects; values above 100 clamp (**G5**). Official UI still uses `GET /route/solve` (**G4**). Playbook: [`skills/AGENTS_GREEDY_BOOK_FIRST.md`](../skills/AGENTS_GREEDY_BOOK_FIRST.md).
