...
| Limit placements / cancellations | `GET .../limit-placements` omits `(pair_id, order_id)` present in `limit_order_cancellations`; `.../limit-cancellations` lists indexed cancels | Unknown pair → **404** | [`api_pairs.rs`](../indexer/tests/api_pairs.rs); [GitLab **#135**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/135) |
