...
| Route discovery | `GET /api/v1/route/solve` — BFS over indexed pairs (**default max 3 hops**; **`pool_only=true`** legacy escape hatch → **max 4 hops**); `token_in` / `token_out` must match `assets.contract_address` (native-only assets without a contract address are not routable) | Unknown token → **400**; no path → **404** | [`route_solver.rs`](../indexer/src/api/route_solver.rs), [`api_route_solve.rs`](../indexer/tests/api_route_solve.rs); GitLab [**#191**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/191) |
| Route GET hybrid (default) | When `amount_in` is set and `pool_only` is not true, GET runs **hybrid optimization by default** (sequential per-hop split search via pair `HybridSimulation`, grid on `book_input`), merges best `hybrid` into `router_operations`, then optional router `simulate_swap_operations` (**max 3 hops**). Response adds `intermediate_tokens`, `quote_kind`, `hybrid_notes`. Short-TTL in-memory cache keys `(token_in, token_out, amount_bucket, max_maker_fills)`. Legacy **`pool_only=true`** (or `hybrid_optimize=false`) skips optimization and returns pool-only ops (**max 4 hops**). Deprecated **`hybrid_optimize=true`** without `amount_in` → **400**. **3-hop path:** `route_solve_get_hybrid_optimize_three_hops`, default GET: `route_solve_get_default_hybrid_two_hops` ([GitLab **#192**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/192)). | Zero `amount_in` with hybrid path → **400**; LCD failure during optimization → **502**; router sim error → **400** (same as POST) | `api_route_solve.rs`, [`hybrid_route_opt.rs`](../indexer/src/api/hybrid_route_opt.rs) |
| Route GET best execution | `GET /api/v1/route/solve/best` — **alias** for default hybrid GET when `amount_in` is set (same engine, max **3 hops**). Retail / Vyntrex integrator path ([GitLab **#189**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/189)). | Missing `amount_in` → **400**; same LCD/degraded semantics as default hybrid GET | `api_route_solve.rs`, [`integrators.md`](./integrators.md) |
