...
| Route GET global best execution | When `amount_in` is set and `pool_only` is not true, GET runs **global best execution** ([GitLab **#209**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/209), [ADR 0002](./adr/0002-global-best-execution-route-solver.md)): up to **5** simple paths by hop count, **joint** per-hop hybrid grid (17 `book_input` fractions + 2 coordinate passes), pick max `estimated_amount_out` via router `simulate_swap_operations` when configured (**max 3 hops**). Response adds `solver_version`, `paths_considered`, `optimality_scope`, `lcd_hybrid_queries`, `intermediate_tokens`, `quote_kind`, `hybrid_notes`. Cache key includes `solver_version`, `(token_in, token_out, amount_bucket, max_maker_fills)`. Legacy **`pool_only=true`** skips optimization (**max 4 hops**). Deprecated **`hybrid_optimize=true`** without `amount_in` → **400**. | Zero `amount_in` on `/best` → **400**; LCD failure → **502**; router sim error → **400** | `api_route_solve.rs`, [`best_execution.rs`](../indexer/src/api/best_execution.rs), [`route_paths.rs`](../indexer/src/api/route_paths.rs), [`hybrid_route_opt.rs`](../indexer/src/api/hybrid_route_opt.rs), [`skills/AGENTS_INDEXER_HYBRID_BEST_EXECUTION.md`](../skills/AGENTS_INDEXER_HYBRID_BEST_EXECUTION.md) |
...
| Route hybrid merge + simulation | `POST /api/v1/route/solve` with JSON `{ token_in, token_out, amount_in?, hybrid_by_hop? }`. When `hybrid_by_hop` is set, its length **must equal** the number of hops; each entry is `null` (pool-only hop) or a `HybridSwapParams`-shaped object (`pool_input`, `book_input`, `max_maker_fills`, `book_start_hint`). Response `router_operations` reflects merged `hybrid` fields. Optional `estimated_amount_out` uses LCD `simulate_swap_operations` on the router when `amount_in` and `ROUTER_ADDRESS` are set (router validates leg sums). **3-hop POST** with hybrid on multiple legs: `route_solve_post_three_hop_multi_leg_hybrid` ([GitLab **#192**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/192)). | Bad hybrid length → **400**; router/LCD query error → **400** with generic message (no raw LCD stack) | `api_route_solve.rs` |
...
| Wasm attributes | `wasm_attr_last`: duplicate keys → last wins (swap / pool attrs). **Limit lifecycle** (`limit_order_expired_parked`, `claim_expired_limit_order`): segment scan per `action` occurrence — see row above (#141) | Matches CosmWasm + flattened LCD wasm streams | [`parser.rs`](../indexer/src/indexer/parser.rs) unit tests + fuzz |
| Hybrid swap columns | Parser maps optional wasm attrs `pool_return_amount`, `book_return_amount`, `limit_book_offer_consumed`, `effective_fee_bps` onto `swap_events` (hybrid txs). API/CSV expose the same fields on trade rows. | Missing attrs → NULL columns (pool-only swaps) | [`swap_events_hybrid_columns.rs`](../indexer/tests/swap_events_hybrid_columns.rs); parser unit tests; GitLab [**#82**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/82), epic [**#199**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/199); [`skills/AGENTS_TESTING_P2_EPIC.md`](../skills/AGENTS_TESTING_P2_EPIC.md) |
