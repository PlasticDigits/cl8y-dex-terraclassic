...
Current columbus-5 controls are contract-specific: factory/router/pairs/fee-discount and wrap-mapper/CMM treasury use the DEX 2-of-3; ust1-window/oracle and listed wrap product CW20 controls have EOA residuals recorded in the [REGISTRY governance split](../deployments/mainnet-ust1-wrap/REGISTRY.md#governance-split-ops-critical). Track transfers on [#526](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/526) and the broader attestation on [#697](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/697). The DEX governance policy target remains multisig or DAO control.

**Production multisig (governance, wasm admin, upgrades):** [`docs/reference/governance-multisig.md`](reference/governance-multisig.md) — `terra1zlmv2xydxcusurtr6rl78wsvytdc6mfex6hep7`.

Operator checklist (governance, treasury, hooks, router trust, pool-only verification): [`docs/runbooks/launch-checklist.md`](runbooks/launch-checklist.md). **Production mainnet** requires **Phase 5 go/no-go sign-off** on the launch tracking issue ([#391](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/391)); see [`skills/AGENTS_LAUNCH_GO_NO_GO.md`](../skills/AGENTS_LAUNCH_GO_NO_GO.md). **Deploy trace** (git SHA, chain version, code IDs, wasm-checksums.txt at deploy time): [`docs/templates/deploy-trace.md`](templates/deploy-trace.md) ([#410](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/410)). **Emergency controls rehearsal** from the planned governance multisig (pause, blacklist, unpause, unblacklist on testnet/staging): [`docs/runbooks/governance-emergency-rehearsal.md`](runbooks/governance-emergency-rehearsal.md) (**SEC-B09**, [#397](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/397)). **Admin-key custody** — multisig type/threshold, named signer roster, backup signer + escalation, key-rotation triggers and process: [`docs/runbooks/key-custody.md`](runbooks/key-custody.md) (**SEC-B10**, [#398](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/398)).
...
|---------|------------------|---------------------|------------|
| **Indexer** (`VITE_INDEXER_URL`) | Route solve (`router_operations`), token metadata (`logo_url`), charts, oracle display | Malicious but **valid** pool routes, inflated quotes, phishing logos; **display/submit path mismatch** on multihop ([**#450**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/450) / SEC-I02 H09) or direct hybrid receive ([**#471**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/471)) | Pin **HTTPS-only** indexer URL in deploy; TLS cert from your CA; human-readable **Route** row at swap confirmation (`swap-route-summary`); cross-check `intermediate_tokens` vs `router_operations` before submit — reconcile display to ops path and warn user (`swap-route-intermediate-reconciled`, [`reconcileSwapRouteIntermediateTokens`](../frontend-dapp/src/utils/swapRouteDisplay.ts)); **direct hybrid:** reconcile receive + slippage to wallet `hybrid_simulation` in [`quoteDirectHybridSwap`](../frontend-dapp/src/utils/directHybridQuote.ts) (`swap-direct-hybrid-amount-reconciled`); optional factory LCD `getPair()` preflight on hops ([`swapRoutePreflight.ts`](../frontend-dapp/src/services/terraclassic/swapRoutePreflight.ts)). **No** client-side BFS fallback vs indexer (RPC rate limits). |
| **Build env** | `VITE_*` addresses, `VITE_WC_PROJECT_ID`, dev mnemonic, `VITE_DEV_MODE` | Wrong contracts inlined; shared WC project; seed in bundle; Simulated Wallet chrome on `dex.cl8y.com` | `vite build` guards: reject `VITE_DEV_MNEMONIC` outside `development` / `VITE_ALLOW_DEV_MNEMONIC=local-only`; require `VITE_WC_PROJECT_ID` in production; reject `VITE_DEV_MODE=true` for `mode === 'production'` ([#695](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/695) / FE-01). See [`docs/frontend.md`](./frontend.md#simulated-dev-wallet-and-vite_dev_mnemonic). |
...
| **Open Graph** | Static `index.html` + `/og-image.png` | Attacker-controlled preview image / origin spoof in shares | Bake allowlisted `https://dex.cl8y.com` only ([`viteOg.ts`](../frontend-dapp/viteOg.ts)); never copy the request host header into `og:image`. [#578](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/578). |
...
4. Leave `VITE_DEV_MODE` **unset** on Coolify production (`dex.cl8y.com`). A `true` value fails `vite build --mode production` ([#695](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/695)).
5. Production CSP `connect-src` lists env LCD/RPC/indexer hosts + WalletConnect relay — not blanket `https:` ([`frontend-dapp/viteCsp.ts`](../frontend-dapp/viteCsp.ts)).
...
All swap commissions are sent directly to each pair’s `FEE_CONFIG.treasury` (copied from factory `config.treasury` at `CreatePair`). The Pair contract holds no fees — they are transferred atomically during each swap. Factory `UpdateConfig { treasury }` does **not** move live pair fees; governance must then run `SetPairTreasuryAll` / `SetPairTreasuryBatch`. Production sink is the ustr-cmm CMM treasury (`terra16j5u6…`), not the DEX governance multisig.
...
- Factory treasury (new pairs + pair-creation LUNC) is updated via `UpdateConfig` (governance only).
- Live pair commission recipients are updated via `SetPairTreasury` / `SetPairTreasuryAll` / `SetPairTreasuryBatch` (governance only; requires pair `UpdateTreasury`).
...
**Post-listing pin + re-check (GitLab [#582](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/582) / [#1234](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1234), invariant F6):** Terra Classic `MsgMigrateContract` can move a listed **instance** onto any stored wasm after `CreatePair`. Pair instantiate snapshots each asset's live `code_id`. Swap / provide / withdraw / limit place+fill / cancel / claim / **`UpdateLimitOrderPrice`** / **`CleanLimitBook`** abort unless live `ContractInfo.code_id` **equals that pin** **and** factory `IsCodeIdWhitelisted` is still true. Freeze means **no DLL writes** (keepers resume `CleanLimitBook` after unfreeze). Governance `RemoveWhitelistedCodeId` therefore freezes every pair whose pin still matches a buggy template. Honest upgrades (e.g. 10184 → a new audited id) freeze those pairs until `RefreshPairAssetCodeIds` / `RefreshPairAssetCodeIdsBatch`. Factory query errors fail closed. Do **not** add fee-on-transfer balance-delta math. Ops: [`docs/runbooks/cw20-whitelist-policy.md`](runbooks/cw20-whitelist-policy.md); playbook [`skills/AGENTS_CW20_CODE_ID_PIN.md`](../skills/AGENTS_CW20_CODE_ID_PIN.md). dApp + indexer visibility ([#585](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/585)): `route/solve` excludes frozen hops and pair APIs flag `code_id_frozen`; LCD quotes stay ungated. Playbook [`skills/AGENTS_FRONTEND_CODE_ID_FREEZE.md`](../skills/AGENTS_FRONTEND_CODE_ID_FREEZE.md).

**Rationale:** this prevents pairs from being created with malicious CW20 contracts that could manipulate balances, re-enter, or steal funds — and keeps that gate live after listing.

**Balance-stability requirement (GitLab [#377](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/377), [#448](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/448), [#589](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/589)):** do **not** whitelist CW20 templates whose recipient balance can diverge from the declared `transfer` / `send` amount. This covers both **fee-on-transfer / transfer-tax** (debits less than declared) and **rebase / elastic-supply / reflection** (balance changes after receipt, with no transfer). Pair reserves and limit-order escrow credit declared amounts, not balance deltas, so either mechanic desyncs accounting (invariant **P2**) and can break withdrawals and escrow refunds. Intake is the **code-ID audit harness**: LCD wasm pin (`data_hash`), decompile, [`CATALOG.md`](../cw20-codeid-audits/CATALOG.md) audit, Layer A+B suite — [`cw20-codeid-audits/PROCEDURE.md`](../cw20-codeid-audits/PROCEDURE.md), `make verify-issue-589`. A byte-identical optimizer rebuild is **not** required. Do not whitelist on checksum match or CertiK file hashes alone. Ops runbook: [`docs/runbooks/cw20-whitelist-policy.md`](runbooks/cw20-whitelist-policy.md). LCD `CodeInfo` helper: [`scripts/verify-cw20-code-ids.sh`](../scripts/verify-cw20-code-ids.sh) (does not replace the harness).
...
**LP ticker (GitLab [#518](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/518)):** pair-created LP CW20 symbols keep digits and strip non-alphanumeric prefix chars (`[a-zA-Z0-9\-]{3,12}`). Classic columbus-5 LP code rejects digits — factory `lp_token_code_id` must be digit-allowing `cw20-mintable` after the #518 upgrade. See invariant **F3** and [`docs/contracts-terraclassic.md` § CreatePair LP ticker](./contracts-terraclassic.md#createpair-lp-ticker-gitlab-518).

See also: GitLab [#121](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/121), [#276](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/276), [#518](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/518), [#582](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/582), [`docs/contracts-security-audit.md`](./contracts-security-audit.md) (invariants **F1**, **F2**, **F3**, **F6**), and factory `lib.rs` / `state.rs` in `smartcontracts/contracts/factory/`.
...
Governance on the **factory** can **pause** individual pairs (`SetPairPaused`) or **blacklist** wallets, tokens, or pairs for incident response ([ADR 0003](adr/0003-governance-trading-blacklist.md)). Copy-pastable `terrad` recipes with confirmation queries: **[`docs/runbooks/emergency-commands.md`](runbooks/emergency-commands.md)** ([SEC-B11](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/399), GitLab [#399](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/399)). **Pool triage (SEC-G03, [#436](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/436)):** rank pools by liquidity before choosing `$PAIR_ADDR` — [Quick pool triage](runbooks/emergency-commands.md#quick-pool-triage-sec-g03). **Unpause** requires the prerequisite checklist in section 2 before broadcasting restore (**SEC-G07**, [#440](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/440)). LocalTerra rehearsal: `make verify-issue-399`. Agent playbooks: [`skills/AGENTS_EMERGENCY_COMMANDS.md`](../skills/AGENTS_EMERGENCY_COMMANDS.md), [`skills/AGENTS_POOL_TRIAGE.md`](../skills/AGENTS_POOL_TRIAGE.md).
...
| Dimension | Blocks |
|-----------|--------|
| Wallet | Swaps, hybrid swaps, provide/withdraw liquidity, limit place/cancel/claim/update, router multihop for that address; **resting limits are not filled** — parks into `EXPIRED_LIMIT_CLAIMS` when a taker walk reaches them ([#468](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/468)) |
...
**Operator decision criteria (SEC-B12, GitLab [#400](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/400)):** when to blacklist wallet, token, or pair during incidents; ToS/compliance escalation; false-positive rollback with evidence preservation — [blacklist-decision runbook](runbooks/blacklist-decision.md). Agent playbook: [`skills/AGENTS_BLACKLIST_DECISION.md`](../skills/AGENTS_BLACKLIST_DECISION.md).

**Rollback vs forward-fix (SEC-H09, GitLab [#445](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/445)):** when to rollback vs hotfix for frontend-only, indexer, contract, and chain-dependency incidents — [rollback-decision runbook](runbooks/rollback-decision.md). Distinct from trading blacklist rollback; covers deploy surfaces and governance migrate paths. Agent playbook: [`skills/AGENTS_ROLLBACK_DECISION.md`](../skills/AGENTS_ROLLBACK_DECISION.md).
...
**Guard fail-closed (SEC-I03 F02, GitLab [#456](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/456)):** pair and router blacklist guards **reject** user executes when the factory `BlacklistCheck` query errors (`Blacklist guard unavailable`). There is no silent fail-open on factory query failure — a stale factory pointer or broken factory disables trading rather than bypassing blacklist enforcement.

**Frontend regression tests (SEC-A02 / SEC-E01, GitLab [#388](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/388), [#425](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/425)):** Vitest mocks `useTradingBlacklist` on Swap, Trade order ticket, **Pool** (provide + withdraw), and **`/limits`** (place + cancel) and asserts `describeTradingBlacklistBlock` copy in the alert plus disabled CTAs for wallet, token, and pair dimensions. Copy source: [`blacklist.ts`](../frontend-dapp/src/services/terraclassic/blacklist.ts); shared mocks: [`tradingBlacklistMocks.ts`](../frontend-dapp/src/test/tradingBlacklistMocks.ts).

**Contract regression tests (SEC-B02, GitLab [#393](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/393)):** `blacklist_tests::wallet_blacklist_blocks_swap_lp_limits_and_unban_restores` asserts wallet blacklist rejection on swap, **hybrid swap**, provide/withdraw liquidity, limit place/cancel/**claim**/**update**, then unblacklist restores swap; `router_multihop_rejects_blacklisted_wallet` covers router multihop. **Maker fill bypass ([#468](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/468)):** `blacklist_tests::blacklisted_maker_resting_limit_not_filled_taker_can_still_swap` asserts a blacklisted maker's resting bid is not filled and parks for post-unblacklist claim. Token/pair dimensions: `token_blacklist_blocks_swap_both_directions`, `pair_blacklist_blocks_swap_and_lp`. **SEC-I03 F02 ([#456](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/456)):** `blacklist_tests::factory_blacklist_query_error_blocks_swap` asserts swap is rejected when the pair's stored factory cannot answer `BlacklistCheck`. Matrix row **B1** in [contracts-security-audit.md](./contracts-security-audit.md).
...
| Simulated Wallet chrome (FE-01) | Production `vite build` fails when `VITE_DEV_MODE=true`. Staging / development may keep the flag. `VITE_ALLOW_DEV_MNEMONIC=local-only` does not bypass ([#695](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/695)). |
