...
**Balance-stability requirement (GitLab [#377](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/377), [#448](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/448)):** do **not** whitelist CW20 templates whose recipient balance can diverge from the declared `transfer` / `send` amount. This covers both **fee-on-transfer / transfer-tax** (debits less than declared) and **rebase / elastic-supply / reflection** (balance changes after receipt, with no transfer). Pair reserves and limit-order escrow credit declared amounts, not balance deltas, so either mechanic desyncs accounting (invariant **P2**) and can break withdrawals and escrow refunds. Code IDs must be **source-reviewed** for these mechanics and the audit evidence recorded before `AddWhitelistedCodeId` — not whitelisted on checksum match alone. Ops runbook: [`docs/runbooks/cw20-whitelist-policy.md`](runbooks/cw20-whitelist-policy.md). Verify production GDEX/TerraPort code IDs via [`scripts/verify-cw20-code-ids.sh`](../scripts/verify-cw20-code-ids.sh).
