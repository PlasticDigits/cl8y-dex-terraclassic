...
3. **Auto-deploy (documented intent, operator leftover).** The indexer Coolify app for this Forgejo path should follow protected `main` like the frontend app. CAC grouped drain maps **one** UUID per `owner/repo` and is **not** the indexer redeploy path. Dual-app skew (Vite vs Rust rebuild) is expected. Schema-on-boot stays `sqlx::migrate!()` (no `set_ignore_missing` in production). Coolify-era indexer rollback is **three-way** (canonical: [`rollback-decision.md`](../runbooks/rollback-decision.md) § Auto-deploy era): **unchanged vs prior Coolify Deploys SHA** (compare `_sqlx_migrations` `version` / `success` / `installed_on` to that SHA’s latest `indexer/migrations/*.sql`; still no SOURCE SHA log scrape) → restore prior image; ahead + no `down.sql` → forward-fix; ahead + documented revert → auto-deploy **off** (#297) → snapshot → apply **that** `down.sql` → `DELETE FROM _sqlx_migrations WHERE version = <that version>` (only that row; revert files do **not** touch the ledger) → restore image → attest. Dirty `success = false` → repair ledger, then restore or forward-fix. A transactional failure of the **first** new migration that left **no** row is unchanged. Success of `N` then failure of `N+1` is ahead.
...
| **Three-way Coolify rollback** | Inspect production `_sqlx_migrations` (`version`, `success`, `installed_on`) via Coolify DB shell / indexer app `DATABASE_URL` (not `scripts/lib/postgres-psql.sh`). **Baseline:** prior successful Coolify Deploys SHA (allowed) → that SHA’s latest `indexer/migrations/*.sql`. Still no SOURCE SHA log scrape. **Unchanged** (max successful `version` equals that baseline; a transactional failure of the **first** new migration that left **no** row) → restore the prior Coolify indexer image. **Dirty** `success = false` → repair that ledger row, then restore or forward-fix (restore of the prior image **fails** while a dirty row remains). **Ahead + no** paired `down.sql` → snapshot; ship a hotfix image (do not restore the previous binary). Success of `N` then failure of `N+1` is ahead (`N` applied). **Ahead + documented revert** under [`indexer/migrations/revert/`](../../indexer/migrations/revert/) → auto-deploy **off** first (#297) → snapshot → apply **that** `down.sql` → `DELETE FROM _sqlx_migrations WHERE version = <that version>` (**only that row**) → restore the prior Coolify image → attest. Leave auto-deploy **off** until a good tip is on `main` (revert commit or hotfix). Revert files undo schema only; they do **not** `DELETE` `_sqlx_migrations`. Production `sqlx::migrate!()` does **not** call `set_ignore_missing` and **rejects applied versions missing from the binary**. Do not invent a revert file. Do not `set_ignore_missing(true)` in production. Local/systemd still uses host `psql` / `postgres-psql.sh` and the same revert + ledger-delete steps. Image binary is `cl8y-dex-indexer`. |
...
| **H1276-7** | While auto-deploy is on: expand-only migrations; dual-app skew is expected. Coolify-era rollback is **three-way** (unchanged vs prior Coolify Deploys SHA’s latest `*.sql` → restore; ahead + no `down.sql` → forward-fix; ahead + documented revert → auto-deploy off + snapshot + that `down.sql` + `DELETE` that `_sqlx_migrations` row + restore). Dirty `success = false` → repair ledger first. Inspect prod via Coolify DB / indexer `DATABASE_URL`, not `postgres-psql.sh`. Revert files do not touch the ledger. No production `set_ignore_missing`. |
...
| Two-way Coolify mermaid / policy table (`ahead` always forward-fix) **or** Coolify never applies `down.sql` while Decision still lists revert | Leaves on-call to invent the third branch. **Three-way** is the Coolify-era rule (unchanged vs prior Coolify Deploys SHA’s latest `*.sql` → restore; ahead + no `down.sql` → forward-fix; ahead + documented revert → auto-deploy off + snapshot + that `down.sql` + `DELETE` that ledger row + restore). Reject “never apply `down.sql` on Coolify.” Inspect prod via Coolify DB / indexer `DATABASE_URL`, not `postgres-psql.sh`. |
| `down.sql` then restore **without** `DELETE` that `_sqlx_migrations` row | Revert files do not touch the ledger. Default `sqlx::migrate!()` rejects applied versions missing from the binary — prior image exits; a later auto-deploy of a binary that still contains `N` **skips** `N` while the schema is reverted. Rejected. |
...
| `SELECT version … LIMIT 5` as “unchanged” / “no new row → restore” | No baseline. First-new-migration transactional failure leaves no row (restore). `N` then failed `N+1` leaves `N` applied (ahead). Dirty `success = false` still fails prior-image boot. Compare `version` / `success` / `installed_on` to the prior Coolify Deploys SHA’s latest `*.sql`. |
| 2(c) with auto-deploy still on | Restoring an old image while `main` is the bad SHA retriggers that tip and re-applies `N`. 2(c) starts with auto-deploy **off**; leave it off until a good tip is on `main`. |
...
No sqlx migration for this ticket. `sqlx::migrate!()` on boot is unchanged (no `set_ignore_missing` in production) and is why auto-deploy needs the expand-only / three-way rollback rule (including ledger `DELETE` after a documented `down.sql`).
...
| Auto-deploy land with new expand-only migration | Boot migrate succeeds; previous image **cannot** start on the new schema. Coolify-era: no `down.sql` → forward-fix; documented revert → auto-deploy off + snapshot + that `down.sql` + `DELETE` that ledger row + restore. |
| `down.sql` then restore without ledger `DELETE` | Prior image exits: applied version `N` is missing from the binary. A later auto-deploy of a binary that still contains `N` **skips** `N` while schema is reverted. Always `DELETE` that row. |
| Dirty `success = false` treated as unchanged | Restore of the prior image still fails migrate validation. Repair the ledger, then restore or forward-fix. |
| 2(c) with auto-deploy still on | Protected-branch rebuild of the bad `main` tip re-applies `N`. Turn the flag off first; leave it off until a good tip is on `main`. |
...
- **Indexer binary after auto-deploy:** see Decision **Three-way Coolify rollback** and [`rollback-decision.md`](../runbooks/rollback-decision.md) § Auto-deploy era Coolify incident checklist. Unchanged vs prior Coolify Deploys SHA’s latest `*.sql` → restore prior Coolify image. Ahead + no `down.sql` → forward-fix. Ahead + documented revert → auto-deploy off + snapshot + that `down.sql` + `DELETE` that `_sqlx_migrations` row + restore. Dirty `success = false` → repair ledger first. Inspect prod via Coolify DB / indexer `DATABASE_URL`, not `postgres-psql.sh`. Image binary is `cl8y-dex-indexer`. Attest rollback with `/health` `git_sha` + `EXPECT_SHA`, not Coolify log scrape.
