...
| Dirty `DELETE` then “forward-fix only” / dirty as a terminal mermaid branch | After `DELETE … success = false`, **re-enter the three-way** tree. No successful row newer than baseline → **2(a)**. A successful newer row remains → **2(b) / 2(c)** with the same every-version gate. Do not treat remaining successful `N` as forward-fix only. A failed first new migrate with **no** row stays **unchanged**, not dirty. |
...
| Dirty `success = false` treated as unchanged **or** `UPDATE success = true` | Restore of the prior image still fails migrate validation, or a half-applied version is skipped. Repair: `DELETE FROM _sqlx_migrations WHERE version = <v> AND success = false` only. Never `UPDATE success`. Then **re-enter the three-way** tree (not “forward-fix only”). A failed first new migrate with **no** row is unchanged, not dirty. |
