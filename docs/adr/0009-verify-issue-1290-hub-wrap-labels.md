...
1. **Bundle.** `make verify-issue-1290` is the leftover regression entry for hub wrap **cLUNC** vs CEX **LUNC** after [!1290](https://git.cl8y.com/code/cl8y-dex-terraclassic/pulls/1290) merged the #1240 naming follow-up. Makefile aliases: **`verify-issue-1302`** (issue AC **B1290-1**) and **`verify-issue-1306`** (this leftover iid). The script always runs child `make verify-issue-1240` (**P1240-1–P1240-8**), then `hubPriceTicker` Vitest, then optional Playwright `e2e/protocol-page.spec.ts` at **5 workers**, then the full **B1290-4** greps (eight docs **plus** Makefile `verify-issue-1302` / `verify-issue-1306` so dropping an alias fails the step).
...
4. **Leftover-complete is this #1306 wiring MR.** Alias `verify-issue-1302` **and** `verify-issue-1306`, Cloud Agent + Frontend `help` for **1290 / 1302 / 1306**, `free_tcp_port` 30129, full **B1290-4** (eight docs **plus** Makefile alias greps), leftover **#1306** naming, slice-0 docs (this ADR **0009**, architecture `#dex-hub-wrap-labels`, **Q22**, README index), and **policy-clean** commit bodies on the merge range. Green `VERIFY_ISSUE_1290_SKIP_E2E=1` is a required implement check, **not** leftover-complete and **not** a land gate on a VM without nvm. Before merge, a **provisioned** Cloud Agent or CI (Node 24 via `scripts/with-node.sh` / `NVM_DIR`, `frontend-dapp/node_modules`) must be green on **both** skip-E2E **and** full `make verify-issue-1306` (Playwright `protocol-page`, **5 workers**). Production `/protocol` visual (hub wrap vs CEX native; selected **vFDUSD** tab not **VFDUSD**) is [#1305](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1305) leftover 1 (related #1290 AC), **not** this ticket and **not** #297. Slice 3 is not the #1306 close gate.
...
- Open [!1306](https://git.cl8y.com/code/cl8y-dex-terraclassic/pulls/1306) (`origin/issue/1302` @ `2d4e8b64`, tree after `50eb149f`) already has leftover **#1306** naming, Status **Proposed ([#1306])**, aliases `verify-issue-1302` / `verify-issue-1306`, `free_tcp_port` 30129, and eight-file B1290-4 greps. That tip is **still not landable**: (1) commit body contains a `Co-authored-by` agent-mailbox trailer (`.githooks/commit-msg` / `pre-push`); (2) B1290-4 does **not** grep Makefile for `verify-issue-1306` / `verify-issue-1302`, so dropping an alias can still PASS; (3) gate VMs without `NVM_DIR` / nvm cannot run child 1240 or `hubPriceTicker` via `with-node.sh` — that is an **environment gap**, not a script defect, and skip-E2E there is **not** a land gate. Recreate the tree with a policy-clean message, add the Makefile greps, and land only after Node-capable skip-E2E **and** full Playwright. Do **not** merge `2d4e8b64` as-is. Do **not** retarget closed !1302.
...
- Retargeting closed !1302. Merging open !1306 as-is without the #1306 retarget, Proposed status, policy-clean commit bodies, Makefile alias greps, and Node-capable full Playwright.
- Treating a gate VM without nvm (`NVM_DIR` missing / `with-node.sh` cannot provision Node 24) as leftover-complete or as a reason to skip Vitest / child 1240. Skip-E2E covers Playwright only.
...
2. Reconstruct `scripts/qa/verify-issue-1290.sh` (Playwright isolation + full B1290-4 greps including Makefile aliases + leftover **#1306** naming). Copy **only** `free_tcp_port` from [`scripts/qa/verify-issue-703.sh`](../../scripts/qa/verify-issue-703.sh) — **not** 703’s `ln -sfn` `frontend-dapp/node_modules` bootstrap. Add Makefile aliases `verify-issue-1302: verify-issue-1290` and `verify-issue-1306: verify-issue-1290`. Add **1290, 1302, and 1306** to Cloud Agent and Frontend `help` (immediately after `verify-issue-1240`). On the published `2d4e8b64` tree those aliases already exist — keep them and **grep** them (B1290-4). Recreate that tree with a policy-clean commit body before merge.
...
| 4 | Grep `verify-issue-1290` in the **eight docs**: `docs/testing.md`, `AGENTS.md`, `skills/AGENTS_FRONTEND_PROTOCOL_STATS.md`, `skills/AGENTS_FRONTEND_PROTOCOL_HUB.md`, `docs/frontend.md`, this ADR (`docs/adr/0009-verify-issue-1290-hub-wrap-labels.md`), `docs/architecture.md` (`dex-hub-wrap-labels` or `verify-issue-1290`), and `docs/qa-invariants.md` **Q22**. **Also grep `Makefile`** for `verify-issue-1306` **and** `verify-issue-1302` (B1290-1 alias coverage). Weaker sketch greps (testing + AGENTS + PROTOCOL_STATS only, or eight docs without Makefile) are **not** B1290-4. |
...
**B1290-4 grep step (eight docs + Makefile aliases):**
...
run_step "docs: #1306 verify crosslink (B1290-4)" \
  ...
  grep -qE "verify-issue-1302" Makefile && \
  grep -qE "verify-issue-1306" Makefile'
```

**`with-node.sh` / nvm:** steps 1–2 (child 1240 Vitest paths + `hubPriceTicker`) and the default-on Playwright step all go through `scripts/with-node.sh` → `cloud_agent_ensure_node`. Missing `NVM_DIR` / `$HOME/.nvm/nvm.sh` is **FAIL**, not skip. `VERIFY_ISSUE_1290_SKIP_E2E=1` skips Playwright only. Land on a provisioned Cloud Agent (`scripts/setup-cloud-agent-toolchain.sh`) or CI with Node 24 — not on a gate VM that cannot bootstrap nvm.
...
| QA script | Reconstruct `scripts/qa/verify-issue-1290.sh` (already on `main` from closed !1302): `free_tcp_port 30129` (copied from 703, not 703’s `ln -sfn`), full B1290-4 greps **including Makefile aliases**, leftover **#1306** header. Recreate `2d4e8b64` with a policy-clean commit body. |
...
| **B1290-3** | Playwright protocol-page is default-on at **5 workers**, port **30129**, `PLAYWRIGHT_SKIP_CHAIN=1`. Copy `free_tcp_port` from `verify-issue-703.sh` (function only — not 703’s `ln -sfn` bootstrap) and free **30129** first. Port + `PLAYWRIGHT_BASE_URL` only in that step’s subshell. Skip only via `VERIFY_ISSUE_1290_SKIP_E2E=1` or missing Playwright package. Installed package + crash (missing Chromium / NSS) = FAIL. Missing nvm / `with-node.sh` bootstrap = FAIL (not skip). |
| **B1290-4** | Crosslinks: testing, AGENTS, PROTOCOL_STATS, PROTOCOL_HUB, frontend.md, this ADR (`docs/adr/0009-verify-issue-1290-hub-wrap-labels.md`), architecture `#dex-hub-wrap-labels`, **Q22**, **and `Makefile` `verify-issue-1302` plus `verify-issue-1306`**. Implement greps **all eight docs and both Makefile aliases**. Slice-0 files ride #1306 as **inserts** so those greps can pass. Docs-only greps without Makefile are **not** B1290-4 (alias drop would still PASS). |
...
| **B1290-8** | Green `SKIP_E2E=1` ≠ leftover-complete. Leftover-complete is the **#1306 wiring MR** on `main` (aliases, `free_tcp_port`, full B1290-4 including Makefile, leftover **#1306** naming, slice-0 ADR 0009 / **Q22** / architecture / README, policy-clean messages). Land requires a **provisioned** Cloud Agent or CI: green skip-E2E **and** full `make verify-issue-1306` (Playwright `protocol-page`, 5 workers). A gate VM without nvm is **not** that runner. Production `/protocol` visual is [#1305](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1305) leftover 1 (related #1290 AC), not this ticket. No HTML scrape. Not #297. |
...
| Merge open !1306 (`2d4e8b64`) as-is | Wiring tree is close, but commit body has a `Co-authored-by` agent-mailbox trailer (`.githooks/commit-msg`) and B1290-4 still omits Makefile alias greps. Recreate the tree; do not keep the trailer. |
| Eight-file B1290-4 without Makefile aliases | Dropping `verify-issue-1306:` still PASSes. Grep Makefile for both aliases (**B1290-4**). |
| Treat gate-VM nvm gap as leftover-complete | Child 1240 / Vitest FAIL when `with-node.sh` cannot load nvm. Land on provisioned Cloud Agent / CI. Skip-E2E does not skip Vitest. |
...
**Added:** one bash wrapper (reconstruct), Makefile aliases `1302` + `1306`, Cloud Agent + Frontend help for **1290 / 1302 / 1306**, full B1290-4 greps (docs + Makefile aliases), optional E2E skip flag, `free_tcp_port` on 30129, **Q22** / **B1290**, slice-0 docs on #1306 (ADR **0009**), policy-clean merge-range messages, Node-capable Playwright land gate.

**Removed:** the gap where post-merge hub **cLUNC** vs CEX **LUNC** had no agent `make` entry besides 1240’s CSS/docs subset; the closed-PR sketch’s weaker grep / missing alias / stale-Vite flake; the 0008/Q21 collision with #1300; leftover-complete split onto a Coolify glance that #1305 already owns; dual “leftover #1302” close-out after that sketch already merged; alias-drop drift (Makefile greps); landing a hook-forbidden trailer.
...
| B1290-4 docs greps PASS after dropping Makefile aliases | Forbidden alias drift (**B1290-1**). Grep `Makefile` for `verify-issue-1302` and `verify-issue-1306`. |
| `with-node.sh` cannot provision Node (`NVM_DIR` missing / no `$HOME/.nvm/nvm.sh`) | Child 1240 and `hubPriceTicker` FAIL. Not skip. Not a product defect. Run on provisioned Cloud Agent / CI. Do not copy 703 `ln -sfn` bootstrap. |
| Skip-E2E green on a no-nvm gate VM treated as land evidence | Forbidden (**B1290-8**). Land needs Node-capable skip-E2E **and** full Playwright. |
...
| Merge of open !1306 as-is (`50eb149f` or `2d4e8b64`) | `50eb149f` self-marks **Accepted (#1302)**; `2d4e8b64` has a hook-forbidden trailer and incomplete B1290-4. Amend / recreate first. |
...
|-------|-----|-------------|--------|
| **0 — this design** | design_author | This ADR **0009**, architecture `#dex-hub-wrap-labels`, **Q22**, `docs/README.md` index. Transported on `cac-design-issue-1306` only (no design-only PR). Builds on published `8945c85c`; after !1306 review: policy-clean merge-range bodies, Makefile alias greps in B1290-4, Node-capable skip-E2E **and** full Playwright land gates. | Slice 1 |
| **1 — script + Makefile** | implement | On **current `main`**: keep/reconstruct `scripts/qa/verify-issue-1290.sh` (`free_tcp_port` 30129 from 703 function only, subshell port env, full B1290-4 greps **including Makefile `verify-issue-1302` / `verify-issue-1306`**, leftover **#1306** header); aliases `verify-issue-1302` and `verify-issue-1306`; Cloud Agent + Frontend help for **1290 / 1302 / 1306**. **Insert** slice-0 sections/file (do not whole-file checkout architecture / qa-invariants / README from this tip). Continue open !1306; do not retarget closed !1302. Do not merge `f237a6e6`, `50eb149f`, or `2d4e8b64` as-is — recreate `2d4e8b64` with a policy-clean commit body. Do not touch `scripts/test-commit-msg-hook.sh`. Keep `git ls-files frontend-dapp/node_modules` empty. Land only after Node-capable skip-E2E **and** full Playwright. | Slice 2 |
| **2 — crosslinks** | implement | **Insert** slice-0 docs (ADR 0009 **Proposed [#1306]**, architecture `#dex-hub-wrap-labels`, **Q22**, `docs/README.md` index) **plus** leftover **#1306** wording in `AGENTS.md`, `docs/testing.md`, `docs/frontend.md`, PROTOCOL_STATS, PROTOCOL_HUB (**Q22** / **B1290-1–B1290-8**; related #1302 / !1290 / #1240). Optional comment on 1240 script / protocol-page spec. **No** product TSX, **no** dirty files from `f237a6e6`, **no** `Fixes #1290`, **no** `Closes #1302`. Merge-range bodies pass `.githooks/commit-msg`. | none — slices 1–2 together are leftover-complete after Node-capable full verify |
...
**Implement merge (slices 1–2) — leftover-complete.** On a **provisioned** Cloud Agent or CI (Node 24 via `scripts/with-node.sh`, `frontend-dapp/node_modules`, Playwright Chromium). A gate VM without `NVM_DIR` is **not** this runner.
...
- Bundle with skip: 1240 + hubPriceTicker + skipped E2E + **full B1290-4 greps** (eight docs **plus** Makefile `verify-issue-1302` / `verify-issue-1306`), exit 0.
...
1. Merge slices 1–2 to `main` via **#1306** (amended open PR or a new PR) **after** policy-clean messages and Node-capable skip-E2E **plus** full Playwright. That merge **is** leftover-complete for #1306. Frontend Coolify auto-deploy may rebuild; labels are already on `main` from !1290 — this merge is verify-only + slice-0 docs.
...
**#1306 leftover-complete is this wiring MR.** Slices 1–2 on `main` are that MR. Green `SKIP_E2E=1` alone does **not** close leftover (the MR also needs aliases, help, `free_tcp_port`, full B1290-4 including Makefile, leftover **#1306** naming, slice-0 docs, policy-clean messages). Slice 3 is not this ticket’s close gate.
...
- `VERIFY_ISSUE_1290_SKIP_E2E=1 make verify-issue-1290` **and** `VERIFY_ISSUE_1290_SKIP_E2E=1 make verify-issue-1306` exit 0 on a **Node-capable** worktree from `main` + that MR, including full B1290-4 greps (eight docs **plus** Makefile aliases).
- Full `make verify-issue-1306` (Playwright `protocol-page`, 5 workers) is green on provisioned Cloud Agent or CI. A gate VM without nvm is not that evidence.
- Aliases `make verify-issue-1302` and `make verify-issue-1306` are the same target; B1290-4 greps both in `Makefile`.
