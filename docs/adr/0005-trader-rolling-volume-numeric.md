...
3. Keep rolling refresh **UPDATE-only** plus the idle zero `UPDATE` (**D2**). In `indexer/src/indexer/poller.rs`, call `heal_trader_lifetime_from_swaps` **before** `refresh_all_volume_windows(..., true)` (the `#1269` slot). Do not copy the `#676` slot after refresh. Poller heal is **mismatch-gated** with the same `COALESCE` idea as `positions_trade_count_diverges` — missing sender **or** `total_trades` / capped `total_volume` diverge **or** leftover-lifetime with no remaining swaps. Registered / zero-lifetime ghosts (`COALESCE` both sides 0) do **not** trip. Not an unconditional `GROUP BY sender` rewrite, and not “`traders` is small”. The scan is `swap_events` (`idx_swaps_sender`). Timestamp rewrite from `min/max(block_timestamp)` is scoped to missing/mismatched rows (`upsert_trader` ingest uses `NOW()`); leftover-zero pins leftover timestamps to `NULL`. **Do not** zero idle traders who still have old `swap_events` (**D2**). `heal_trader_lifetime_from_swaps` runs INSERT + lifetime UPDATE + leftover-zero + priced USD + leftover-USD in **one transaction**. Crash rolls back; gate still fires next boot. Poller does not call pool-level `refresh_trader_total_volume_usd` after that commit.
...
| **V1277-3** | `total_volume_usd` stays `(38, 18)`. Heal INSERT / lifetime UPDATE / leftover-zero do not touch it. Idle **D2** does not touch lifetime columns. Unpriced stays NULL (**R5**). In the same heal transaction: execute `refresh_trader_total_volume_usd` SQL, then leftover-USD `NULL` when no remaining priced `swap_events`. Do not change P522-Q for senders who still have priced swaps. |
