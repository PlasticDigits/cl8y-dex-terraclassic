...
Close leftover **#1300** only after **operator + leftover-*ops*** evidence for the merged stack. Green `make verify-issue-1300` **never** means close. Green child `make verify-issue-*` on a laptop is not leftover-complete. Tracker-close comments on #1300 do **not** waive the four walk notes. The tracker is already **closed**; leftover-*ops* **comments (or reopens) the closed tracker** until those four notes exist. Forgejo closed ≠ leftover-complete.
...
| 1295 | #1255 | Unlisted CW20 LCD `token_info` decimals | **LocalTerra** Create Pair then Swap pick (not paste; not `dex.cl8y.com`) |
...
- Tracker-close comments on #1300 do **not** waive the four LocalTerra walk notes. The tracker is already **closed**; leftover-*ops* **comments (or reopens)** it until those notes exist. Forgejo closed ≠ leftover-complete.
...
| **Q1255** | Unlisted CW20 scale from LCD `token_info`. Walk is LocalTerra-only: Create Pair then Swap pick; leftover pass is **Q1255-6** Max. |
...
| Mint then Swap without **Create Pair** | Rejected. Create Token does not create a pair. `mintCommunityTax` only mints. Swap `lcdEnabled` is `fromInPicker` (`getAllTokens(pairs)`). Pasting `terra1` is **Q1255-4** / **QS-3**. After mint, Create Pair vs UST1/cUSTC/EMBER, then Swap **pick**. |
...
| Pair wasm probe still 1.16.0 | F6 reprice / L24 min-size not on columbus-5. Leftover stays open until store/2-of-3. Local verify stays green (in-tree 1.17.0). |
| Pair wasm probe already 1.17.0 / 11672, leftover stores again | Leftover **defect**. Attest LCD cw2; do not store. |
| Treat August 1.15.0 “already RAN” as leftover-complete | Operator skips attest of **1.17.0**. Leftover stays open until LCD cw2 is 1.17.0. Use `upgrade-582-code-id-pin.sh` probe, not `cw20-code-id-ops.md` Launch checklist. |
...
| T11 | LCD cw2 probe | `UPGRADE582_PROBE_ONLY=1` reports **1.17.0**. Store is leftover-implement / leftover-*ops* **FAIL** unless probe is still 1.16.0. |
...
- [ADR 0004](./0004-terraclassic-retail-gas-census.md) — wrap+2hop envelope; #1264 AC1 measured (`53B06B65…BAC36` / `gas_used=2630228`), issue closed, do not reopen; leftover must **not** treat the family census row as a reopen trigger (**G-CENSUS-2**)
