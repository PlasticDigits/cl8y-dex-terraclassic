...
- Inventing named make commands that execute the four leftover walks, or copying #701/#702 leftover Playwright (`run_leftover_e2e` / `VERIFY701_REQUIRE_CHAIN=1` FAIL when Playwright missing). Treating `VERIFY_ISSUE_1218_CHAIN=1` as the #1218 leftover walk.
...
| LocalTerra walks | Keep issue item 5 as leftover-complete **operator evidence**. Close #1300 only after the four walks (Outcome item 6 how-tos) **or** explicit SKIP (`make has-localterra` down **and** `VERIFY1300_REQUIRE_CHAIN` unset). **Chain probe:** default leftover verify calls `make has-localterra` (down → SKIP; up → PASS probe). **`VERIFY1300_REQUIRE_CHAIN=1` = fail-closed `make has-localterra` only** — no Playwright, does not execute the walks, do not copy #701 Playwright FAIL. `SKIP_CHAIN=1` skips the probe. Default leftover verify does not execute walks when the chain is up. Child Vitest is not the walk. `VERIFY1300_LEFTOVER_E2E=1` SKIP until named (`REQUIRE_CHAIN` is not that flag). |
