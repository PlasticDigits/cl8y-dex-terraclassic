...
Close leftover **#1300** only after **operator + leftover-*ops*** evidence for the merged stack. Green `make verify-issue-1300` **never** means close. Green child `make verify-issue-*` on a laptop is not leftover-complete. Tracker-close comments on #1300 do **not** waive the four walk notes.
...
6. **LocalTerra / manual** (issue leftover item 5 / **M1300-3** — not a substitute for Coolify). Split two gates:
   ...
   Leftover-complete for item 5 is leftover-*ops* walk evidence on #1300 (how-tos below). Tracker-close comments on #1300 do **not** waive those four notes. `has-localterra` down is **not** leftover-complete. Keep #1300 open until LCD cw2 **1.17.0** (probe) and the four walks. Coolify live HTTP SKIP is already not leftover-complete; a probe still at 1.16.0 already keeps #1300 open; walks follow the same split. Slice 2 (leftover implement / `make verify-issue-1300`) does **not** provision LocalTerra and does **not** record walks. Slice 7 (leftover-*ops* / Cloud Agent QA) owns the four how-tos. Optional: Cloud Agent leftover-*ops* may `make setup-cloud-localterra` when item 5 is still open; that is **not** implement-slice close. Do **not** close on probe SKIP. Chain-probe SKIP/PASS is **not** item-5 evidence.
   ...
   **Chain probe / skip-flag contract:** leftover verify may call `make has-localterra` as a probe only. No Playwright. Use **`VERIFY1300_SKIP_HAS_LOCALTERRA` only** (same class as `VERIFY1300_SKIP_LIVE`). Do **not** ship `SKIP_HAS_LOCALTERRA` (short name never skips). Do **not** copy #701 leftover Playwright FAIL (`VERIFY701_REQUIRE_CHAIN=1` FAILs when Playwright is missing; `run_leftover_e2e`). **Slice-2 FAIL:** defining `VERIFY1300_REQUIRE_CHAIN` as Playwright / `run_leftover_e2e` is a leftover-implement FAIL (that Q18/Q19 name fail-closes leftover Playwright; this ticket does not). Default `make verify-issue-1300` (flag unset): chain down → **SKIP** the probe; chain up → **PASS** the probe; **does not** execute the four walks even when the chain is up. `VERIFY1300_REQUIRE_HAS_LOCALTERRA=1`: chain down → **FAIL** (not SKIP); chain up → PASS the probe; still **does not** execute the walks. `VERIFY1300_SKIP_HAS_LOCALTERRA=1`: skip the probe. Child Vitest / laptop-green `make verify-issue-1218` (chain opt-in `VERIFY_ISSUE_1218_CHAIN=1` is wrap-swap E7/E8, not this walk) is **not** the walk. Child skills `#1255` / `#1218` / `#1263` have no LocalTerra walk recipe — leftover-complete uses the how-tos below, not those flags. `VERIFY1300_LEFTOVER_E2E=1` stays **SKIP until named** (**M1300-7**). `VERIFY1300_REQUIRE_HAS_LOCALTERRA` is **not** that flag.
   ...
   | **#1255** | **LocalTerra only** (same class as #1219 not-prod). `mint_control` is a **50 UST1 SKU**. A `dex.cl8y.com` Create Token is spend + a live CW20 — **not** this walk. Create Token **18-dec unlisted** factory CW20. Set **Decimals** to **18** (`create-token-decimals`; default is `'6'` in `CreateTokenPage.tsx`). Enable **Minting** at create (`mint_control` is create-only). Mint cap may be **empty (unlimited — valid)** or human **`≥1`**. `initialBalances: []`. Swap pick as Pay; type `1`. For Max / **Q1255-6**: on Manage Token Mint enter raw **`1000000000000000000`** (18 zeros), not `1`. Manager Mint is raw CosmWasm `mint.amount` (`Amount (raw)` in `ManageTokenPage.tsx`; `mintCommunityTax` forwards the string with no `toRawAmount`). Create Token **mint cap** is human (`parseHumanRaw`) — do **not** mix it with the raw Mint field, and do **not** invent a human Mint amount because the cap is empty. Do **not** use registry-pinned TCL8Y / CL8Y / USTR. | Typed Swap `1`: execute/sim raw **`10^18`** (not `10^6`). Max of that minted raw → human `1` (**Q1255-6**). Typing `1` on Manage Token Mint mints **1 raw**; Max of that is `1e-18` at 18-dec and does **not** prove Q1255-6. Max of a zero balance is `0` at any scale and does **not** prove Q1255-6. |
...
| 1295 | #1255 | Unlisted CW20 LCD `token_info` decimals | **LocalTerra** (not `dex.cl8y.com` Create Token) |
...
- Tracker-close comments on #1300 do **not** waive the four LocalTerra walk notes.
...
| **Q1255** | Unlisted CW20 scale from LCD `token_info`. Walk is LocalTerra-only. |
...
| Option | Why not |
|--------|---------|
| Treat green child verify / green `make verify-issue-1300` as leftover-complete | Repeats the leftover-ops failure mode: prod schema/wasm/UI stay on the previous tip. Leftover-complete is **operator + leftover-*ops***. Green verify never means close. Tracker-close comments do not waive the four walks. |
...
| Drop issue item 5 (Coolify frontend + child verifies enough) | Rejected: leftover-complete keeps the four LocalTerra/manual walks as **operator evidence**. Child Vitest is not the walk. Tracker-close is not the walk. |
| Treat `has-localterra` down / chain-probe SKIP as leftover-complete for item 5 | Rejected. Coolify live HTTP SKIP is not leftover-complete. A probe still at 1.16.0 keeps #1300 open. Walks follow the same split. Slice 7 leftover-*ops* / Cloud Agent QA may `make setup-cloud-localterra` when item 5 is still open; that is **not** implement-slice close. |
...
| Treat a tracker-close comment on #1300 as leftover-complete for issue item 5 | Forbidden. The four walk notes still apply. Green `make verify-issue-1300` still never means leftover-complete. |
| Treat `has-localterra` down / chain-probe SKIP as leftover-complete for issue item 5 | Forbidden. Keep #1300 open, same class as wasm probe still 1.16.0. Coolify live HTTP SKIP is already not leftover-complete. Slice 2 leftover implement does **not** provision or walk. Slice 7 leftover-*ops* / Cloud Agent QA may `make setup-cloud-localterra` when item 5 is still open; that is **not** implement-slice close. Do **not** close on probe SKIP. Chain-probe SKIP/PASS is not item-5 evidence. |
...
| Treat Create Token then Max of a zero balance as **Q1255-6** | Forbidden. Create Token ships `initialBalances: []`. Max of `0` does not distinguish 18-dec vs 6-dec. **LocalTerra only.** Set **Decimals** to **18**, enable **Minting** at create (`mint_control`, 50 UST1 SKU). Mint cap empty (unlimited — **valid**) or human `≥1`. Then Manage Token Mint raw **`1000000000000000000`**. Typing `1` on Mint is **1 raw** (Max `1e-18` at 18-dec). Do not mix mint cap (human `parseHumanRaw`) with Mint amount (raw). Do not invent a human Mint amount because the cap is empty. Do not use registry-pinned TCL8Y / CL8Y / USTR. |
...
7. Close **#1300** only with **operator + leftover-*ops***: Coolify HTTP + sqlx attest (`…000` then `…001`) + LCD cw2 **1.17.0** probe **and** the four walk notes. Tracker-close comments do **not** waive the walks. Green `make verify-issue-1300` never means close. Chain-probe SKIP/PASS is not item-5 evidence. Do **not** reopen **#1264**. Leave **#1279** open unless its own leftover-complete holds.
...
Leftover-complete is **operator + leftover-*ops***. Green `make verify-issue-1300` **never** means close. Tracker-close comments on #1300 do **not** waive the four walk notes.
...
- LocalTerra / manual (issue item 5, slice 7 leftover-*ops*): the four walks recorded per Outcome item 6 how-tos. `#1255` is **LocalTerra-only** (not `dex.cl8y.com` Create Token). Unlimited mint cap is valid. `has-localterra` down is **not** leftover-complete — keep #1300 open. Chain-probe SKIP/PASS is not item-5 evidence. Child Vitest is not the walk. Default leftover verify probes `has-localterra` (down → SKIP) and does not execute the walks. `VERIFY1300_REQUIRE_HAS_LOCALTERRA=1` is fail-closed `has-localterra` only — does not execute the walks, not five-worker e2e. Defining `VERIFY1300_REQUIRE_CHAIN` as Playwright / `run_leftover_e2e` is a leftover-implement FAIL. **`VERIFY1300_SKIP_HAS_LOCALTERRA` only.** Slice 2 leftover implement does **not** provision LocalTerra or record walks. Optional: Cloud Agent leftover-*ops* may `make setup-cloud-localterra` when item 5 is still open; that is **not** implement-slice close.
...
- [ADR 0004](./0004-terraclassic-retail-gas-census.md) — wrap+2hop envelope; #1264 AC1 measured, issue closed, do not reopen
