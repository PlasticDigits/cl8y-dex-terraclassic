...
4. **Leftover visual.** Production `/protocol` glance (hub wrap vs CEX native; selected **vFDUSD** tab not **VFDUSD**) is reviewer leftover after frontend Coolify auto-deploy. It is **not** #297 and **not** inferred from green make. Green `VERIFY_ISSUE_1290_SKIP_E2E=1` is **implement-complete only**.
...
On `main` after closed !1302 (`92c84406` merged `f237a6e6`):
...
- `scripts/qa/verify-issue-1290.sh` **exists** (child 1240 + Vitest + 5-worker Playwright on port 30129 in a subshell). Gaps vs this ADR: no `free_tcp_port 30129`; greps are only `docs/testing.md` + `AGENTS.md` + `PROTOCOL_STATS` (missing PROTOCOL_HUB, `frontend.md`, this ADR, architecture `#dex-hub-wrap-labels`, **Q21**); header/echo title leftover as **#1290 implement PR**.
- Makefile has `verify-issue-1290`. **Missing:** alias `verify-issue-1302` and Frontend-help `verify-issue-1302`.
...
- Slice-0 files are **not** on `main`: this ADR, `docs/architecture.md` `#dex-hub-wrap-labels`, `docs/qa-invariants.md` **Q21**, `docs/README.md` ADR 0008 index. B1290-4 greps those paths — they must ride the implement follow-up.
...
Open a new branch from **current `main`**. Do not retarget or reopen !1302. Do not merge `f237a6e6` as-is.

1. **Land slice 0 on that MR.** Copy these four paths from this design-branch tip onto `main` (`git checkout <design-sha> --` the paths). Cherry-pick of `5957c7f` plus this revision is equivalent if it applies cleanly. Paths:
   - `docs/adr/0008-verify-issue-1290-hub-wrap-labels.md`
   - `docs/architecture.md` (`#dex-hub-wrap-labels`)
   - `docs/qa-invariants.md` **Q21**
   - `docs/README.md` ADR 0008 index
2. Reconstruct `scripts/qa/verify-issue-1290.sh` (Playwright isolation + full B1290-4 greps + leftover **#1302** naming). Add Makefile alias `verify-issue-1302: verify-issue-1290` and Frontend help `verify-issue-1302`.
...
4. PR body cites **#1302**. Related !1290 / #1240. Do **not** `Fixes #1290`.

### Makefile

Place immediately after `verify-issue-1240` (current `main` already has `verify-issue-1290`; **add the alias**):
...
Alias matches `verify-issue-1289: verify-issue-1265`. Frontend help already lists `verify-issue-1290` after !1302; **add** `verify-issue-1302`.

### `scripts/qa/verify-issue-1290.sh`

Header/echo name leftover **#1302** (Q21 / B1290), related !1290 / #1240 — not “#1290 implement PR”.
...
| 3 | Playwright `e2e-smoke` **5 workers** on `e2e/protocol-page.spec.ts`. Isolation matches sibling `verify-issue-703.sh`: copy `free_tcp_port`, call `free_tcp_port 30129`, then run the test in a **subshell** with `PLAYWRIGHT_SKIP_CHAIN=1 PLAYWRIGHT_WEB_PORT=30129 PLAYWRIGHT_BASE_URL=http://127.0.0.1:30129` **only for that step**. Do not `export` those into the parent shell (CORS). Stale Vite on 30129 is a flake FAIL without `free_tcp_port`. |
| 4 | Grep `verify-issue-1290` in the **full B1290-4 set**: `docs/testing.md`, `AGENTS.md`, `skills/AGENTS_FRONTEND_PROTOCOL_STATS.md`, `skills/AGENTS_FRONTEND_PROTOCOL_HUB.md`, `docs/frontend.md`, this ADR (`docs/adr/0008-verify-issue-1290-hub-wrap-labels.md`), `docs/architecture.md` (`dex-hub-wrap-labels` or `verify-issue-1290`), and `docs/qa-invariants.md` **Q21**. Weaker sketch greps (testing + AGENTS + PROTOCOL_STATS only) are **not** B1290-4. |

**Playwright step (copy `free_tcp_port` from [`scripts/qa/verify-issue-703.sh`](../../scripts/qa/verify-issue-703.sh)):**
...
Do not title rows as the #1290 product PR. Green skip-E2E is implement-complete only.

**`docs/testing.md`** (new or replacement row; keep the #1240 product row):

`| Hub wrap vs CEX leftover verify | [#1302](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1302) | [\`make verify-issue-1290\`](../Makefile) (alias \`verify-issue-1302\`) | **Q21** / **B1290-1–B1290-8** — child **1240** + \`hubPriceTicker\` Vitest + optional Playwright \`protocol-page\` (5 workers). Related [!1290](https://git.cl8y.com/code/cl8y-dex-terraclassic/pulls/1290) / [#1240](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1240). Green \`VERIFY_ISSUE_1290_SKIP_E2E=1\` is implement-complete only; leftover is reviewer \`/protocol\` visual |`

**`AGENTS.md`** Cloud Agent line (after `verify-issue-1240`): leftover **#1302** hub wrap vs CEX (**Q21** / **B1290**; alias `verify-issue-1302`; related !1290 / #1240). Lint table: keep **1240** as the product harness; leftover row points at `make verify-issue-1290` · `make verify-issue-1302`. PROTOCOL_STATS playbook line already cites P1240; add leftover `make verify-issue-1290`.

**`skills/AGENTS_FRONTEND_PROTOCOL_STATS.md`:** issue line cites leftover [**#1302**](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1302) (**Q21** / **B1290-1–B1290-8**), related !1290 / #1240 — not “#1290 implement bundle”. Verify block: `make verify-issue-1290  # leftover #1302 (Q21 / B1290; alias verify-issue-1302)`.

**`skills/AGENTS_FRONTEND_PROTOCOL_HUB.md`:** leftover verify `make verify-issue-1290` (alias `verify-issue-1302`) — **#1302**, **Q21** / **B1290**; related !1290 / #1240.
...
| Layer | Change |
|-------|--------|
| Design docs (slice 0 — **must land on the implement follow-up**) | Keep / cherry-pick `docs/adr/0008-verify-issue-1290-hub-wrap-labels.md`, `docs/architecture.md` `#dex-hub-wrap-labels`, `docs/qa-invariants.md` **Q21**, `docs/README.md` ADR 0008 index. B1290-4 greps these files. |
| QA script | Reconstruct `scripts/qa/verify-issue-1290.sh` (already on `main` from closed !1302): `free_tcp_port 30129`, full B1290-4 greps, leftover **#1302** header. |
| Makefile | Keep `verify-issue-1290`; **add** alias `verify-issue-1302` + Frontend help `verify-issue-1302`. |
| Docs / skills (slice 2) | `docs/testing.md` leftover **#1302** row; `docs/frontend.md` regression line; `AGENTS.md` Cloud Agent + lint table; PROTOCOL_STATS / PROTOCOL_HUB leftover **#1302** pointers (**Q21** / **B1290**; related !1290 / #1240). |
...
| **B1290-1–B1290-8** | Bundle/leftover IDs (this ADR + **Q21**). Keep Q21 in sync with the table below. |
...
| **B1290-3** | Playwright protocol-page is default-on at **5 workers**, port **30129**, `PLAYWRIGHT_SKIP_CHAIN=1`. Copy `free_tcp_port` from `verify-issue-703.sh` and free **30129** first. Port + `PLAYWRIGHT_BASE_URL` only in that step’s subshell. Skip only via `VERIFY_ISSUE_1290_SKIP_E2E=1` or missing Playwright package. Installed package + crash (missing Chromium / NSS) = FAIL. |
| **B1290-4** | Crosslinks: testing, AGENTS, PROTOCOL_STATS, PROTOCOL_HUB, frontend.md, this ADR, architecture `#dex-hub-wrap-labels`, **Q21**. Implement greps **all eight**. Slice-0 files ride the follow-up so those greps can pass. |
...
| **B1290-7** | Do not add `frontend-dapp/node_modules`, hook-test FAIL residue, or `.gitignore` churn. Do not edit `scripts/test-commit-msg-hook.sh`. Do not create `AGENTS_POST_MERGE_OPS_1302.md`. |
...
**Added:** one bash wrapper (reconstruct), Makefile alias, full B1290-4 greps, optional E2E skip flag, `free_tcp_port` on 30129, **Q21** / **B1290**, slice-0 docs on the follow-up.

**Removed:** the gap where post-merge hub **cLUNC** vs CEX **LUNC** had no agent `make` entry besides 1240’s CSS/docs subset; the closed-PR sketch’s weaker grep / missing alias / stale-Vite flake.
...
| B1290-4 greps fail because slice-0 docs stayed on the design branch | Follow-up must land the four slice-0 paths. Do not shrink the grep set to the sketch. |
...
|-------|-----|-------------|--------|
| **0 — this design** | design_author | This ADR, architecture `#dex-hub-wrap-labels`, **Q21**, `docs/README.md` index. Transported on `cac-design-issue-1302` only (no design-only PR). | Slice 1 |
| **1 — script + Makefile** | implement | On **current `main`**: reconstruct `scripts/qa/verify-issue-1290.sh` (`free_tcp_port 30129`, subshell port env, full B1290-4 greps, leftover **#1302** header); alias `verify-issue-1302`; Frontend help. **Checkout slice-0 paths from this design tip onto the follow-up** so B1290-4 can pass. Do not retarget closed !1302. Do not merge `f237a6e6` as-is. Do not touch `scripts/test-commit-msg-hook.sh`. Keep `git ls-files frontend-dapp/node_modules` empty. | Slice 2 |
| **2 — crosslinks** | implement | Land slice-0 docs (ADR 0008, architecture `#dex-hub-wrap-labels`, **Q21**, `docs/README.md` index) **plus** leftover **#1302** wording in `AGENTS.md`, `docs/testing.md`, `docs/frontend.md`, PROTOCOL_STATS, PROTOCOL_HUB (**Q21** / **B1290-1–B1290-8**; related !1290 / #1240). Optional comment on 1240 script / protocol-page spec. **No** product TSX, **no** dirty files from `f237a6e6`, **no** `Fixes #1290`. | none |
| **3 — reviewer leftover** | reviewer / operator | After frontend Coolify rebuild of the follow-up merge (or already-shipped !1290 labels): `/protocol` hub **cUSTC / USD** + **cLUNC / USD**; oracle tabs **USTC** / **LUNC** / **vFDUSD**; selected vFDUSD not **VFDUSD**. | Closes leftover AC only |

Slice 1 **must not** wait on slice 3. Slice 3 **must not** be attempted as a Coolify checkbox or deploy.

No product-issue dependencies. #1240 and !1290 are already on `main`. Closed !1302 is a weaker sketch to reconstruct, not a blocker.
...
- Bundle with skip: 1240 + hubPriceTicker + skipped E2E + **full B1290-4 greps** (including this ADR, architecture, **Q21**), exit 0.
- Bundle without skip on a provisioned VM: Playwright P1/P2/P4 casing (5 workers); `free_tcp_port` / 30129 subshell present in the script.
...
- `docs/testing.md` leftover row cites **#1302** / **Q21** / **B1290**, not the #1290 product PR.

## Rollout

1. Merge slices 1–2 to `main` via a **new** follow-up PR. Frontend Coolify auto-deploy may rebuild; labels are already on `main` from !1290 — this merge is verify-only + slice-0 docs.
...
Revert the follow-up (verify script, Makefile alias, crosslinks, slice-0 docs). Product labels remain from !1290. No schema, wasm, or env rollback. Frontend image rollback is unrelated (verify-only MR).

## Integration completion criteria

**Implement merge is complete when:**

- Slice-0 paths (this ADR, architecture `#dex-hub-wrap-labels`, **Q21**, `docs/README.md` index) are on `main` via the follow-up — not left only on `cac-design-issue-1302`.
- `VERIFY_ISSUE_1290_SKIP_E2E=1 make verify-issue-1290` exits 0 on a worktree from `main` + that MR, including full B1290-4 greps.
- Alias `make verify-issue-1302` is the same target; Frontend help lists it.
- Script copies `free_tcp_port` from 703 and keeps 30129 env in the Playwright subshell.
