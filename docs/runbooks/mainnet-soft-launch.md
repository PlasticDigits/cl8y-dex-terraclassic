...
**Indexer:** build context = repo root; Dockerfile [`docker/indexer/Dockerfile`](../../docker/indexer/Dockerfile) (not Nixpacks). Set `DATABASE_URL`, `FACTORY_ADDRESS`, `ROUTER_ADDRESS`, `FEE_DISCOUNT_ADDRESS`, `CORS_ORIGINS=https://dex.cl8y.com`, `RUN_MODE=prod`, operator-controlled `LCD_URLS`, `API_BIND=0.0.0.0`. Bake a real commit hex via build-args **`GIT_SHA`** / **`SOURCE_COMMIT`** declared on the **runtime** stage (never the literal `HEAD` token; empty `GIT_SHA` falls through to `SOURCE_COMMIT`). `GET /health` may then include `git_sha` ([ADR 0006](../adr/0006-indexer-health-git-sha.md) / [#1276](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1276)). Protected-branch **auto-deploy** on this app is operator leftover ([#297](https://git.cl8y.com/PlasticDigits/cl8y-agent-control/issues/297)); optional watch paths `/indexer/**` and `/docker/indexer/**`. Attest with public `/health`, not Coolify log scrape. CAC drain is not this app’s redeploy path.
