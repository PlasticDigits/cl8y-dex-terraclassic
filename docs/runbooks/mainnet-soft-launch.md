...
**Frontend:** build-args from `frontend.env.example` including `VITE_WC_PROJECT_ID` and `VITE_INDEXER_URL=https://indexer.dex.cl8y.com`. **Do not set `VITE_FAUCET_ADDRESS` on production `dex.cl8y.com`** ([#562](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/562) / **F11**) — Mint nav must stay hidden; Pause the faucet per [`soft-launch-faucet.md`](./soft-launch-faucet.md) **F9**. `VITE_TOKEN_*` gem addresses are optional (hide uses hardcoded columbus-5 addrs). Do not set `VITE_SHOW_TEST_TOKENS` or `VITE_DEV_MNEMONIC` on the production image. After stacking !368–!377, rebuild Coolify frontend + restart indexer together — [`skills/AGENTS_POST_MERGE_STACK.md`](../../skills/AGENTS_POST_MERGE_STACK.md) ([#573](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/573) **M573-2** / **M573-3**).
