...
3. **Migrate** each contract that supports `Migrate` (or follow contract-specific upgrade path in `smartcontracts/`). **#518 LP digits:** [`scripts/upgrade-518-lp-symbol.sh`](../../scripts/upgrade-518-lp-symbol.sh) stores factory + pair, migrates factory, then `UpdateConfig` `pair_code_id` / `lp_token_code_id` ([`AGENTS_LP_SYMBOL_DIGITS.md`](../../skills/AGENTS_LP_SYMBOL_DIGITS.md)). **#582 / #584 F6 code-id pin:** [`scripts/upgrade-582-code-id-pin.sh`](../../scripts/upgrade-582-code-id-pin.sh) — **factory 1.9.0 before any pair 1.15.0**; then `UpdateConfig { pair_code_id }` to the new pair wasm; LCD `GET /cosmwasm/wasm/v1/contract/{addr}` ContractInfo probe first; smoke retries `IsCodeIdWhitelisted` ([`AGENTS_CW20_CODE_ID_PIN.md`](../../skills/AGENTS_CW20_CODE_ID_PIN.md)). Read-only probe: [`scripts/qa/probe-columbus5-contract-info.sh`](../../scripts/qa/probe-columbus5-contract-info.sh). Columbus-5 first run: 2026-08-21 (factory 11602 / pairs 11601).
