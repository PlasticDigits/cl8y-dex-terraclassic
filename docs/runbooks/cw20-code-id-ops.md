...
**Mitigation (ops, not on-chain):** governance verifies production code IDs are **standard** (full-amount transfer) templates before `AddWhitelistedCodeId`.

**Do not** implement on-chain balance-delta reconciliation for this — prohibition + verification only (H-01 guardrail).
