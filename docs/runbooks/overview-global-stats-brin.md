...
Token `token_volume_stats` and trader `volume_24h` / `7d` / `30d` use the same trailing cutoffs and **zero idle rows** on refresh. Lifetime `traders.total_volume*` is unchanged. Trader rolling + lifetime **raw** columns are `NUMERIC(38, 0)` ([#1277](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1277), [ADR 0005](../adr/0005-trader-rolling-volume-numeric.md)); `total_volume_usd` stays `(38, 18)` (unpriced NULL). Coolify leftover: sqlx migrate (widen + heal missing/stale senders from `swap_events` + #553 USD stamp) then indexer restart — poller heal is mismatch-gated and runs **before** `refresh_all_volume_windows(..., true)`. Block replay does not insert skipped `traders` rows. No narrowing down-migration. Skill: [`AGENTS_INDEXER_VOLUME_WINDOW_DECAY.md`](../../skills/AGENTS_INDEXER_VOLUME_WINDOW_DECAY.md); [`AGENTS_INDEXER_TRADER_ROLLING_VOLUME.md`](../../skills/AGENTS_INDEXER_TRADER_ROLLING_VOLUME.md).
