...
Token `token_volume_stats` and trader `volume_24h` / `7d` / `30d` use the same trailing cutoffs and **zero idle rows** on refresh. Lifetime `traders.total_volume*` is unchanged. Trader rolling + lifetime **raw** columns are `NUMERIC(38, 0)` ([#1277](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1277), [ADR 0005](../adr/0005-trader-rolling-volume-numeric.md)); `total_volume_usd` stays `(38, 18)`. Coolify leftover: sqlx migrate (widen + heal missing senders from `swap_events`) then indexer restart — block replay does not insert skipped `traders` rows. Skill: [`AGENTS_INDEXER_VOLUME_WINDOW_DECAY.md`](../../skills/AGENTS_INDEXER_VOLUME_WINDOW_DECAY.md); [`AGENTS_INDEXER_TRADER_ROLLING_VOLUME.md`](../../skills/AGENTS_INDEXER_TRADER_ROLLING_VOLUME.md).
