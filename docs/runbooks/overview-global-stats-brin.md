...
Token `token_volume_stats` and trader `volume_24h` / `7d` / `30d` use the same trailing cutoffs and **zero idle rows** on refresh. Lifetime `traders.total_volume*` is unchanged. Trader rolling + lifetime **raw** columns are `NUMERIC(38, 0)` ([#1277](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1277), [ADR 0005](../adr/0005-trader-rolling-volume-numeric.md)); `total_volume_usd` stays `(38, 18)` (unpriced NULL). Coolify leftover: sqlx migrate (widen + heal missing/stale senders + leftover-zero for leftover-lifetime ghosts + #553 USD stamp + leftover-USD `NULL`) then indexer restart — poller heal is `COALESCE`-gated (like #676) and runs **before** `refresh_all_volume_windows(..., true)` in one transaction. Block replay does not insert skipped `traders` rows. No narrowing down-migration. Skill: [`AGENTS_INDEXER_VOLUME_WINDOW_DECAY.md`](../../skills/AGENTS_INDEXER_VOLUME_WINDOW_DECAY.md); [`AGENTS_INDEXER_TRADER_ROLLING_VOLUME.md`](../../skills/AGENTS_INDEXER_TRADER_ROLLING_VOLUME.md).
...
Multihop AMM hops must each persist (**#1269**). `swap_amm` uniqueness is `(tx_hash, source, pair_id, ordinal)` where `pair_id IS NOT NULL`; wrap / window / book / place stay `(tx_hash, source, ordinal)` where `pair_id IS NULL`. Per-pair `swap_index` 0 on two pairs in one router tx must not collapse. After the pair_id migration, poller startup backfills missing hops from `swap_events.commission_amount` then the aggregator refresh. Replay of the same hop stays `ON CONFLICT DO NOTHING` — never `DO UPDATE`. Playbook: [`AGENTS_INDEXER_PROTOCOL_FEE_HOPS.md`](../../skills/AGENTS_INDEXER_PROTOCOL_FEE_HOPS.md).
