...
- **Trader rolling volume skip (#1277):** `insert_swap` commits before `upsert_trader`. A `(38, 18)` overflow (pre-widen) leaves the swap and skips trader identity on replay (`trade_exists`). Heal from `swap_events` at migrate + poller startup **before** `refresh_all_volume_windows(..., true)` (`poller.rs`, #1269 slot — not the #676 slot after refresh). Same skip class as #676 positions; poller heal is mismatch-gated with the same `COALESCE` idea as `positions_trade_count_diverges` (leftover-lifetime with no remaining `swap_events` **trips**; registered / zero-lifetime ghosts do **not**). Rolling refresh is UPDATE-only. Heal is one transaction: INSERT + lifetime UPDATE + leftover-zero `UPDATE` (`total_trades = 0`, `total_volume = 0`, `first_trade_at` / `last_trade_at` `NULL` — **do not `DELETE`**; keep tier / registered / P&L) + copied `traders::refresh_trader_total_volume_usd` SQL (`traders.rs`, not `volume.rs`; not `refresh_trader_total_volume_usd(&pool)`) + leftover-USD `NULL` when no remaining priced swaps (unpriced stays NULL). Gate is raw-lifetime only. Canonical SQL: [`design/1277-trader-rolling-volume-numeric.md`](../../design/1277-trader-rolling-volume-numeric.md). Playbook: [`AGENTS_INDEXER_TRADER_ROLLING_VOLUME.md`](../../skills/AGENTS_INDEXER_TRADER_ROLLING_VOLUME.md). ADR [0005](../adr/0005-trader-rolling-volume-numeric.md).
