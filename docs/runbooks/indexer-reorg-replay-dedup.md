...
- **Trader rolling volume skip (#1277):** `insert_swap` commits before `upsert_trader`. A `(38, 18)` overflow (pre-widen) leaves the swap and skips trader identity on replay (`trade_exists`). Heal from `swap_events` at migrate + poller startup **before** `refresh_all_volume_windows(..., true)` (`poller.rs`, #1269 slot — not the #676 slot after refresh). Poller wraps heal in `if let Err` like `#1269` / `#676` — does **not** halt the indexer; next boot retries; a later heal `UPDATE` recomputes from `swap_events`. Same skip class as #676 positions; poller heal is mismatch-gated with the same `COALESCE` idea as `positions_trade_count_diverges` (leftover-lifetime with no remaining `swap_events` **trips**; registered / zero-lifetime ghosts do **not**). Rolling refresh is UPDATE-only. Heal is one transaction: INSERT + lifetime UPDATE + leftover-zero `UPDATE` (`total_trades = 0`, `total_volume = 0`, `first_trade_at` / `last_trade_at` `NULL` — **do not `DELETE`**; keep tier / registered / P&L) + SQL **text** of `traders::refresh_trader_total_volume_usd` (function, not line numbers; shared `&str` allowed; keep `(&PgPool)`; not `volume.rs`; not `refresh_trader_total_volume_usd(&pool)` from the heal helper) + leftover-USD `NULL` when no remaining priced swaps (unpriced stays NULL). Gate is raw-lifetime only. Canonical SQL: [`design/1277-trader-rolling-volume-numeric.md`](../../design/1277-trader-rolling-volume-numeric.md). Playbook: [`AGENTS_INDEXER_TRADER_ROLLING_VOLUME.md`](../../skills/AGENTS_INDEXER_TRADER_ROLLING_VOLUME.md). ADR [0005](../adr/0005-trader-rolling-volume-numeric.md).
...
3. **Choose recovery:**
   ...
   - **Heavy SQL:** Use `--cleanup-derived` from an earlier fork height `H` and accept that `trader_positions` / trader rollups may need a full re-backfill or snapshot (not height-keyed). After the indexer is back, run **`cl8y-dex-indexer rebuild-positions`** (or wait for poller `repair_positions_if_trade_count_mismatch` — GitLab **#676**, `NUMERIC(78, 18)`). Block replay alone does not rebuild positions for swaps that already exist. After `--cleanup-derived`, leftover `traders.total_trades` / `total_volume` still count deleted swaps. Positions `DELETE`s leftover `(trader, pair)` rows; trader heal **must not `DELETE`** `traders` (keep tier / registered / P&L). Canonical leftover-zero is `UPDATE … SET total_trades = 0, total_volume = 0, first_trade_at = NULL, last_trade_at = NULL WHERE NOT EXISTS (swap_events) AND lifetime inflated` plus leftover-USD `NULL` when no remaining priced swaps — same file as INSERT/UPDATE, same poller transaction (`poller.rs`, before `refresh_all_volume_windows(..., true)` — GitLab **#1277**). Gate is `COALESCE` like `positions_trade_count_diverges`, not `s.sender IS NOT NULL`. Do **not** zero idle traders who still have old `swap_events` (**D2**). Additive `upsert_trader` on re-ingest **double-counts** unless that leftover-zero ran. Block replay alone does not rebuild skipped or inflated trader totals.
...
| 5 | Restart indexer | Logs show blocks indexing from `H`. After `--cleanup-derived`, startup `heal_trader_lifetime_from_swaps` runs **before** `refresh_all_volume_windows(..., true)` so leftover-lifetime ghosts (`total_trades > 0`, no remaining `swap_events`) are **zeroed** (not deleted) and leftover USD is `NULL`d in the same transaction before re-ingest (#1277; `COALESCE` gate like `positions_trade_count_diverges` / #676). Registered `total_trades = 0` ghosts must not trip. Pass: leftover row is `total_trades = 0` / `total_volume = 0`; simulated `upsert_trader` is not additive on the old totals. |
