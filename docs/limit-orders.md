...
- **Limit price vs tape reference (GitLab [#154](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/154)):** the trade ticket and standalone limit page show **last-trade** token1/token0, **% deviation**, a **headline-scaled USD** line (same tape string as the chart headline), and **block submit** when a **bid** is at/above the reference or an **ask** is at/below it. Invariants: [`docs/frontend.md` § Trade page — limit order price field](./frontend.md#trade-page-limit-order-price); agent skill [`skills/AGENTS_FRONTEND_LIMIT_ORDER_PRICE.md`](../skills/AGENTS_FRONTEND_LIMIT_ORDER_PRICE.md).
