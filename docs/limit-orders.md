...
- **`hybrid`:** optional [`HybridSwapParams`](../smartcontracts/packages/dex-common/src/pair.rs): `pool_input`, `book_input` (must sum to the CW20 `amount`), `max_maker_fills`, optional `book_start_hint` (order id). `hybrid: None` together with `greedy: None` is **pool-only** (**G1**, [#708](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/708)). For a book-inclusive quote, execute with the same params returned by the solver or used in `HybridSimulation`.
