...
- **Scan step budget ([GitLab #254](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/254)):** Each book walk counts **every** doubly-linked list iteration (fills, parks, skips, zero-remaining continues) against **`MAX_SCAN_STEPS`** (`MAX_MAKER_FILLS_HARD_CAP` + 32 = **288**). When the budget is exhausted, the walk stops early; unfilled book budget rolls to the pool leg (existing spread rules). Swap attr: `scan_steps_capped=true`. **`HybridSimulation`** applies the same step budget so quotes match execute when the cap binds.
...
| `scan_steps_capped` | Book walk hit **`MAX_SCAN_STEPS`** (288) before `max_maker_fills` or list end ([#254](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/254)) |
