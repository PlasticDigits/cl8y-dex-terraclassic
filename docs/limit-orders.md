...
- **Integrator warning (I707):** a pair-direct `Swap {}` that omits both `hybrid` and `greedy` is pool-only and skips resting orders. Use [`/route/solve`](./integrators.md#pair-swap-pool-only-vs-best-execution-forgejo-707) for best execution; do not guess a Pattern C split. This is current behavior until the separate pair/router default rollout in [#718](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/718).
- **`hybrid`:** optional [`HybridSwapParams`](../smartcontracts/packages/dex-common/src/pair.rs): `pool_input`, `book_input` (must sum to the CW20 `amount`), `max_maker_fills`, optional `book_start_hint` (order id). Current `hybrid: None` + `greedy: None` is **pool-only** (**G1**, [GitLab #708](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/708)); the default-greedy change is still pending in [#718](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/718).
- **`greedy`:** optional [`GreedySwapParams`](../smartcontracts/packages/dex-common/src/pair.rs) — opt-in book-first walk (**G2**): only `max_maker_fills` + optional `book_start_hint`. Mutually exclusive with `hybrid` on **query and execute** (**G11** / [#709](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/709)). Walks live same-side makers whose net Decimal rate **strictly beats** the pool spot after fees, then residual offer → pool (**G3** / **G9**). `greedy_stop=remainder_to_pool` when makers filled and offer remains (`empty` means no maker was filled). Pool-spot overflow Skip, not panic. `max_maker_fills == 0` rejects; oversize clamps to 100 (**G5**). Execute requires `belief_price` or `min_return` (**G8**). Quote via `HybridSimulation` with `greedy` set — same `resolve_swap_hybrid_mode` as execute (**G7**). Official dApp does **not** use this path (solver stays **#501** / **#596**). Pause / blacklist / AfterSwap / community-tax extra-debit: [#710](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/710). Playbook: [`skills/AGENTS_GREEDY_BOOK_FIRST.md`](../skills/AGENTS_GREEDY_BOOK_FIRST.md).
- **Match walk:** If `book_start_hint` is set and that order id still exists **on the side being matched** (bid hint for `match_bids`, ask hint for `match_asks`), matching starts from that id; wrong-side, stale, or missing hints fall back to the book head with no error. Each walked order must match the active matcher side (defense in depth — GitLab [**#272**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/272)). Same **L17** rule on greedy (**G6**). See invariant **L17** in [contracts-security-audit.md](./contracts-security-audit.md); agent playbook [skills/AGENTS_BOOK_MATCH_HINT_SECURITY.md](../skills/AGENTS_BOOK_MATCH_HINT_SECURITY.md).

### Place / cancel limit (GitLab [#206](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/206))

- **`Cw20HookMsg::PlaceLimitOrderBatch`:** `side`, `orders[]` each with `price`, `amount` (gross escrow for that rung), `max_adjust_steps`, optional `expires_at`, optional **`hint_after_order_id`** (GitLab **#261** — per-rung predecessor for O(1) insert; explicit client hint wins over internal batch chaining from the prior successful rung). The CW20 `send` **amount must equal the sum** of per-rung `amount` values. **Same side per batch** (bid escrows token1, ask escrows token0). **Validation is all-or-nothing** (empty batch, cap, amount mismatch, invalid price/expiry/**price band** ([#467](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/467)), **post–maker-fee remaining `< 10`** ([#1219](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1219) / **L24** — named `LimitOrderAmountTooSmall`, not Overflow), maker fee too large → whole tx reverts). **Book-walk is partial** ([#206](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/206) design note): if a rung hits `LimitInsertStepsExceeded`, that rung is skipped, escrow for skipped rungs is **CW20-refunded** to the maker, and later rungs still attempt; if **no** rung places, the tx reverts (`LimitBatchNoRungsPlaced`). Undersize is **not** a skip.
- **`Cw20HookMsg::PlaceLimitOrderLadder`:** `ladder` with `start_price`, `end_price`, `count` (≥ 2), `total_amount`, `distribution` (`equal`), shared `max_adjust_steps` / `expires_at`, optional **`hint_after_order_id`** on the **head-most rung in book order** (GitLab **#266** — boundary anchor for deep-book placement; resolved client-side via indexer **#267**). Expanded on-chain to the same rules as batch. Prices step from **`start` toward `end`** (descending `end < start` is valid; #1219). Equal-split rungs whose gross amount is **`< 10`** fail expand before insert.
...
- **Gas envelope ([#1329](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1329)):** retail place uses one `increase_allowance` plus one CW20 `send` → batch/ladder hook. The hook gas limit is `1_000_000 + 180_000 × rung_count + 25_000 × sum(rung max_adjust_steps)`; a legacy single place uses a `1_200_000` base plus its step allowance. Price relink uses `350_000 + 25_000 × max_adjust_steps`. The pair clamps each insert walk at **256**. Batch placement sorts rungs and threads the cursor after success, but a skipped rung may retry a bounded head walk; budget the sum of the per-rung caps. LUNC preflight and the displayed ladder estimate use the same selected caps. These are gas **limits** derived from the signed message shape, not query-string input. See [§ Batch / ladder gas savings](#batch-ladder-gas-savings) and skill [`AGENTS_HYBRID_LIMIT_GAS.md`](../skills/AGENTS_HYBRID_LIMIT_GAS.md).
...
- **`MAX_ADJUST_STEPS`:** each rung uses its own `max_adjust_steps` when walking from the book head; the pair clamps the walk at `MAX_ADJUST_STEPS_HARD_CAP` (**256**). Gas estimation sums rung caps because a rung skipped for exceeding its walk can leave no thread cursor for the next rung. Place, ladder, price-update gas and native LUNC preflight must stay aligned (**L25**, [#1329](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1329)). Hybrid fill has separate maker/scan/expired-park caps. Query-string `gas` / `credit` values are ignored and never enter the signed fee. See [`skills/AGENTS_HYBRID_LIMIT_GAS.md`](../skills/AGENTS_HYBRID_LIMIT_GAS.md).
...
- **Bids:** sort by **descending** `price`, then **arrival at that price** (higher price first).
- **Asks:** sort by **ascending** `price`, then **arrival at that price** (lower ask price first).

**New placements** (and equal-price **batch** rungs, [#266](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/266)): arrival ≈ ascending `order_id` because ids are monotonic at place time.

**Reprice (`UpdateLimitOrderPrice`, [#1227](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1227)):** the node keeps its `order_id` (no second maker fee, no CW20 — [#247](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/247)) but **inserts as a new arrival** at the destination price — tail of that equal-price run. A same-price update does not move the node. Cross-price priority is unchanged (a better price is still closer to the head). The closed [#424](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/424) “leapfrog by preserved id” quirk is **fixed in the matcher**, not left as by-design. `hint_after_order_id` remains advisory (**L14**) and cannot invert equal-price FIFO on relink.

Match / `simulate_match_*` walk the DLL (not a re-sort by id). Indexer `db_orderbook_sim` uses snapshot **`walk_index`** so quotes agree after relink (**L8**). Regression: `make verify-issue-1227`.
...
**Protocol constant:** `LIMIT_ORDER_DUST_FLUSH_THRESHOLD = 10` in [`dex-common::limit_placement`](../smartcontracts/packages/dex-common/src/limit_placement.rs) (re-exported from [`dex-common::pair`](../smartcontracts/packages/dex-common/src/pair.rs); not governance-configurable in v1). **Placement** uses the same floor: post–maker-fee remaining **`< 10`** reverts named (**L24** / [#1219](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1219)). Match-time flush still applies to **legacy** sub-10 rests and to dust left **after a fill**. The size floor does not enforce economic fillability at MIN_LIMIT_PRICE; that unresolved match/eviction gap is [Forgejo #1225](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1225), tracked by invariant [L26](./contracts-security-audit.md) and documented in [§ Unfillable in-band floor orders](./limit-orders.md#unfillable-in-band-floor-orders-forgejo-1225).
...
<a id="unfillable-in-band-floor-orders-forgejo-1225"></a>

### Unfillable in-band floor orders (Forgejo #1225)

**Open security gap; current main does not satisfy this invariant.** The human-scale gate accepts exactly MIN_LIMIT_PRICE; it does not check whether the post-fee remaining can produce a non-zero floor(fill × price). The matcher and simulate_match_* advance past a zero-cost row without unlinking it, and indexer [db_orderbook_sim](../indexer/src/api/db_orderbook_sim.rs) mirrors that skip. A live row with no expiry is not eligible for the default CleanLimitBook thresholds (0/0), so a minimum-price ask can remain at the FIFO head while consuming scan steps.

**Required invariant:** a live in-band order that cannot produce a positive counter-asset cost for any partial fill must be rejected at placement, price update, and ladder expansion, or be unlinked through the existing claimable park path under the default clean configuration. A prefix of such rows must not exhaust MAX_SCAN_STEPS while leaving later fillable orders linked. Preserve the L18 guarantee that a maker is never paid zero; keep pair execute, pair simulation, and indexer simulation aligned; and preserve escrow, pause, and expiry-park-cap rules (L1, L5, L6, L8).

This is distinct from [#470](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/470): the zero-cost skip is correct for a single unaffordable partial fill, but it is not a liveness remedy. It is also distinct from [#467](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/467) / [#529](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/529) (out-of-band price rejection), [#289](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/289) (expired-prefix hints), [#263](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/263) (clean thresholds default to zero), [#264](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/264) (post-fill dust parking), or [#1219](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1219) (minimum remaining size). A book_start_hint past the rows only helps one taker; it does not unlink them. The open default-greedy proposal [#718](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/718) remains gated on this fix.

Current code under review: [validate_limit_order_price](../smartcontracts/packages/dex-common/src/limit_placement.rs), pair [match_asks / match_bids and simulate_match_*](../smartcontracts/contracts/pair/src/orderbook.rs), [CleanLimitBook](../smartcontracts/contracts/pair/src/limit_book_clean.rs), and indexer [db_orderbook_sim](../indexer/src/api/db_orderbook_sim.rs). Existing #470 tests verify no zero-value payout; they do not cover #1225's minimum-price head prefix, removal, or indexer parity. See required evidence in invariant [L26](./contracts-security-audit.md) and the agent playbooks [AGENTS_LIMIT_ORDER_BATCH_LADDER.md](../skills/AGENTS_LIMIT_ORDER_BATCH_LADDER.md) and [AGENTS_GREEDY_BOOK_FIRST.md](../skills/AGENTS_GREEDY_BOOK_FIRST.md).
...
|------|------------------------------|--------------------------------------------------------------------------------------------------|
| **N separate** limits | N × (`increase_allowance` + `send`) = **2N** wallet prompts | N × (`1.18M + 25k × max_adjust_steps`) + N allowance envelopes |
| **One batch / ladder** | **1** `send` | `1M + 180k × N + 25k × sum(rung max_adjust_steps)` + one allowance envelope |

Example (5 rungs, Medium = 32 steps, fee math only — actual gas used varies by chain): batch attach is **5.9M** gas units vs 5×1.98M = **9.9M** for separate placements. Native LUNC preflight uses `estimateLimitOrderBatchPlaceSequenceUlunaFeesTotal(rungCount, maxAdjustSteps)` ([`transactions.ts`](../frontend-dapp/src/services/terraclassic/transactions.ts)). UI copy: [`LimitOrderLadderPanel`](../frontend-dapp/src/components/trade/LimitOrderLadderPanel.tsx) · agent skills [`AGENTS_LIMIT_ORDER_BATCH_LADDER.md`](../skills/AGENTS_LIMIT_ORDER_BATCH_LADDER.md) and [`AGENTS_HYBRID_LIMIT_GAS.md`](../skills/AGENTS_HYBRID_LIMIT_GAS.md). Retail single place is batch `n=1` (**1.98M** at Medium / 32 steps). The old 400k base OOGed tax CW20 `Send` + pair place at 580k ([#625](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/625)).
