...
- **Broadcast `Fee` (gas price × `gas_wanted`):** placement is **`increase_allowance`** (base gas) then **`send` → `place_limit_order`**. The dApp must attach enough **uluna** in `Fee.amount` for Station / network minimum gas price; with **`VITE_NETWORK=local`**, Station connect also runs **`experimentalSuggestChain`** so the extension’s **`gasPriceStep`** matches Columbus-5 minimums (see [`wallet.ts`](../frontend-dapp/src/services/terraclassic/wallet.ts)). **QA:** verify with **both Station and Terra Classic Keplr** on LocalTerra — extension fee handling can diverge. Details: [`docs/frontend.md` § Terra Classic gas limits](./frontend.md#terra-classic-gas-limits) and [GitLab #127](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/127).
