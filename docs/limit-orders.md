...
- **`greedy`:** optional [`GreedySwapParams`](../smartcontracts/packages/dex-common/src/pair.rs) — opt-in book-first walk (**G2**): only `max_maker_fills` + optional `book_start_hint`. Mutually exclusive with `hybrid` (**G11**). Walks live same-side makers whose net Decimal rate **strictly beats** the pool spot after fees, then residual offer → pool (**G3** / **G9**). `max_maker_fills == 0` rejects; oversize clamps to 100 (**G5**). Execute requires `belief_price` or `min_return` (**G8**). Quote via `HybridSimulation` with `greedy` set (**G7**). Official dApp does **not** use this path (solver stays **#501** / **#596**). Playbook: [`skills/AGENTS_GREEDY_BOOK_FIRST.md`](../skills/AGENTS_GREEDY_BOOK_FIRST.md).
