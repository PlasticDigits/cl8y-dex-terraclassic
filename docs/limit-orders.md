...
- **Swap empty book ≠ Trade empty book:** On Swap Settings, an empty **Book leg amount** means `book_input = 0` (full pay on the pool). On Trade market, empty means **full book leg**. Do not conflate the two.
...
- **Hybrid quote = execute ([#418](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/418)):** when a book leg is active, quotes use [`quoteDirectHybridSwap`](../frontend-dapp/src/utils/directHybridQuote.ts) (indexer hybrid solve, then LCD `hybrid_simulation` with the same split). There is **no** pool-only receive line with a configured book leg; `useHybridBook` defaults **on** on Swap and Trade market.
