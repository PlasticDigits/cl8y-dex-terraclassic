...
**Frontend hybrid gas ([GitLab #249](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/249)):** Terra Classic does not refund unused gas — the dApp sizes `Fee.gas` from the route quote’s `max_maker_fills` (makers cap per hop), not a flat **1.2M** per hop. Formula (one hop, book leg): `gasWanted = min(1_200_000, max(600_000, 550_000 + 65_000 × (max_maker_fills + 2)))`; pool-only hybrid (`book_input = 0`) uses the buffered one-hop pool envelope (**840k**). Multi-hop sums per-hop estimates; **≥ 2 hops** still use the router. **Single-hop** with a known pair uses CW20 `send` → pair `swap` (no router `SubMsg`/`reply` overhead). Unknown quote → flat **1.2M** fallback. Code: [`hybridSwapGas.ts`](../frontend-dapp/src/services/terraclassic/hybridSwapGas.ts), [`swapRouting.ts`](../frontend-dapp/src/services/terraclassic/swapRouting.ts). LocalTerra `gas_used` benchmarks: [#252](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/252).
...
| `expired_parks_used`, `expired_parks_capped`, `expired_parks_skipped` | Hybrid book walk parked ≤ **5** expired limits; skipped head count when cap reached ([#250](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/250)) |
