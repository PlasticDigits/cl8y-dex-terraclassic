...
- **Broadcast `Fee` (gas price × `gas_wanted`):** placement is **`increase_allowance`** (base gas) then **`send` → `place_limit_order`**. The dApp must attach enough **uluna** in `Fee.amount` for Station / network minimum gas price; with **`VITE_NETWORK=local`**, Station connect runs **`experimentalSuggestChain`** before and after extension connect, the cosmes **`KeplrExtension`** patch passes **`preferNoSetFee`** on each sign call and **fail-fast** post-sign fee checks on LocalTerra (amino + direct; [`extensionSignedFeeGuard.ts`](../frontend-dapp/src/utils/extensionSignedFeeGuard.ts), [`humanizeTerraTxError.ts`](../frontend-dapp/src/utils/humanizeTerraTxError.ts)), and the patched **`StationController`** uses **amino** signing when **`chainId`** matches LocalTerra (**case-insensitive**) **or** the account is Ledger (**`isNanoLedger`**) (see [`docs/frontend.md` § Terra Classic gas limits](./frontend.md#terra-classic-gas-limits), [`patches/@goblinhunt+cosmes+0.0.71-ghunt.21.patch`](../frontend-dapp/patches/@goblinhunt+cosmes+0.0.71-ghunt.21.patch)). Run **`npm ci`** so patch-package applies ([`cosmesPatch127.test.ts`](../frontend-dapp/src/services/terraclassic/__tests__/cosmesPatch127.test.ts)). **QA:** verify with **both Station and Terra Classic Keplr** on LocalTerra — extension fee handling can diverge. [GitLab #127](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/127).
