...
Implementation: [`LimitOrdersPage`](../frontend-dapp/src/pages/LimitOrdersPage.tsx), shared components under [`frontend-dapp/src/components/trade/`](../frontend-dapp/src/components/trade/) ([issue #110](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/110)). Pure helpers: [`limitOrderExpiry.ts`](../frontend-dapp/src/utils/limitOrderExpiry.ts), [`limitOrderEscrowBalanceGate.ts`](../frontend-dapp/src/utils/limitOrderEscrowBalanceGate.ts) ([GitLab #130](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/130)).
...
**Invariants (#130 escrow preflight)**

- **Raw comparison:** gate compares `toRawAmount(amountHuman, escrowDecimals)` to the LCD/CW20 balance string as **BigInt** (same units as the allowance / hook `send` amount).
- **Conservative when uncertain:** if the balance query is **loading**, **errored**, or **missing `data`**, `canPlaceLimit` is false — no allowance tx (avoids the race where the user submits before the first balance response).
- **UI + mutation:** the Place button is disabled when the gate is closed, and `mutationFn` re-evaluates the gate so a stale click cannot reach `executeTerraContract`.
- **Broadcast `Fee` (gas price × `gas_wanted`):** placement is **`increase_allowance`** (base gas) then **`send` → `place_limit_order`**. The dApp must attach enough **uluna** in `Fee.amount` for Station / network minimum gas price; with **`VITE_NETWORK=local`**, Station connect runs **`experimentalSuggestChain`** before and after extension connect, the cosmes **`KeplrExtension`** patch passes **`preferNoSetFee`** on each sign call, and the patched **`StationController`** uses **amino** signing for **`localterra`** when not on Ledger (see [`docs/frontend.md` § Terra Classic gas limits](./frontend.md#terra-classic-gas-limits), [`patches/@goblinhunt+cosmes+0.0.71-ghunt.21.patch`](../frontend-dapp/patches/@goblinhunt+cosmes+0.0.71-ghunt.21.patch)). **QA:** verify with **both Station and Terra Classic Keplr** on LocalTerra — extension fee handling can diverge. [GitLab #127](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/127).
