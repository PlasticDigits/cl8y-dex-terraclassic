...
Same-crate bump is still `MigrateMsg {}` + `cw2::ensure_from_older_version` (CMM-only; **not** the retail page). Foreign adopt is `MigrateMsg { adopt: AdoptMigrateMsg }` for honest cw2 ∈ `{crates.io:cw20-base, crates.io:cw20-mintable, crates.io:terraport-token}` and ALPHA taxed cw2 / leftover `tax_info`. S4 **wipes** `tax_info` / `tax_map` / `whale_info` (raw and cw-storage-plus), then maps 4.5%/1% → buy 450 / sell 100 when the payload is zeros. Unknown cw2 or `cfg`/`feat` smash → revert. Balances / `total_supply` unchanged. Source minter is revoked. Do **not** whitelist **8654** — listing **11619** covers the adopted address. `CONFIG.launcher` is the official launcher. `GetMigrateOrigin` is written so catalog can attest without faking `launcher_tx`. Retail `/token/migrate` is **free** (no 50 UST1) and broadcasts `MsgMigrateContract` + `MsgUpdateAdmin` → CMM.
...
| **8654** ALPHA `terra1x6e64…zysuxz` | **S4 go** (wipe; never whitelist 8654) | Terraport ALPHA/LUNC `terra12u7kh…9e7p6` (ALPHA `25732882067035` / uluna `5603001027933`); ALPHA/USTC `terra1jg2vu…wph` (uusd `54087298` / ALPHA `23466167250`). After wipe, forward 1:1; historical 4.5% skim not unwound. No GDEX factory pin in-repo. |
