...
| `UpdateConfig`             | `governance?`, `treasury?`, `default_fee_bps?`, `pair_code_id?`, `lp_token_code_id?` | Governance — factory pointer only (`treasury` applies to **new** pairs + pair-creation LUNC; live pair fees need `SetPairTreasury*`) |
