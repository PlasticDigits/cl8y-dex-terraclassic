...
Honest token upgrades: whitelist the new id → migrate instances → `RefreshPairAssetCodeIds` / `RefreshPairAssetCodeIdsBatch` (refuses unlisted live ids). **Migrate factory 1.9.0 before pair 1.15.0** so write-path queries exist. Pair migrate backfills missing pins from live `ContractInfo`. Invariant **F6**. Regression: `make verify-issue-582`. Playbook: [`skills/AGENTS_CW20_CODE_ID_PIN.md`](../skills/AGENTS_CW20_CODE_ID_PIN.md).
