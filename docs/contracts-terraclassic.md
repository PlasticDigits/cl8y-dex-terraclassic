...
`make verify-issue-592` (crates). `make verify-issue-608` (LaunchGuards cooldown / `max_wallet` liveness). `make verify-issue-610` (AutoLP factory pair + skim floor). `make verify-issue-601` (store + REPORT + LocalTerra smoke). Free listed-template adopt: [`#626`](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/626) / `make verify-issue-626`. Migrate pair inventory: [`#634`](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/634) / `make verify-issue-634` ([`AGENTS_FRONTEND_TOKEN_MIGRATE.md`](../skills/AGENTS_FRONTEND_TOKEN_MIGRATE.md)).
