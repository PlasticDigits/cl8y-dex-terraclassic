...
Same-crate bump is still `MigrateMsg {}` + `cw2::ensure_from_older_version` (CMM-only; **not** the retail page). Foreign adopt is `MigrateMsg { adopt: AdoptMigrateMsg }` and only accepts cw2 ∈ `{crates.io:cw20-base, crates.io:cw20-mintable, crates.io:terraport-token}`. `tax_map` storage, unknown cw2, or `cfg`/`feat` smash → revert. Balances / `total_supply` unchanged. Source minter is revoked. `CONFIG.launcher` is the official launcher. `GetMigrateOrigin` is written so catalog can attest without faking `launcher_tx`. Retail `/token/migrate` is **free** (no 50 UST1) and broadcasts `MsgMigrateContract` + `MsgUpdateAdmin` → CMM.
...
| **8654** ALPHA `terra1x6e64…zysuxz` | **S4 no-go** | Terraport ALPHA/LUNC `terra12u7kh…9e7p6` (ALPHA `25732882067035` / uluna `5603001027933`); ALPHA/USTC `terra1jg2vu…wph` (uusd `54087298` / ALPHA `23466167250`). Page shows **not supported**. No GDEX factory pin in-repo. |
