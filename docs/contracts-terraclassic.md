...
- Token `UpdateSettings { settings }` — one flat 50 UST1 for the whole already-activated batch. No-op / unactivated SKU / non-manager → revert, fee not kept. AutoLP no-op is sister `GetConfig` identity (not key presence); identical `minter` is also a no-op ([#1237](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1237)).
...
| **3** MIR / TWD / VKR / WHALE / KUJI / Station | **NO-GO** ([#627](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/627)) | `cw20-legacy` CanonicalAddr maps. cw2 `crates.io:cw20-base` is **not** layout proof. Crate reverts `AdoptLegacyLayout`. Do **not** append `VITE_COMMUNITY_MIGRATE_CODE_IDS` or `AddWhitelistedCodeId 3`. Report: [`codeids/3/REPORT.md`](../cw20-codeid-audits/codeids/3/REPORT.md). |
