...
| One chrome layer per region | Do **not** wrap `shell-panel` / `shell-panel-strong` / `card-glass` in another of the same family for the same visual region. Page background → one section surface → content. Nested `card-glass` inside a page `shell-panel` is OK for distinct inner blocks (Swap IO cards). First applied on `/trade` ([#561](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/561)). `/protocol` Global stats + fees metric grids use `StatBox variant="flat"` — no nested `card-glass` ([#652](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/652)). Repo-wide metric-grid pass: [#653](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/653). |
