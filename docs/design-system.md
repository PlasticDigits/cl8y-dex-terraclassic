...
| One chrome layer per region | Page `--bg-*` → **one** `shell-panel*` per region → **content**. Do **not** wrap `shell-panel*` in another `shell-panel*`, and do **not** wrap a **grid of** `card-glass` / default `StatBox` for the same region. Metric tiles use `StatBox variant="flat"` / `.stat-flat` (typography only). Nested `card-glass` is **not** a general exception — it is OK **only** on the short allowlist: Swap Pay/Receive IO cards; a **single** table/chart well; Trade **sibling** panels (not a panel-of-panels). `/trade` first applied this on the chart ([#561](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/561) **L561-2**). Repo-wide rule + dApp pass: [#653](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/653). `/protocol` Global stats + fees **inline Δ%** stays [#652](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/652); chips sit with their headline, not `justify-between` to the next column ([#667](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/667)). Total liquidity is **24h-only**; UTC volume x-axis labels every bar or every 2nd ([#677](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/677)). |
...
| Pool / Provide / Withdraw | v2 AMM LP on `/pool`. **Provide Liquidity** needs both tokens. **Zap Add** / **Zap Withdraw** use one token. LP tokens = share. |
| Zap Add / Zap Withdraw | One-sided zap under pair **Manage** (not page-level Add/Withdraw). Pair is the expanded row. |
| Use native LUNC (auto-wrap) | Provide Liquidity checkbox: bank LUNC wraps into cLUNC in the provide tx. **Defaults on** when the pair leg is wrap-equivalent (cLUNC / cUSTC). Zap Add wraps when you pick native LUNC as Token. [#661](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/661). |
