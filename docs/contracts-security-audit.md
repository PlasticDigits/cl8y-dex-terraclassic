...
| L8  | **Quotes:** pair `Simulation` / `ReverseSimulation` and router sims **without** `hybrid` are pool-only. With `hybrid`, pair **`HybridSimulation` / `HybridReverseSimulation`** and router forward/reverse ops model the limit book read-only (see `docs/limit-orders.md`); parity vs execute assumes the same chain snapshot (queries do not mutate expired head cleanup). | [pair `simulate_hybrid_swap` / queries](../smartcontracts/contracts/pair/src/contract.rs); [router sim](../smartcontracts/contracts/router/src/contract.rs) | `limit_order_tests::router_simulate_swap_hybrid_matches_pool_when_book_empty`, `hybrid_forward_sim_matches_execute_when_book_empty`, `hybrid_reverse_pool_only_matches_reverse_simulation`; [limit-orders.md](./limit-orders.md) |
