...
describe('LimitOrderExpiryField (GitLab #156)', () => {
  const noop = () => {}
  ...
  it('exposes No expiry semantics via title tooltip', () => {
    render(<LimitOrderExpiryField value={null} onChange={noop} idPrefix="test" />)
    const noExpiryBtn = screen.getByRole('button', { name: /no expiry/i })
    expect(noExpiryBtn.getAttribute('title')).toMatch(/rests indefinitely/i)
  })

  it('shows hint copy when No expiry is selected', () => {
    render(<LimitOrderExpiryField value={null} onChange={noop} idPrefix="test" />)
    expect(screen.getByText(/rests indefinitely until filled or cancelled/i)).toBeInTheDocument()
  })

  it('does not show hint copy when an expiry is set', () => {
