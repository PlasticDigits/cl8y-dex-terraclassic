...
export const RETAIL_COMBINED_ENVELOPE_FIXTURES: readonly RetailCombinedEnvelopeFixture[] = [
  ...
  },
  {
    ...
    note: 'native↔native wrap + 2-hop + unwrap_output (#587 / #599)',
  ...
  {
    id: 'send_1hop_unwrap',
    note: 'router 1-hop + unwrap_output — no unwrap combo (#599 AC4)',
    msgs: [{ msg: routerSendMsg(1, true) }],
    expectedGas: gasLimitForRouterExecuteSwapOperations(1) + UNWRAP_GAS_LIMIT,
  },
  {
    id: 'send_2hop_unwrap',
    note: 'USTR→LUNC router 2-hop + unwrap_output (no wrap msg) (#599 AC5)',
    msgs: [{ msg: routerSendMsg(2, true) }],
    expectedGas: gasLimitForRouterExecuteSwapOperations(2) + UNWRAP_GAS_LIMIT + UNWRAP_ROUTER_COMBO_OVERHEAD_GAS,
  },
  {
    id: 'send_2hop_unwrap_ustc',
    note: 'USTR→USTC taxed uusd InstantWithdraw 2-hop + unwrap_output (#599)',
    msgs: [{ msg: routerSendMsg(2, true) }],
    expectedGas: gasLimitForRouterExecuteSwapOperations(2) + UNWRAP_GAS_LIMIT + UNWRAP_ROUTER_COMBO_OVERHEAD_GAS,
  },
  {
