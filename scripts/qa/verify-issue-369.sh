#!/usr/bin/env bash
# Verification for GitLab #369: DB-hybrid route/solve must not 502 when a path
# candidate touches a zero-reserve pair but a direct viable route exists.
#
...
run_step "indexer lib: skippable_hybrid_sim_error unit test" \
  bash -c 'cd indexer && cargo test skippable_hybrid_sim_error --lib -- --quiet'

run_step "indexer integration: route_solve_db_hybrid zero-reserve skip" \
