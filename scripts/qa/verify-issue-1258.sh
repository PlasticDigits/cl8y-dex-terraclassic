...
run_step "docs: invariants + skill + AGENTS crosslinks" \
  bash -c "
    ...
    test -f indexer/migrations/20260916120001_usdt_quote_usd_null_backfill.sql
...
run_step "source: identity pin, not Peg1 / hub / CEX FDUSD / symbol arm" \
  bash -c "
    ...
    grep -q 'price_usd IS NULL' indexer/migrations/20260916120001_usdt_quote_usd_null_backfill.sql
    grep -q '${PIN}' indexer/migrations/20260916120001_usdt_quote_usd_null_backfill.sql
