...
#      (EMBER/CORAL, TOPAZ/ONYX, ONYX/CORAL) 1-unit indexer quotes are
#      near-inverse in both directions (≤5% off reciprocal; fees/spread).
#
...
echo ""
echo "[3] Live OE-1 inverse quotes (indexer route/solve, 1.0 token)"
...
if [[ -z "$CONTAINER" ]] || ! curl -sf "${INDEXER}/health" >/dev/null 2>&1; then
  ...
  PY_OUT="$(VERIFY293_INDEXER_URL="$INDEXER" VERIFY293_MAX_DEVIATION_PCT="$MAX_DEVIATION_PCT" python3 - <<'PY'
...
for a, b in PAIRS:
    try:
        ...
        if not ok_pair:
            all_pass = False
        hops_fwd = len(fwd.get("router_operations") or [])
        hops_rev = len(rev.get("router_operations") or [])
        lines.append(
            f"{a}/{b}: fwd={hf:.6f} rev={hr:.6f} expected_rev={expected_rev:.6f} "
            f"dev={dev_pct:.2f}% hops={hops_fwd}/{hops_rev} {'PASS' if ok_pair else 'FAIL'}"
        )
...
PY
  ...
  echo "$PY_OUT" | sed 's/^/    /'
  if echo "$PY_OUT" | grep -q 'ALL_PASS=true'; then
    ok "OE-1 hub pairs near-inverse (≤${MAX_DEVIATION_PCT}% deviation, swarm running)"
  else
    bad "OE-1 hub pairs not near-inverse with swarm (see lines above)"
  fi
