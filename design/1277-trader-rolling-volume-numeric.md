...
**Revision:** 2026-09-20-r7 (pins: #553 SQL by function/text not line numbers; shared `&str` allowed, keep `&PgPool` signature; poller `if let Err` like #1269/#676 does not abort `run_indexer`)  
...
| #553 USD after heal | Same migration file **and** same poller transaction, after leftover-zero: SQL **text** of `refresh_trader_total_volume_usd` (pin the function, not line numbers), then leftover-USD `NULL`. Required, not optional. Unpriced stays NULL (**R5**). Heal INSERT/UPDATE/leftover-zero still do not `SET` USD (**V1277-3**). Do not change P522-Q for senders who still have priced swaps. A shared SQL `&str` with the pool helper is allowed; keep `refresh_trader_total_volume_usd(pool: &PgPool)`. |
...
| Poller startup | In `indexer/src/indexer/poller.rs`, call `heal_trader_lifetime_from_swaps` **before** `refresh_all_volume_windows(..., true)` (the `#1269` slot, after `backfill_missing_swap_amm_fees`). Wrap like `#1269` / `#676`: `if let Err(e) = … { tracing::error!(…) }` — do **not** abort `run_indexer`. After a failed heal, **D5** still runs (UPDATE-only; skip-class wallets stay off the board until the next boot’s gate). Gate retries next boot; a later heal `UPDATE` recomputes from `swap_events`. Do **not** copy the `#676` slot after refresh — that leaves post-deploy skip-class wallets off `sort=volume_24h` until the 5-minute loop. Mismatch-gated (`COALESCE` like `#676` `positions_trade_count_diverges`: missing sender **or** `total_trades` / capped `total_volume` diverge **or** leftover-lifetime with no remaining swaps). Gate is **raw-lifetime only** — do **not** expand it to `total_volume_usd` on this ticket (USD-only skew with matching raw totals is migrate leftover-USD plus later catalog refresh). Registered / zero-lifetime ghosts do **not** trip. When the gate fires, one transaction: INSERT + lifetime UPDATE + leftover-zero + priced USD + leftover-USD `NULL`. Priced USD is the **SQL text** of `traders::refresh_trader_total_volume_usd` (pin the **function**, not line numbers — they move when heal helpers land in `traders.rs`) on `&mut Transaction<'_, Postgres>`. A shared SQL `&str` used by that pool helper and the heal copy is **allowed** (preferred vs two literals). **Do not** change `refresh_trader_total_volume_usd(pool: &PgPool)`. `volume.rs` only *calls* that helper; it does not contain the trader USD `UPDATE`. **Do not** call `refresh_trader_total_volume_usd(&pool)` inside the helper (`.execute(pool)` auto-commits on another connection) **or** after that commit (crash window). Do not move the pool helper onto `&mut Transaction`. Catalog `volume.rs` keeps the pool-level function for non-heal callers. |
...
-- Required #553 stamp. Copy the SQL text of traders::refresh_trader_total_volume_usd
-- (function in traders.rs; do not pin line numbers — they move when heal helpers land).
-- volume.rs only calls the helper. A shared SQL &str with the pool helper is allowed;
-- do not change refresh_trader_total_volume_usd(&PgPool).
...
Rust `heal_trader_lifetime_from_swaps` **copies** INSERT + lifetime UPDATE + leftover-zero + leftover-USD **and** the #553 priced SQL onto the **same** `sqlx` transaction (`&mut Transaction<'_, Postgres>`). Pin priced SQL by the **function** `traders::refresh_trader_total_volume_usd` and its SQL **text**, not line numbers (they move when heal helpers land in `traders.rs`). A shared SQL `&str` used by that pool helper (`.execute(pool)`) and the heal copy (`.execute(&mut *tx)`) is **allowed** and preferred to two drifted literals. **Do not** change `refresh_trader_total_volume_usd(pool: &PgPool)`. `volume.rs` only *calls* that helper; it does not contain that `UPDATE`. Keep-in-sync comment *pattern* may still cite `volume.rs` `backfill_swap_volume_usd` / #548. sqlx migrations cannot call Rust. Live helper is `pool: &PgPool` + `.execute(pool)` — **do not call it from this helper** (would leave the heal `Transaction`). Do **not** move that pool helper onto `&mut Transaction`; catalog / `volume.rs` still uses it for non-heal priced-sender refresh (formula unchanged). Do **not** `SET` USD in INSERT / lifetime UPDATE / leftover-zero. Do **not** extract a `&PgPool` from the helper and pass it to `refresh_trader_total_volume_usd`.
...
Migration always runs heal (one-shot after `ALTER`; one sqlx file = one transaction). Poller uses the gate; when it fires, `heal_trader_lifetime_from_swaps` runs the five statements in **one transaction**. Crash rolls back; gate still fires next boot. Poller wraps the call in `if let Err` like `#1269` / `#676` — do **not** abort `run_indexer`. Then `refresh_all_volume_windows(..., true)` (**D5**) still runs even if heal logged an error.
...
| Call `refresh_trader_total_volume_usd(&pool)` inside `heal_trader_lifetime_from_swaps` | Live helper is `pool: &PgPool` + `.execute(pool)` — another connection, auto-commit. A later rollback of the heal `Transaction` (or a crash before leftover-USD) splits USD from INSERT/lifetime/leftover-zero. Use the #553 SQL **text** on `&mut Transaction` (shared `&str` with the pool helper is allowed). Catalog `volume.rs` keeps the pool-level function. |
| Copy #553 SQL from `volume.rs` | `volume.rs` does not contain it; it only *calls* `traders::refresh_trader_total_volume_usd`. Source is that function’s SQL **text** (do not pin line numbers). Do not move the pool helper. |
| Two drifted #553 SQL literals with no shared `&str` | Allowed (literal copy still matches r6). Prefer a shared SQL `&str` used by `refresh_trader_total_volume_usd(pool: &PgPool)` and the heal `Transaction` so the formula cannot drift. Do **not** change the pool helper signature. |
...
| Abort `run_indexer` on heal `Err` | Forbidden. `#1269` backfill and `#676` rebuild use `if let Err` log and continue. Heal is the same skip-class leftover; a transient SQL error must not crash-loop Coolify. Gate retries next boot; a later heal `UPDATE` recomputes from `swap_events`. **D5** still runs after a failed heal. |

## Complexity added / removed

**Added:** one migration (`ALTER` + heal including leftover-zero + #553 USD + leftover-USD `NULL`), Rust copy of heal SQL with keep-in-sync comment (optional shared #553 `&str`), `COALESCE` mismatch-gate, one `LEAST` on lifetime raw add, JSON helper swap, I10 + **A1** fixtures, poller call in the #1269 slot (`if let Err` like #1269/#676), one heal transaction.  
...
| Helper calls `refresh_trader_total_volume_usd(&pool)` | Forbidden. Use the #553 SQL **text** on `&mut *tx` (shared `&str` allowed). Pool-level execute is not on the heal `Transaction`. Do not change the pool helper signature. |
...
| Heal SQL error at poller startup | `if let Err` log like `#1269` / `#676`; do **not** abort `run_indexer`. **D5** still runs. Gate still fires next boot. A later heal `UPDATE` recomputes from `swap_events`. |
...
3. **`heal_trader_lifetime_from_swaps` + `trader_lifetime_diverges_from_swaps`** in `traders.rs`. Migration SQL is canonical; Rust copies it with a keep-in-sync comment (`backfill_swap_volume_usd` *pattern*). Pin priced USD by `traders::refresh_trader_total_volume_usd` SQL **text** (not `volume.rs`, not line numbers). A shared SQL `&str` with the pool helper is allowed; **do not** change `refresh_trader_total_volume_usd(pool: &PgPool)`. Gate is the `COALESCE` SQL above — **raw-lifetime only**. Helper opens **one** transaction: INSERT + lifetime UPDATE + leftover-zero + priced USD (**text** on `&mut Transaction`, **not** `refresh_trader_total_volume_usd(&pool)`) + leftover-USD `NULL`; then commit. Do not move the pool helper. Poller calls the helper **before** `refresh_all_volume_windows(..., true)` in `poller.rs` with `if let Err` log (same as `#1269` / `#676`) — do **not** abort `run_indexer`. Poller does not chain a second pool-level USD call.
