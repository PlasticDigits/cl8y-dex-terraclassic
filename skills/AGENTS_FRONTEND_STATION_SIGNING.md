...
6b. **Station mobile / WC auto-gas ([#679](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/679)):** Atomic `broadcastTx` / Station `post` still receives the dApp `Fee.gas` + `Fee.amount`. Station mobile **Confirm Transaction** may ignore that and LCD-simulate a 4-hop hybrid book walk near block gas (~147M / ~4,158 LUNC). That is a **wallet-app residual**, not a reason to LCD-sim in the dApp or to re-enable SEC-E08 on columbus-5 (a “reject if signed fee ≫ expected” guard would brick Station mobile if it still rewrites upward). Keep Station/LuncDash WC atomic (`isAtomicWalletConnectPost`). The Swap **Network fee (est.) ~X LUNC** row is the dApp source of truth (~200 LUNC class for the live mixed 4-hop). Extension `preferNoSetFee` + amino should match that envelope when honored. Verify: `make verify-issue-679`. Post-fix census (Stay): [ADR 0004](../docs/adr/0004-terraclassic-retail-gas-census.md) / [#1222](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1222) (`make verify-issue-1222`).
...
- Gas limits / fee envelope: [`AGENTS_TERRACLASSIC_GAS.md`](./AGENTS_TERRACLASSIC_GAS.md) — LocalTerra wallet matrix ([#235](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/235)); mixed hybrid router hops + Station mobile auto-gas residual ([#679](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/679), **G-AUTO-8**); remaining-gap census **Stay** ([#1222](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1222), [ADR 0004](../docs/adr/0004-terraclassic-retail-gas-census.md))
