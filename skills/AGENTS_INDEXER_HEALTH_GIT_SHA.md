...
- **Don’t** `UPDATE _sqlx_migrations SET success = true`. Dirty is a **pre-step**: `DELETE FROM _sqlx_migrations WHERE version = <v> AND success = false` only; then **re-enter the three-way** tree (not forward-fix only).
