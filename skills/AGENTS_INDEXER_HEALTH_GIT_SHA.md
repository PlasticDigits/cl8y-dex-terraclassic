...
- **Don’t** `UPDATE _sqlx_migrations SET success = true`. Dirty is a **sequential gate**: any `success=false`? → Coolify **Stop** / scale-to-zero (auto-deploy off is **not** a process stop) → `DELETE FROM _sqlx_migrations WHERE success = false` (**every** `success=false` row); then **re-enter the three-way** tree (not forward-fix only; mixed dirty+ahead uses the same every-ahead-version **2(c)** gate).
...
- **Don’t** treat **2(b)** (hotfix, no ledger surgery) as needing Stop.
