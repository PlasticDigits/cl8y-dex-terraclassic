...
| **D5** | Indexer **startup** (`poller` → `refresh_all_volume_windows(..., true)`) refreshes token + trader + pair + global **without** waiting 5 min. Loop still sleeps 300s first. [#1277](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1277) `heal_trader_lifetime_from_swaps` runs in `poller.rs` **before** that call (#1269 slot; `if let Err` log like #1269/#676 — do not abort `run_indexer`) so skip-class senders exist for the **D5** stamp. A failed heal still lets **D5** run. Leftover-lifetime ghosts with no remaining `swap_events` are zeroed in that heal tx (**not** **D2**); idle traders who still have old swaps keep lifetime. |
