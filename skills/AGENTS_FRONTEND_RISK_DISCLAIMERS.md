...
6. **Environment ribbon opacity / seam ([#482](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/482)):** keep the strip under the sticky header; do not remove it or bury network risk copy behind a menu. Ribbon CSS must layer panel background + network tint (and `.app-top-sticky` opaque stack bg) so Trade/page subtitles cannot read through while scrolling. See [`AGENTS_FRONTEND_RESPONSIVE_HEADER.md`](./AGENTS_FRONTEND_RESPONSIVE_HEADER.md).
7. **NetworkBadge vs ribbon ([#483](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/483)):** on ≥768px, do not reintroduce a duplicate header `NetworkBadge` next to theme/wallet — the ribbon owns that signal.
8. **Menus above ribbon ([#486](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/486)):** open More / wallet dropdowns must paint above the ribbon; fix via header-vs-ribbon stacking inside `.app-top-sticky`, not by narrowing or hiding the strip.
9. **Safety > brevity ([#489](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/489)):** keep required first-visit ack and footer NFA; do not paste duplicative NFA walls next to every CTA. On-card copy minimization is separate — see [`AGENTS_FRONTEND_COPY_COGNITIVE_LOAD.md`](./AGENTS_FRONTEND_COPY_COGNITIVE_LOAD.md).
