...
5. **Automation / bots:** poll **`parked_expired`** (indexer or LCD **`ExpiredLimitRefund`**) and confirm the pair is **not** paused before broadcasting claim — prefer batch when many rows; see [`AGENTS_LOCALNET_TRADING_SWARM.md`](./AGENTS_LOCALNET_TRADING_SWARM.md) cross-links. **On-chain vaults / contracts** that need typed custody for an `order_id` (Active / ParkedRefund / Unknown) should use pair **`OrderStatus`** ([GitLab **#505**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/505)) — see [`AGENTS_ORDER_STATUS_QUERY.md`](./AGENTS_ORDER_STATUS_QUERY.md); do not treat `Unknown` as proof of fill.
