...
3. **Deploy key** pays gas; **multisig** is `--admin` / governance (**SL4**). Factory **treasury** is the ustr-cmm CMM (`terra16j5u6…`), not the multisig. Host txs use `--gas-prices=28.325uluna` (not a flat fee). Keyring auto-detects `file` when `~/.terra/keyring-file/<key>.info` exists (avoids bech32-of-name errors from wrong `--keyring-backend os`).
...
- [`AGENTS_UST1_WRAP_PRODUCTION_OPS.md`](./AGENTS_UST1_WRAP_PRODUCTION_OPS.md) — Phase 5 UST1/wrap ops registry + pause playbooks (#503); `make verify-issue-503`
