...
| **H6c** | Deterministic CosmWasm smart-query HTTP **500** → `ContractQueryRejected` (DEBUG, **no** sibling-endpoint fan-out). Classic phrases (`pair not found`, `unknown variant`, `error parsing into type`) plus, on `/cosmwasm/wasm/v1/contract/...` only, wasmd markers `query wasm contract failed` / `codespace wasm` ([#493](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/493)). Infra **429/408/502+** still cool down + fail over; plain infra **500** bodies without wasm markers still fan out (WARN). | [`lcd/mod.rs`](../indexer/src/lcd/mod.rs) `is_deterministic_contract_query_reject`; wiremock: `wasm_query_reject_does_not_fan_out`, `infra_500_still_fans_out`, `gateway_502_cools_down_and_fails_over` |
