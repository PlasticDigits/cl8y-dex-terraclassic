...
- **Post-sign fee guard (extension undershoot):** [GitLab **#371**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/371) — map **`EXTENSION_SIGNED_FEE_UNDERSHOOT_PREFIX`** throws to **`EXTENSION_SIGNED_FEE_USER_MESSAGE`** in **`tryHumanizeTerraTxMessage`**; do not surface GitLab refs, **`uluna`**, **`npm ci`**, or Station-only steps in UI. Guard/patch throws keep diagnostics for **`console.error`**.
