...
3. **Production `npm run build`:** `vite.config.ts` **fails the build** if `VITE_DEV_MNEMONIC` is set for any non-`development` mode unless `VITE_ALLOW_DEV_MNEMONIC=local-only` (see GitLab #118, [#378](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/378)).
4. **Production WC ID:** `vite build --mode production` **requires** `VITE_WC_PROJECT_ID` — no shared default in `wallet.ts` ([#378](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/378)).
5. **Production `VITE_DEV_MODE`:** `vite build --mode production` **fails** if `VITE_DEV_MODE=true`. Unset it in Coolify / `.env.production`. `VITE_ALLOW_DEV_MNEMONIC=local-only` does not permit the flag. LocalTerra / Vitest / Playwright keep `VITE_DEV_MODE=true` ([#695](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/695) / FE-01). See [`AGENTS_FRONTEND_DEV_MODE_GUARD.md`](./AGENTS_FRONTEND_DEV_MODE_GUARD.md).
6. **E2E / Playwright:** `playwright.config.ts` injects the mnemonic from `docker/init-chain.sh` into the `webServer` process env (not from frontend source). On-chain limit place/cancel: [`AGENTS_E2E_LIMIT_ORDERS_TX.md`](./AGENTS_E2E_LIMIT_ORDERS_TX.md) ([#195](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/195)).
7. **Gitleaks** default rules do not reliably flag multi-word BIP39 phrases; this repo adds a path-scoped custom rule (`bip39-like-phrase-frontend-src`). The pre-commit hook uses `gitleaks protect --staged -c .gitleaks.toml` (blocks new violations). **GitLab CI** runs mandatory incremental `gitleaks detect` via [`scripts/ci/gitleaks-detect.sh`](../scripts/ci/gitleaks-detect.sh) ([#380](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/380)). A full `gitleaks detect` over **all git history** may still list old commits that predated the fix; use `gitleaks detect --no-git` for a working-tree-only check. Do not allowlist `frontend-dapp/src` for this pattern without a security review.
...
- **Connect modal (extension detection + layout):** [docs/frontend.md § Connect modal: extension install detection](../docs/frontend.md#connect-modal-extension-install) · [`AGENTS_FRONTEND_WALLET_CONNECT_MODAL.md`](./AGENTS_FRONTEND_WALLET_CONNECT_MODAL.md) · [GitLab #139](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/139) · [GitLab #160](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/160) · Leap removed ([GitLab #159](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/159))
