...
| **`E2E_INDEXER_OUTAGE=1`** | Specs `test.skip` without it — default `npm run test:e2e` unchanged |
| **Indexer bound to `127.0.0.1:3001`** | Stop only this process in the job/script |
