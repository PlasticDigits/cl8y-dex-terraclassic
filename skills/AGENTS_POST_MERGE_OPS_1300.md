...
**Parents (closed unless a merged invariant is wrong):** #1286/#1287, #1234, #1265, #1240, #1277, #1285, #1255, #1219, #1218, #1263, **#1264** (AC1 measured; do **not** reopen). **Stay open:** [#1279](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1279) (ops-bot 1.5). Tracker-close comments on #1300 do **not** waive the four walk notes.
...
`make verify-issue-1300` does **not** execute these walks. Default leftover verify probes `make has-localterra` (down → SKIP; up → PASS the probe). `VERIFY1300_REQUIRE_HAS_LOCALTERRA=1` is fail-closed `make has-localterra` only. **`VERIFY1300_SKIP_HAS_LOCALTERRA=1`** skips the probe (do **not** ship `SKIP_HAS_LOCALTERRA`). Record the four how-tos on #1300, **or keep #1300 open**. Tracker-close comments on #1300 do **not** waive those four notes. `has-localterra` down is **not** leftover-complete. Slice 2 leftover implement does **not** provision LocalTerra or record walks. Optional: Cloud Agent leftover-*ops* may `make setup-cloud-localterra` when item 5 is still open; that is **not** implement-slice close. Do **not** close on probe SKIP. Chain-probe SKIP/PASS is not item-5 evidence.
