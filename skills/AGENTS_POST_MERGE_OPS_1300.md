...
| **M1300-3** | Coolify **frontend rebuild** from `6d34da13+`. Production HTTP marker: **`protocol-top-pairs` / `Top pairs (30d)`** (hashed Vite chunk grep, same class as #701). Do **not** grep hub `cLUNC / USD` (that is **#1305 leftover-complete**). LocalTerra / manual (**keep issue item 5** as leftover-complete operator evidence; how-tos below): **#1218** Swap LUNC→USTR Route wrap-then-cUSTC; **#1255** Create Token 18-dec unlisted factory CW20 then type `1` / Max; **#1219** dust ladder copy / Place disabled, no wallet popup; **#1263** LocalTerra `/protocol` section renders, ≤5 rows (Coolify-only: `?limit=6` → 400). Child Vitest is **not** the walk. Close #1300 only after those four **or** explicit SKIP (`make has-localterra` down **and** REQUIRE_CHAIN unset). |
...
`make verify-issue-1300` does **not** execute these walks. `VERIFY1300_REQUIRE_CHAIN=1` is fail-closed `make has-localterra` only. Record on #1300, **or** explicit SKIP when `has-localterra` is down and REQUIRE_CHAIN is unset.

| Walk | How-to | Pass |
|------|--------|------|
| **#1218** | Swap Pay LUNC / Receive USTR (LocalTerra + `make dev`). | Route shows wrap-then-cUSTC (wrap prefix + solver hops), not BFS 2-hop (**H1218-8**). Child `VERIFY_ISSUE_1218_CHAIN=1` is wrap-swap E7/E8, not this walk. |
...
- LocalTerra / manual (issue item 5): the four walks recorded per how-tos above, **or** explicit SKIP because `make has-localterra` is down **and** `VERIFY1300_REQUIRE_CHAIN` is unset. Child Vitest is **not** the walk. `VERIFY1300_REQUIRE_CHAIN=1` is fail-closed `has-localterra` only — does not execute the walks, not invented five-worker e2e. `VERIFY1300_LEFTOVER_E2E=1` stays SKIP until named (`REQUIRE_CHAIN` is not that flag).
...
- **Don’t** invent leftover Playwright five-worker coverage (`VERIFY1300_LEFTOVER_E2E=1` SKIP until named). `VERIFY1300_REQUIRE_CHAIN=1` is fail-closed `has-localterra` only — not that e2e flag and not “run the four walks”.
...
`VERIFY1300_SKIP_CHAIN=1` skips the `has-localterra` probe (not walks make never runs). `VERIFY1300_REQUIRE_CHAIN=1` fail-closes that probe. `VERIFY1300_LEFTOVER_E2E=1` stays SKIP until named.
