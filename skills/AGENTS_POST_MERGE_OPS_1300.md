...
- **Do** `make setup-cloud-localterra` when issue item 5 is still open and `has-localterra` is down, then record the four walks. Do **not** close on probe SKIP.
...
- **Don’t** invent leftover Playwright five-worker coverage (`VERIFY1300_LEFTOVER_E2E=1` SKIP until named). `VERIFY1300_REQUIRE_CHAIN=1` is fail-closed `has-localterra` only — not that e2e flag and not “run the four walks”. Do **not** invent named make commands for the walks.
...
# item 5 still open and chain down — provision, then record the four walks on #1300
# (make never executes the walks; probe SKIP is not leftover-complete)
