...
make verify-issue-1290  # #1290 bundle: #1240 + hubPriceTicker Vitest + Playwright protocol-page
```
...
- Post-merge leftover: [#1305](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1305) / `make verify-issue-1305` (implement) / [`AGENTS_POST_MERGE_OPS_1305.md`](./AGENTS_POST_MERGE_OPS_1305.md) — leftover-1 is Coolify glance hub `cUSTC / USD` + `cLUNC / USD` vs CEX `USTC` / `LUNC` / `vFDUSD` / click **vFDUSD** selected not `VFDUSD` (idle `/protocol` is USTC; not HTTP `cLUNC`); quote-agnostic HTTP (`cLUNC wrap` / `cLUNC` / `cUSTC` / `vFDUSD`, never source `'`) is supporting rebuild-presence; child #1290 E2E skipped by default; #703 `YY-MM` ticks are not leftover-complete
