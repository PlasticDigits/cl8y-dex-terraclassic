...
- Post-merge leftover: [#1305](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1305) / `make verify-issue-1305` / [`AGENTS_POST_MERGE_OPS_1305.md`](./AGENTS_POST_MERGE_OPS_1305.md) — Coolify glance hub `cUSTC / USD` + `cLUNC / USD` vs CEX `USTC` / `LUNC` / `vFDUSD`; quote-agnostic HTTP (`cLUNC wrap` / `cLUNC` / `cUSTC` / `vFDUSD`, never source `'`); child #1290 E2E skipped by default; #703 `YY-MM` ticks are not leftover-complete
