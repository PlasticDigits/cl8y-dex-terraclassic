...
- **Don't** nest `card-glass` per row in `protocol-top-pairs`, rank gems, or compute vol/LP from 24h `/pairs` in the browser ([#1263](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1263)).

## Invariants (P1263 — top-5 30d pairs)

Census ranking of factory pools on `/protocol`. Not `/pool`, not `/charts`, not a vote, not a farm.

| ID | Rule |
|----|------|
| **P1263-1** | Sibling `shell-panel` `protocol-top-pairs` **after Global stats, before Protocol fees**. Flat table (overflow-x OK). No `card-glass` per row. Do not nest inside `protocol-global-stats`. |
| **P1263-2** | Title **Top pairs (30d)**. Lead: trailing 30-day volume vs current pool USD. Columns: Pair (symbols) \| 30d vol (USD) \| v2 LP (USD) \| Vol/LP (`N×` or **—**). Trailing window, not calendar month. Not 24h Vol reused as 30d. |
| **P1263-3** | At most **five** rows from `GET /api/v1/protocol/top-pairs`. Ranked by trailing 30d priced USD. Gems (`COLUMBUS5_GEM_ADDRESSES`) never appear. Idle 0 does not pad. Empty → **No pairs yet**. |
| **P1263-4** | Ratio is indexer `volume_per_tvl`. Missing / ≤0 / hostile / Inf → **—** (`formatVolumePerTvl`). Compact multiplier, never a percent. |
...
## Pending UI extension (P1317)

The shipped table remains the four #1263 columns. #1317 is open; the UI must not imply that CMM-weighted LP, fee bps, or turnover exists until the indexer stamps and API fields land. Backend rules are in [`AGENTS_INDEXER_PROTOCOL_TOP_PAIRS.md`](./AGENTS_INDEXER_PROTOCOL_TOP_PAIRS.md) (**I1317-1–I1317-5**); system row: [`docs/indexer-invariants.md`](../docs/indexer-invariants.md).

| ID | Required invariant before #1317 can close |
|----|--------------------------------------------|
| **P1317-1** | Keep the #1263 top-five set/order, gem exclusion, and current full-pool v2 LP + Vol/LP cells unchanged. |
| **P1317-2** | Add CMM v2 LP, 30d fee bps, and Vol/CMM LP after Vol/LP. Use only additive API values `cmm_lp_usd_30d`, `trading_fees_usd_30d`, `fees_bps_per_cmm_lp`, and `volume_per_cmm_lp`; old indexers/missing fields render em dashes. |
| **P1317-3** | Label the CMM denominator as time-weighted trailing-30d LP. Explain that fee bps excludes wrap/unwrap and UST1 mint/redeem, and Vol/CMM LP is a `×` multiple, not a percent or full-pool Vol/LP. |
| **P1317-4** | Preserve horizontal scrolling at narrow widths, with no column overlap. Census copy stays neutral: no APR/farm/points or incentive claim; see separate [#1207](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1207) and existing LP guidance [#558](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/558). |
| **P1317-5** | Add page regressions for all three headers/cells, null dashes, unchanged Vol/LP, legacy payload compatibility, tooltip scope, and the existing phone width. A passing #1263 test alone does not verify this extension. |

## Invariants (PFee — GitLab #586)

| ID | Rule |
|----|------|
| **PFee-1** | Fee panel `protocol-fee-stats` sits **after** Top pairs (`protocol-top-pairs`, [#1263](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1263)) and **before** DEX hub. Do not merge factory/router into fees. Do not headline `traders.total_fees_paid` (lifetime mixed-unit, includes spread). |
...
| **PFee-7** | P522-Q / hub catalog **plus factory economic marks** (CL8Y and later listed non-gems) by **contract** ([#683](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/683) **EFee-1–EFee-8**). Never vFDUSD. Never `$1` UST1 / CL8Y or `2.5×` USTR. Stamp `fee_usd` at ingest — do not rewrite non-null stamps from the live mark (#568). NULL-only backfill is as-of. |
...
make verify-issue-1269  # multihop AMM hop uniqueness
make verify-issue-683   # CL8Y / economic fee USD
...
make verify-issue-1290  # leftover #1306 (Q22 / B1290; aliases verify-issue-1302 / verify-issue-1306)
```
...
- [`AGENTS_INDEXER_PROTOCOL_FEE_HOPS.md`](./AGENTS_INDEXER_PROTOCOL_FEE_HOPS.md) — pair-scoped `swap_amm` unique (**F1269-1–F1269-8**, [#1269](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1269))
...
- [`AGENTS_INDEXER_PROTOCOL_TOP_PAIRS.md`](./AGENTS_INDEXER_PROTOCOL_TOP_PAIRS.md) — `GET /protocol/top-pairs` stamp + gem exclude + 60s/400 (**I1263**, [#1263](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1263))
