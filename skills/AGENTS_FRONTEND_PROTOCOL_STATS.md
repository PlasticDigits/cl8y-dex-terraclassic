...
| **P550-9** | Path `vfdusd` polls CEX **FDUSD** (`first-digital-usd` / `FDUSDUSDT`). Indexer logs/API `display_name` is **FDUSD/USD**, not `vFDUSD/USD` ([#580](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/580)). Not `$1`. Not USTC/LUNC ids. No `fdusd` path alias. Protocol Venus 1 vFDUSD Price is [#571](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/571). |
