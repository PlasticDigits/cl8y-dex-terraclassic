...
4. **Styling:** Buy/Sell **side** controls use **`side-control` + `side-buy-*` / `side-sell-*`** (semantic fills, GitLab **#563**) — **not** `tab-glass-active`. Limit vs Market tabs and other segmented chrome stay **`tab-glass*`**. Keep **`:focus-visible`** on `.side-control` per [`AGENTS_FRONTEND_A11Y_FOCUS.md`](./AGENTS_FRONTEND_A11Y_FOCUS.md). Do **not** paint `btn-primary` Place/Market green or red.
