...
4. **Styling:** Buy/Sell **side** controls use **`side-control` + `side-buy-*` / `side-sell-*`** (semantic fills, GitLab **#563**) — **not** `tab-glass-active`. `/trade` Limit vs Market tabs are compact underlined text ([#693](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/693)), not **`tab-glass*`**; slippage chips still use **`tab-glass*`**. Keep **`:focus-visible`** on `.side-control` per [`AGENTS_FRONTEND_A11Y_FOCUS.md`](./AGENTS_FRONTEND_A11Y_FOCUS.md). Do **not** paint `btn-primary` Place/Market green or red.
...
- Ticket heading + Buy/Sell colors: [`AGENTS_FRONTEND_TRADE_TICKET_HEADING.md`](./AGENTS_FRONTEND_TRADE_TICKET_HEADING.md) ([#563](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/563))
- Ticket flatten (no Side chrome): [`AGENTS_FRONTEND_TRADE_TICKET_FLATTEN.md`](./AGENTS_FRONTEND_TRADE_TICKET_FLATTEN.md) ([#693](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/693))
