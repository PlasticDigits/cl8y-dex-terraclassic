...
| `frontend-dapp/src/test/lightweightChartsJsdomMock.ts` | Vitest double for `createChart` / series APIs (default `npm run test:run`) |
| `frontend-dapp/vitest.config.charts.ts`, `src/test/chartsSetup.ts`, `src/test/chartTestFixtures.ts` | Real **lightweight-charts** in Node (`npm run test:charts`, [#211](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/211)) |
| [docs/testing.md § Real lightweight-charts in Vitest](../docs/testing.md#real-lightweight-charts-in-vitest-gitlab-211) | Stub vs real-library split, CI, commands |
...
7. **Tests:** Default Vitest uses **`lightweightChartsJsdomMock`** for fast React/indexer wiring — extend it when adding series APIs (`createPriceLine`, `priceScale`, etc.). Library-backed init, `setData`, indicators, volume fallback, and autoscale belong in **`*.charts.test.{ts,tsx}`** (`npm run test:charts`, [#211](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/211)). Prefer **`data-testid`** over canvas pixel assertions; use real canvas only where the library API is under test.
