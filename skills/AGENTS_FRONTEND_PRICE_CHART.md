...
10. **Timeframe switches ([#148](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/148)):** Candles query uses **`placeholderData: keepPreviousData`** so interval changes do **not** unmount the canvas. Only the **first** load shows **Loading chart…**; refetches show **`price-chart-interval-loading`**. **`createChart` runs once** per canvas mount; interval updates use **`setData`**. Guard stale async inits with **`chartInitIdRef`** in `PriceChartLightweightCanvas`. Regression: `PriceChart.test.tsx` — *reuses one chart instance across many interval switches*.
