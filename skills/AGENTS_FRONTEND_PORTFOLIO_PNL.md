...
| **P551-5** | **Do not** render `traders.total_realized_pnl`, `total_volume`, `total_fees_paid`, `best_trade_pnl`, or `worst_trade_pnl` via `formatNum`. Volume / fees / best / worst → **—** (mixed tokens). Header realized P&L = **USD sum** of per-pair P&L using P522-Q on the **base** token. Unpriced pairs are omitted, not `$0`. Empty positions → `$0`. |
