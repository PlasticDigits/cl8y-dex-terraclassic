...
| Recovery script | [`scripts/indexer-reorg-recover.sh`](../../scripts/indexer-reorg-recover.sh) (`make indexer-reorg-recover`) |
| Reorg alerting | [`indexer/src/indexer/reorg_alert.rs`](../../indexer/src/indexer/reorg_alert.rs) — `REORG_ALERT_WEBHOOK_URL` |
...
```bash
./scripts/indexer-reorg-recover.sh --height FORK_HEIGHT                    # dry-run + row preview
./scripts/indexer-reorg-recover.sh --height FORK_HEIGHT --cleanup-derived  # preview derived deletes
./scripts/indexer-reorg-recover.sh --height FORK_HEIGHT --cleanup-derived --apply
# or: make indexer-reorg-recover HEIGHT=FORK_HEIGHT CLEANUP=1 APPLY=1
# restart indexer
```

Shallow vs deep reorg: [`docs/runbooks/indexer-reorg-replay-dedup.md`](../docs/runbooks/indexer-reorg-replay-dedup.md).

Query failed blocks: `SELECT * FROM indexer_failed_blocks ORDER BY height;`

## Do not regress

- Swap dedup `(tx_hash, pair_id, swap_index)` on replay ([#287](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/287)).
