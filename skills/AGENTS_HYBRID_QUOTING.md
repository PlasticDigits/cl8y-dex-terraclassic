...
6. **Frontend hybrid quote = execute ([GitLab #418](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/418), [#501](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/501)):**
   ...
   - Best-execution hybrid defaults **on** (Swap Settings / Trade market Advanced). Empty manual book ≠ “skip the book”: GET may still allocate an interior split. Trade market **hybrid off** remains pool-only. Integrators may pass `pool_only=true` on indexer GET for explicit pool quotes.
6b. **Empty manual book + Execution notice ([GitLab #492](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/492), [#501](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/501)):** Swap and Trade market Advanced both treat empty book as **no caller-declared override** (`getDirectHybridBookSplit` → `book_input 0` for POST). When hybrid Settings are on and the manual book is empty, **show no** “add a book leg” Execution copy — [`getDirectHybridSettingsExecutionSummary`](../frontend-dapp/src/utils/swapDisclosure.ts). Prefer silence ([`AGENTS_FRONTEND_COPY_COGNITIVE_LOAD.md`](./AGENTS_FRONTEND_COPY_COGNITIVE_LOAD.md)).
