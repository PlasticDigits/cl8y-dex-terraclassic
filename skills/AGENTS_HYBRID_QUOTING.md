...
6. **Frontend hybrid quote = execute ([GitLab #418](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/418)):** When submit includes a positive book leg or indexer hybrid split, the receive line must use indexer `POST /route/solve` and/or wallet LCD `hybrid_simulation` with the **same** `pool_input` / `book_input` / `max_maker_fills` — never pool-only `simulateSwap` while hybrid submit is configured. Shared helper: [`directHybridQuote.ts`](../frontend-dapp/src/utils/directHybridQuote.ts). **Indexer branch must reconcile receive + slippage to wallet `hybrid_simulation`** (GitLab [#471](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/471), same as multi-hop). Swap `useHybridBook` defaults **on**; Trade market hybrid checkbox defaults **on**. Integrators may still pass `pool_only=true` on indexer GET for explicit pool quotes.
6b. **Swap Execution notice ([GitLab #492](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/492)):** Swap empty book input = `book_input 0` (Trade empty = full book). When hybrid Settings are on and the manual book is empty, **show no** “add a book leg” Execution copy — [`getDirectHybridSettingsExecutionSummary`](../frontend-dapp/src/utils/swapDisclosure.ts). Prefer silence ([`AGENTS_FRONTEND_COPY_COGNITIVE_LOAD.md`](./AGENTS_FRONTEND_COPY_COGNITIVE_LOAD.md)).
