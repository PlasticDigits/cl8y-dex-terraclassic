...
6. **Frontend hybrid quote = execute ([GitLab #418](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/418), [#501](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/501)):**
   ...
   - Best-execution hybrid is **always on** for retail Swap + Trade market ([GitLab **#596**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/596)): no opt-in and no opt-out. Empty manual book ≠ “skip the book”: GET may still allocate a **hop-0** split. Interior hops on retail GET are pool-only so execute cannot freeze quote-time hop-1+ integers ([#1280](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1280), [`AGENTS_HYBRID_HOP_OFFER.md`](./AGENTS_HYBRID_HOP_OFFER.md)). **Native wrap-enter / unwrap-exit** map wrap CW20s then GET, but execute stays pool-only (`omitAllHybrid`, [#1218](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1218), [`AGENTS_FRONTEND_WRAP_ENTER_ROUTE_SOLVE.md`](./AGENTS_FRONTEND_WRAP_ENTER_ROUTE_SOLVE.md)). Advanced typed book is a **split override** (POST), not a pool-only switch. Integrators may pass `pool_only=true` on indexer GET for explicit pool quotes (or run a custom frontend).
...
- **Integrator semantics (Forgejo #707):** pair `Swap` with omitted `hybrid` and `greedy` is pool-only under the current contract. For best execution, use GET `/api/v1/route/solve` with `amount_in`; preserve the returned `hybrid` for a one-hop pair-direct execution, or use returned router operations for multi-hop. Caller-declared Pattern C must quote and execute the same params. Current explicit greedy is a separate opt-in heuristic (#708–#710); future omitted-parameter default behavior is gated and tracked by [#718](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/718). Full table + invariants: [`docs/integrators.md` § Pair swaps](../docs/integrators.md#pair-swap-pool-only-vs-best-execution-forgejo-707).
...
- Invariant **L9** (`max_spread` pool + hybrid): [`dex_common::max_spread`](../smartcontracts/packages/dex-common/src/max_spread.rs), [GitLab **#197**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/197), [#1230](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1230) (zero / dust-floor `belief_price` rejected), [`docs/integrators.md`](../docs/integrators.md#slippage-max_spread-and-belief_price-hybrid), [`AGENTS_MAX_SPREAD_HYBRID.md`](./AGENTS_MAX_SPREAD_HYBRID.md)
...
- **Tax-aware ranking (#615):** indexer `GET /route/solve` ranks by catalog **net** (`estimated_amount_out_net`). Swap/Trade You Receive uses that net when `token_out` is the tax token; submit `min_return` stays pre-tax (`executeAmountOut`). [`AGENTS_INDEXER_TAX_AWARE_ROUTING.md`](./AGENTS_INDEXER_TAX_AWARE_ROUTING.md). `make verify-issue-615`.
...
make verify-issue-1280  # #1280 hop-offer partition; Policy A hop0-only GET
make verify-issue-1264  # #1264 wrap+2hop USTC→USTR pool-only envelope (no AC1 raise)
...
- Closed GET `/route/solve` failure census (**Stay**): [ADR 0007](../docs/adr/0007-route-solve-remaining-failures.md) / [#1265](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1265), implemented by merged [PR #1289](https://git.cl8y.com/code/cl8y-dex-terraclassic/pulls/1289). F0 mapping/solver implementation landed under [#1218](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1218) / PR #1297; F1 is resolved under closed #1257. See [closeout QA](../docs/qa/issue-1265/README.md); verify: `make verify-issue-1265`.
