...
4. **Router** `simulate_swap_operations`: `terra_swap.hybrid: null` with `greedy` omitted means pool-only on-chain — router maps that to `pool_only_hybrid_params` per hop. Explicit `greedy` is a separate book-first mode.
...
- Remaining GET `/route/solve` failures census (**Stay**): [ADR 0007](../docs/adr/0007-route-solve-remaining-failures.md) / [#1265](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1265). Verify: `make verify-issue-1265`. F1 stays #1257.
