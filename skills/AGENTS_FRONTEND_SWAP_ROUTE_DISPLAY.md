...
| **Trade market** quote source ([#501](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/501)) | Same file — default (hybrid on, empty book): [`quoteCw20ViaRouteSolve`](../frontend-dapp/src/utils/cw20RouteSolveQuote.ts) → `GET /route/solve` + wallet sim (same as Swap). Advanced typed book leg: [`quoteDirectHybridSwap`](../frontend-dapp/src/utils/directHybridQuote.ts) (`POST` + LCD) — **no pool-only fallback** when manual book &gt; 0 ([#418](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/418)). Hybrid off → pool-only. Manual split + hybrid toggle live under collapsible **Advanced** (`trade-market-advanced-toggle`). |
...
| **Swap Execution hybrid Settings notice ([#492](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/492))** | [`swapDisclosure.ts`](../frontend-dapp/src/utils/swapDisclosure.ts) — `getDirectHybridSettingsExecutionSummary`; hybrid on + empty manual book → **no** Execution notice (do not say “add a book leg”). Gate `swap-execution-summary` on `indexerHybridExec.show \|\| settingsExec.show`. |
...
7b. **Hybrid on + empty book (#492):** Direct CW20 swap, Settings hybrid checked, book leg amount empty, pool-only quote — **no** “Pool only — add a book leg…” and no hollow `swap-execution-summary` unless indexer hybrid shows.
...
8. Open `/trade/<pairAddr>` → **Market** tab → enter amount `1` (Advanced collapsed; best execution on by default) → confirm **`trade-market-route-summary`** is visible with a **Route** label inside **`trade-market-quote`**. Network: quote should hit **`GET /api/v1/route/solve`**, not POST, when book leg is empty (#501).
...
9. **Pool-only:** Advanced → uncheck best execution, amount set → quote card shows; route line is direct `PAY → RECEIVE` (or row absent only when `marketRouteLine` is null per table above).
...
11. **Hybrid quote = execute (#418 / #501):** Default GET path — receive uses wallet sim; submit uses solver hybrid from `indexerOperations`. Manual book &gt; 0 — **no** “pool-only quote / hybrid execution” warning; `quoteDisclosure` reflects hybrid or pool-only path honestly.
