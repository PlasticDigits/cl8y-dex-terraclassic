...
- Retail hybrid always-on (no opt-out): [GitLab **#596**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/596), [`AGENTS_FRONTEND_HYBRID_ALWAYS_ON.md`](./AGENTS_FRONTEND_HYBRID_ALWAYS_ON.md), `make verify-issue-596`
...
7b. **Always-on hybrid + empty book (#492 / #596):** Direct CW20 swap, book leg amount empty — **no** “Pool only — add a book leg…” and no hollow `swap-execution-summary` unless indexer hybrid shows. **No** hybrid opt-out checkbox in Settings → Advanced.
...
15. **Trade market:** Repeat (12–13) on `/trade/:pairAddr` Market tab (hybrid always on).
