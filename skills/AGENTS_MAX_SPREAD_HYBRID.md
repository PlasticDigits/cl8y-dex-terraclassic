...
| **`belief_price` set** | `(expected − actual) / expected ≤ max_spread` where `expected = offer / belief_price` and `actual = book_net + pool_net + pool_commission`. `belief_price` must be strictly positive and `expected >= 1` raw unit ([#1230](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1230)); otherwise typed `InvalidBeliefPrice`, never panic / never skip L9 or #307. |
...
- Treat `belief_price = 0` or a dust-floor belief (`floor(offer / bp) == 0`) as a valid floor or as “belief was set” for G8 / #307 skip ([#1230](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1230)).
...
cd smartcontracts && cargo test -p cl8y-dex-tests test_swap_belief_price
cd frontend-dapp && npm test -- src/utils/swapMaxSpread.test.ts
make verify-issue-1230
```
...
Pool-only regression: `test_swap_max_spread` in [`smartcontracts/tests/src/lib.rs`](../smartcontracts/tests/src/lib.rs). Zero / dust-floor belief ([#1230](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1230)): `test_swap_belief_price_*`.

## Frontend mirror

TypeScript preflight (no `belief_price`): [`frontend-dapp/src/utils/swapMaxSpread.ts`](../frontend-dapp/src/utils/swapMaxSpread.ts) — keep in sync with `dex_common::max_spread`. Direct pair `swap()` rejects non-positive `beliefPrice` strings ([#1230](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1230)); do not add a retail Swap UI field.
