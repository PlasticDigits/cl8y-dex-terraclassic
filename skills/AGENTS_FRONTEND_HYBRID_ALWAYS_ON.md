...
7. **H596-7 — wrap / native unchanged.** Direct wrap/unwrap and native denom routes are not CW20 Pattern C hybrid. Do not force a book leg there. Do not copy leftover `GET /route/solve` `hybrid` onto wrap hops ([#1264](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1264) / [#1280](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1280)). USTR→USTC gas is the unwrap+≥2hop combo ([#599](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/599)), not hybrid 15M — see [`AGENTS_TERRACLASSIC_GAS.md`](./AGENTS_TERRACLASSIC_GAS.md). Post-merge E9 / columbus-5: [#600](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/600) / `make verify-issue-600`. USTC→USTR wrap+2hop shares the **2,710,000** #587 envelope until columbus-5 AC1 (`make verify-issue-1264`, **G1264-1–G1264-8**). Swap Max / LUNC gas-gate treat only `uluna` as fee-paying native (**G1264-4**).
