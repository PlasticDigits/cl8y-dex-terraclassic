...
| [ADR 0007](../docs/adr/0007-route-solve-remaining-failures.md) / closed [#1265](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1265) | Accepted retail solve failures census (**Stay**); [PR #1289](https://git.cl8y.com/code/cl8y-dex-terraclassic/pulls/1289) is merged. **H596-7** wrap stays pool-only; F0 mapping/solver implementation landed under [#1218](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1218) / PR #1297. Verify: `make verify-issue-1265` |
...
2. **H596-2 — default quote is GET.** Empty manual book → [`quoteCw20ViaRouteSolve`](../frontend-dapp/src/utils/cw20RouteSolveQuote.ts) (`GET /api/v1/route/solve`). Empty book ≠ skip the book: the solver may still allocate a **hop-0** `book_input`. Retail GET hops 1+ stay pool-only ([#1280](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1280)).
...
7. **H596-7 — wrap / native unchanged.** Direct wrap/unwrap and native denom **execute** are not CW20 Pattern C hybrid. Do not force a book leg there. Wrap-enter / unwrap-exit **quotes** map wrap CW20s then `GET /route/solve` ([#1218](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1218), [`AGENTS_FRONTEND_WRAP_ENTER_ROUTE_SOLVE.md`](./AGENTS_FRONTEND_WRAP_ENTER_ROUTE_SOLVE.md)); submit still strips all `hybrid` / `book_input` (**H1218-4**). Do not copy leftover GET `hybrid` onto wrap hops ([#1264](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1264) / [#1280](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1280)). USTR→USTC gas is the unwrap+≥2hop combo ([#599](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/599)), not hybrid 15M — see [`AGENTS_TERRACLASSIC_GAS.md`](./AGENTS_TERRACLASSIC_GAS.md). Post-merge E9 / columbus-5: [#600](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/600) / `make verify-issue-600`. USTC→USTR wrap+2hop remains at the **2,710,000** #587 envelope after AC1 measured **2,630,228** gas used (`make verify-issue-1264`; [ADR 0004](../docs/adr/0004-terraclassic-retail-gas-census.md)).8. **H596-8 — gas assumes hybrid.** Trade market `estimateMarketPairSwapSequenceUlunaFeesTotal(true, …)` / `marketUsesHybrid={true}`. Do not reserve pool-only gas for the retail market ticket.
...
# Wrap-enter GET (pool-only execute): make verify-issue-1218
```
