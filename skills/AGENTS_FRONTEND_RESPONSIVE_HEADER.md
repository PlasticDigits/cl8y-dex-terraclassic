...
Use when changing **`Layout.tsx`**, **primary / More nav items**, **`HEADER_FULL_NAV_MIN_WIDTH_PX`**, sticky **environment ribbon** spacing/opacity, header **NetworkBadge** visibility, or Playwright tests under **`frontend-dapp/e2e/navigation.spec.ts`** that assert header overlap / sticky seam.

## Canonical references

| Doc / code | Purpose |
|------------|---------|
| [docs/frontend.md § Responsive shell & header navigation](../docs/frontend.md#responsive-header-navigation) | Breakpoint invariants, mobile vs tablet vs desktop behavior ([GitLab **#136**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/136), sticky seam **#482**, nav→controls **#483**, menu-above-ribbon **#486**) |
...
| `frontend-dapp/src/index.css` | `.app-top-sticky`, `.app-env-ribbon*`, `.app-desktop-nav` vs `.app-mobile-nav-shell` visibility (`max-width: 767px`) |
...
4. **Header brand** — logo + **`CL8Y DEX`** title only; do **not** reintroduce a “Terra Classic ecosystem” (or similar) kicker in the sticky header — it crowded the nav row and regressed [#136](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/136). Network branding belongs in the **environment ribbon** (and footer), not a second header line.
5. **Desktop/tablet NetworkBadge ([#483](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/483)):** do **not** re-add header `NetworkBadge` beside theme/wallet on ≥768px — `EnvironmentRibbon` is the primary network signal. Mobile may keep the badge next to the wallet chip. Assert **≥ ~8px** gap between last nav control (More) and `.app-header-theme-group` at **1200 / 1280 / 1440**. Full inline nav starts at **`HEADER_FULL_NAV_MIN_WIDTH_PX` = 1200**.
6. **Sticky ribbon opacity & seam ([#482](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/482)):** keep `.app-top-sticky` on an opaque `var(--bg-0)` stack background; ribbon must layer **panel-bg + network tint** (not a wash alone); preserve header↔ribbon gap and Trade H1 clearance. Prefer shell CSS over page-by-page sticky offsets. When changing ribbon margin/padding, also revisit `.app-main-shell` `padding-top`.
7. **Header menus above ribbon ([#486](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/486)):** inside `.app-top-sticky` (`isolation: isolate`), keep `.app-header-shell` **above** `.app-env-ribbon` so open `.app-menu` / `.wallet-menu` are fully visible and clickable over the ribbon band. Raising menu `z-index` alone does **not** escape the sticky isolate relative to a later ribbon sibling. Do **not** shrink/hide the content-column ribbon as the “fix,” and do not raise the sticky stack above modal portals (`z-index: 200`). Regression: `navigation.spec.ts` — “Header menus above EnvironmentRibbon (GitLab #486)”.
...
- Environment ribbon / risk strip: [`AGENTS_FRONTEND_RISK_DISCLAIMERS.md`](./AGENTS_FRONTEND_RISK_DISCLAIMERS.md) ([GitLab **#138**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/138), [#482](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/482), [#486](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/486))
