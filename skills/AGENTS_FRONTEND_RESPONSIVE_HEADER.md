...
6. **Footer environment ribbon:** keep `.app-env-ribbon` inside `footer.app-footer-shell` on **all** breakpoints (including mobile above the bottom tab bar). Do **not** move it back under `.app-top-sticky` without updating docs, skills, and `navigation.spec.ts`. Official **Homepage** / **Bridge** links are a **separate** footer row — not header More ([#663](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/663), [`AGENTS_FRONTEND_PRODUCT_LINKS.md`](./AGENTS_FRONTEND_PRODUCT_LINKS.md)).
...
- Official CL8Y product links (footer Homepage / Bridge): [`AGENTS_FRONTEND_PRODUCT_LINKS.md`](./AGENTS_FRONTEND_PRODUCT_LINKS.md) ([GitLab **#663**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/663))
