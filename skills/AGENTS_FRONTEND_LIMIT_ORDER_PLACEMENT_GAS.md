...
3. **Gas semantics** for the two-tx place sequence (allowance + hook) use the selected step cap in `getGasLimitForTx` and native LUNC preflight ([#1329](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1329)). Keep the formula and query-parameter exclusion in sync with [`AGENTS_HYBRID_LIMIT_GAS.md`](./AGENTS_HYBRID_LIMIT_GAS.md) and [`AGENTS_TERRACLASSIC_GAS.md`](./AGENTS_TERRACLASSIC_GAS.md).
4. **Both surfaces** must stay shared via `LimitOrderAdvancedLimitSettings` — do not fork trade vs `/limit` copy.
5. **`hint_after_order_id` (GitLab #256 / #261 / indexer #267):** when the indexer knows the predecessor order id (`GET .../limit-book/insert-hints` or merged pages), pass it on `UpdateLimitOrderPrice` and **single-rung batch placement** (`placeLimitOrderWithAllowance` → batch item field) to avoid head walks. On-chain verifies hint; stale/missing anchors fall back to head walk; **near-miss** hints on valid anchors walk from the hint toward head/tail ([#265](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/265)). Resolver: indexer [#267](../docs/integrators.md#insert-hints-price-window-gitlab-267); client fallback [`limitBookInsertHint.ts`](../frontend-dapp/src/utils/limitBookInsertHint.ts). See [limit-orders.md § hint_after](../docs/limit-orders.md#place--cancel-limit-gitlab-206), invariant **L14** in [contracts-security-audit.md](../docs/contracts-security-audit.md).
