...
5. **`hint_after_order_id` (GitLab #256):** when the dApp/indexer knows the predecessor order id (deep book, prior rung), pass it on `UpdateLimitOrderPrice` and single/batch placement to avoid head walks. On-chain verifies hint; stale hints fall back safely. See [limit-orders.md § hint_after](../docs/limit-orders.md#place--cancel-limit-gitlab-206), invariant **L14** in [contracts-security-audit.md](../docs/contracts-security-audit.md).
