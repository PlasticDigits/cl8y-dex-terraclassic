...
5. **Hide Mint nav** unless `VITE_FAUCET_ADDRESS` is set (**F11**). Use `includeMint` on `getHeaderMoreMenuItems` / `getMobileMoreMenuItems`. Production `dex.cl8y.com` must **unset** this env ([#562](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/562) **P562-8**).
...
6. **Emergency:** faucet `Pause` + CW20 `RemoveMinter`; pause preserves cooldown map (**F9**). Production hide of gem pickers ([#562](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/562)) is **not** a substitute for Pause.
...
- Production gem hide / Mint unset: [`AGENTS_FRONTEND_RETAIL_TEST_TOKENS.md`](./AGENTS_FRONTEND_RETAIL_TEST_TOKENS.md) ([GitLab **#562**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/562))
