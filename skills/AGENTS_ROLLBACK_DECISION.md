...
3. **Indexer rollback** is **three-way** after Coolify auto-deploy ([#1276](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1276) / [ADR 0006](../docs/adr/0006-indexer-health-git-sha.md)): **unchanged vs prior Coolify Deploys SHA** (compare `_sqlx_migrations` `version` / `success` / `installed_on` to that SHA’s latest `indexer/migrations/*.sql`; still no SOURCE SHA log scrape) → restore the **prior Coolify indexer deploy**; **ahead + no** `down.sql` → **forward-fix**; **ahead + documented revert** → auto-deploy **off** (#297) → snapshot → apply **that** `down.sql` → `DELETE FROM _sqlx_migrations WHERE version = <that version>` (only that row) → restore the prior Coolify image → attest. Leave auto-deploy off until a good tip is on `main`. Revert files undo schema only; they do **not** touch the ledger. Dirty `success = false` → repair ledger, then restore or forward-fix. Do **not** `set_ignore_missing(true)` in production. Inspect production schema via Coolify DB shell / indexer app `DATABASE_URL`, not `scripts/lib/postgres-psql.sh` (LocalTerra/dev only). Image binary is `cl8y-dex-indexer`. Attest `/health` `git_sha` with `VERIFY1276_EXPECT_SHA`; do not scrape Coolify logs. See runbook § Auto-deploy era Coolify incident checklist.
