...
3. **Indexer rollback** may need paired `down.sql` under `indexer/migrations/revert/` — see [docs/testing.md § Manual rollback SQL](../docs/testing.md). After indexer Coolify auto-deploy ([#1276](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1276) / [ADR 0006](../docs/adr/0006-indexer-health-git-sha.md)), a landed migration **without** `down.sql` is **forward-fix** (do not restore the previous image). Schema unchanged → restore the **prior Coolify indexer deploy**. Attest `/health` `git_sha` with `VERIFY1276_EXPECT_SHA`; do not scrape Coolify logs. See runbook § Auto-deploy era Coolify incident checklist.
