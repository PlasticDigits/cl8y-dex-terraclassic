...
3. **Indexer rollback** is **three-way** after Coolify auto-deploy ([#1276](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1276) / [ADR 0006](../docs/adr/0006-indexer-health-git-sha.md)): schema **unchanged** (a failed migrate that rolled back leaves **no** new `_sqlx_migrations` row) → restore the **prior Coolify indexer deploy**; schema **ahead** with **no** `down.sql` → **forward-fix**; schema **ahead** with a documented file under `indexer/migrations/revert/` → snapshot → apply **that** `down.sql` → restore the prior Coolify image → attest. Inspect production schema via Coolify DB shell / indexer app `DATABASE_URL`, not `scripts/lib/postgres-psql.sh` (LocalTerra/dev only). Image binary is `cl8y-dex-indexer`. Attest `/health` `git_sha` with `VERIFY1276_EXPECT_SHA`; do not scrape Coolify logs. See runbook § Auto-deploy era Coolify incident checklist.
