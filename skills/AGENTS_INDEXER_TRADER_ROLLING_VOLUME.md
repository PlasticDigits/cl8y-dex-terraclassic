...
- **Do** widen (`USING …::numeric(38, 0)`), then heal (INSERT + lifetime UPDATE + leftover-zero), then #553 USD UPDATE, then leftover-USD `NULL`, in the same migration file. Migration SQL is canonical; Rust **copies** it. Copy priced USD from `traders::refresh_trader_total_volume_usd` (`traders.rs`, currently 313–328), not from `volume.rs` (call site only). Keep-in-sync comment *pattern* may still cite `backfill_swap_volume_usd`. Run those statements in **one** transaction. sqlx migrations cannot call Rust. Do **not** call `refresh_trader_total_volume_usd(&pool)` from the helper. Do not move the pool helper.
