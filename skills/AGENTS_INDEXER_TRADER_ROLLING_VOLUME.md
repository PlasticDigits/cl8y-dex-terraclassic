...
| **V1277-3** | `total_volume_usd` stays `NUMERIC(38, 18)` / `10^20` USD cap. Heal INSERT/UPDATE does **not** rewrite it. Idle **D2** zeros rolling columns only. Unpriced stays NULL (**R5**). After heal, call `refresh_trader_total_volume_usd`. |
| **V1277-4** | After overflow-skip, one-shot + poller heal from `swap_events`: `INSERT` missing senders (`ON CONFLICT DO NOTHING`); `UPDATE` existing including register-then-overflow; recompute `total_trades` / `total_volume` with the integer cap. Block replay alone does not heal. In `poller.rs`, `heal_trader_lifetime_from_swaps` **before** `refresh_all_volume_windows(..., true)` (#1269 slot, not the #676 slot after refresh). Mismatch-gated (missing sender **or** `total_trades` / capped `total_volume` diverge). Scan is `GROUP BY sender` on `swap_events` (`idx_swaps_sender`), not “`traders` is small”. |
...
- **Do** widen (`USING …::numeric(38, 0)`), then heal, then #553 USD UPDATE, in the same migration file. Migration SQL is canonical; Rust **copies** it with a keep-in-sync comment (`backfill_swap_volume_usd`). sqlx migrations cannot call Rust.
...
| Area | Path |
|------|------|
| Migration | `indexer/migrations/20260920120000_traders_raw_volume_numeric_38_0.sql` (name may shift; must `ALTER` + `USING`, then heal, then #553 USD) |
...
| Reorg | [`docs/runbooks/indexer-reorg-replay-dedup.md`](../docs/runbooks/indexer-reorg-replay-dedup.md) — startup heal before re-ingest after `--cleanup-derived` |
...
- [`AGENTS_INDEXER_TRADER_POSITIONS_DECIMALS.md`](./AGENTS_INDEXER_TRADER_POSITIONS_DECIMALS.md) — skip-class precedent, different type; mismatch-gate analog, not an unconditional boot rewrite
