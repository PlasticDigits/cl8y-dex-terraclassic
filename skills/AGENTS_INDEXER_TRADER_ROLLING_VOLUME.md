...
| **V1277-3** | `total_volume_usd` stays `NUMERIC(38, 18)` / `10^20` USD cap. Heal INSERT / lifetime UPDATE / leftover-zero do **not** rewrite it. Idle **D2** zeros rolling columns only. Unpriced stays NULL (**R5**). Same heal transaction: SQL **text** of `traders::refresh_trader_total_volume_usd` (pin the function, not line numbers) on `&mut Transaction`, then leftover-USD `NULL` when no remaining priced `swap_events`. A shared SQL `&str` with the pool helper is allowed. Do **not** copy from `volume.rs` (call site only). Do **not** call `refresh_trader_total_volume_usd(&pool)` inside `heal_trader_lifetime_from_swaps`. Do not change `refresh_trader_total_volume_usd(pool: &PgPool)` or move the pool helper. Do not change P522-Q for senders who still have priced swaps. |
| **V1277-4** | After overflow-skip, one-shot + poller heal from `swap_events`: `INSERT` missing senders (`ON CONFLICT DO NOTHING`); `UPDATE` existing including register-then-overflow; leftover-zero `UPDATE` (`total_trades = 0`, `total_volume = 0`, `first_trade_at` / `last_trade_at` `NULL`) when leftover-lifetime and **no** remaining `swap_events` — **do not `DELETE`** (keep tier / registered / P&L). Leftover-zero omits `COALESCE` on `NOT NULL` `total_volume`. Do **not** zero idle traders who still have old `swap_events` (**D2**). Block replay alone does not heal. In `poller.rs`, `heal_trader_lifetime_from_swaps` **before** `refresh_all_volume_windows(..., true)` (#1269 slot, not the #676 slot after refresh). Wrap like `#1269` / `#676`: `if let Err` log; do **not** abort `run_indexer`. **D5** still runs after a failed heal; gate retries next boot; a later heal `UPDATE` recomputes from `swap_events`. Mismatch-gated with `COALESCE` like `#676` `positions_trade_count_diverges` (not `s.sender IS NOT NULL`) and **raw-lifetime only** (do not expand to USD): leftover-lifetime ghosts **trip**; registered / zero-lifetime ghosts **do not**. Scan is `GROUP BY sender` on `swap_events` (`idx_swaps_sender`), not “`traders` is small”. Heal writes + USD are **one transaction**. |
...
- **Do** widen (`USING …::numeric(38, 0)`), then heal (INSERT + lifetime UPDATE + leftover-zero), then #553 USD UPDATE, then leftover-USD `NULL`, in the same migration file. Migration SQL is canonical; Rust **copies** it. Pin priced USD by `traders::refresh_trader_total_volume_usd` SQL **text** (not line numbers), not from `volume.rs` (call site only). A shared SQL `&str` with the pool helper is allowed; do **not** change `refresh_trader_total_volume_usd(pool: &PgPool)`. Keep-in-sync comment *pattern* may still cite `backfill_swap_volume_usd`. Run those statements in **one** transaction. sqlx migrations cannot call Rust. Do **not** call `refresh_trader_total_volume_usd(&pool)` from the helper. Do not move the pool helper.
- **Do** keep rolling refresh and idle zero-out in one transaction (**D2** / **A6**).
- **Do** call `heal_trader_lifetime_from_swaps` in `poller.rs` **before** `refresh_all_volume_windows(..., true)` with `if let Err` log like `#1269` / `#676` — do **not** abort `run_indexer`.
- **Do** gate with `COALESCE` like #676 on **raw lifetime only**. Leftover-lifetime ghosts trip; registered zeros do not. Do **not** expand the gate to `total_volume_usd` on this ticket.
...
- **Don’t** call `refresh_trader_total_volume_usd(&pool)` **inside** `heal_trader_lifetime_from_swaps` (use the #553 SQL **text** on `&mut Transaction`; shared `&str` allowed) **or** after a committed heal (crash window). Catalog `volume.rs` may still use the pool-level function.
- **Don’t** abort `run_indexer` on heal `Err`.
...
| Rolling SQL + upsert + heal copy | [`indexer/src/db/queries/traders.rs`](../indexer/src/db/queries/traders.rs) — `#553` SQL source is `refresh_trader_total_volume_usd` here; `volume.rs` only *calls* it |
