...
Use when changing **WalletConnect QR / pairing UX**, **Lunc Dash / Galaxy Station / Keplr mobile connect**, **cosmes `QRCodeModal`**, **`wc:` deep links**, or **Connect Wallet cancel/timeout** ([GitLab #519](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/519), [#554](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/554)).
...
| `frontend-dapp/src/components/wallet/connectWalletOptions.ts` | Mobile Keplr WalletConnect vs Extension |
...
10. **WC-M10 — Keplr on mobile Chrome.** When `isWalletConnectMobileClient()` and `window.keplr` is absent, offer **Keplr via WalletConnect** — not Install-only. Injected `window.keplr` (in-app) stays Extension (**WC-M7**).
...
4. Keep retail copy short: **Open Lunc Dash**, **Open Galaxy Station**, **Open Keplr**, **Open wallet**, **Copy pairing link**, **Cancel**, **Open your wallet, then return here.**
...
6. Known-good same-device connect on Android Chrome: ustr-cmm `frontend/` uses one Connect modal + **Cancel** (no pairing sheet under the list). DEX pairing must stay above Connect.
...
Manual: Android 16 Chrome → Connect → Lunc Dash / Galaxy Station / Keplr → **Open {wallet}** or **Copy pairing link** → approve → `terra1…` in the header. Dismiss / timeout must re-enable Connect. Desktop: Lunc Dash still QR-only; Keplr extension unchanged.
