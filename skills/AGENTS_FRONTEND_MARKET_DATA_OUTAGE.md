...
| [`indexerErrors.ts`](../frontend-dapp/src/utils/indexerErrors.ts) | `isIndexerUnavailableError`, `isIndexerClientError` (4xx), `isIndexerPairNotFoundError` (404) |
...
2. **Do not** treat indexer **4xx** as a global outage — **404** → not-found / `RetryError` ([#177](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/177)); **400** route-solve simulation failures → LCD fallback, no banner ([#326](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/326)). Outage = network/`AbortError`/`Failed to fetch` or **5xx** only.
