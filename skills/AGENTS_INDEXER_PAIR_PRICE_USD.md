...
| **P522-4** | Both-catalog candle `open/high/low/close` are factory **USD of `asset_0` from `price_usd` only** (no human fallback — [#543](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/543)). Additive `*_human` is quote-per-base. Mixed and neither-catalog candles are [#1315](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1315) / [ADR 0012](../docs/adr/0012-alpha-pair-price-candles.md): USD of the non-catalog leg (`usd_leg` stored at write), or NULL USD plus `*_human` only when neither leg is catalog. A missing catalog print skips the candle. Never copy human into USD columns. |
| **P522-5** | Headline Last, 24h OHLC, TWAP, tape Price, and limit USD notional stay on swap `price_usd` / `*_usd` stats — **never** raw or human `trades[].price`, and **never** candle subject USD. The price pane may use candle subject USD ([#1315](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1315)): USD of `asset_0` on `open/high/low/close`, or `subject_*` when `usd_leg=asset_1`. Trades table **Price** stays human quote-per-base and must not compact-format as `T`. |
...
- **Don’t** add `CL8Y` or `ALPHA` to `quote_usd_kind` (symbol alone stays `None`). Protocol **fee** USD for factory-listed economic CW20s is a sibling mark path ([#683](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/683)); volume/TVL stay P522-Q. Chart candles for those pairs are [ADR 0012](../docs/adr/0012-alpha-pair-price-candles.md) ([#1315](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1315)), not a peg.
