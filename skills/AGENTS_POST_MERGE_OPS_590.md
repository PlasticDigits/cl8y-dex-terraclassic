...
| **M590-6** | `LAYER_B_LT=1` **must not PASS as a stub**. It runs [`layer-a-lcd.sh`](../cw20-codeid-audits/scripts/layer-a-lcd.sh) (store + instantiate + 1:1 Transfer of pinned `token.wasm`) then [`layer-b-lt.sh`](../cw20-codeid-audits/scripts/layer-b-lt.sh) (whitelist **local** store id, `CreatePair` vs EMBER, 1:1 into pair). Columbus-5 templates need LocalTerra (`requires_terra`). |
