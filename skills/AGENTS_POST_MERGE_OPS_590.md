...
| **M590-4** | Limit place → `limit_place` from `maker_fee_amount`. Unwrap → wrap-mapper **`fee`** (legacy `fee_amount`) only ([#613](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/613)). InstantWithdraw burn tax / `tax_amount` are **not** protocol fees. |
