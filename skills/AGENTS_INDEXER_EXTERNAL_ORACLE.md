...
GET /api/v1/oracle/price/vfdusd       → { ticker: "vfdusd", quote_asset: "FDUSD", display_name: "FDUSD/USD", price_usd, sources, venus }  # CEX FDUSD + Venus additive
...
- **Do** keep Protocol UI labeled **USTC / USD** when using the default ticker; LUNC keeps `/ USD`; the vFDUSD tab heading is **vFDUSD** with **FDUSD reference price** + **1 vFDUSD Price** ([#571](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/571)).
...
7. `make verify-issue-579` when touching the oracle HTTP client, CoinGecko fetch, or User-Agent (wiremock; no live CoinGecko)
8. `make verify-issue-580` when touching oracle display_name, catalog metadata, or P522-Q vFDUSD mapping
