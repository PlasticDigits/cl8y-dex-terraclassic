...
| [`useLimitOrderMakerFeeRates.ts`](../frontend-dapp/src/hooks/useLimitOrderMakerFeeRates.ts) | React Query: `getPairFeeConfig` + pair `DISCOUNT_REGISTRY` probe + `getTraderDiscount` only when the pair is wired to `VITE_FEE_DISCOUNT_ADDRESS` ([#537](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/537) / **I14**) |
...
3b. **#529 mixed decimals:** typed / ref / gates stay **human** token1 per token0. Convert **human → raw** only in `placeLimitOrder*` / `updateLimitOrderPrice` via [`limitOrderPriceScale.ts`](../frontend-dapp/src/utils/limitOrderPriceScale.ts). Convert indexer/LCD book prices **raw → human** before display, invert (#524), crossing, or insert-hint compare. See [`AGENTS_LIMIT_PRICE_DECIMALS.md`](./AGENTS_LIMIT_PRICE_DECIMALS.md).
