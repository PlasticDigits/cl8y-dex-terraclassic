...
8. **Copy (anti-cognitive-overload):** Write for humans, not agents. Labels ≤ ~5 words; blocking errors ≤ 1 short sentence; optional single-word **Docs** link. Never show `token0` / `token1` / raw `bid`/`ask` in retail UI — use symbols and **Buy** / **Sell**. Keep blocking errors, risk acknowledgement, and chain anchors. Agents read docs/skills; users should not drown in disclosure.
