...
2. **L620-2 — funding fork.** Wrap CW20s skip Mint. Community-tax (`VITE_TOKEN_COMMUNITY_TAX_ADDRESS`, or a successful `GetLauncherOrigin` decode — including `launcher: null` Layer B / #623 instantiations) **Transfer** from `test1`. Gems and TCL8Y stay **Mint**. Transfer failure on a **pinned** token is **fail-closed** — do not add `MintControl` or fall back to Mint. Playwright’s e2e wallet **is** `test1`: Transfer `test1→test1` cannot raise the balance. The pinned QA token must already meet the floor from CreateToken `initial_balances`. Leftover tax wasm on a long-lived chain is **skipped** (not Minted) so gem `e2e-tx` stays green (**#622**).
3. **L620-3 — QA profile.** Default seed token has **no** `MintControl`, no `LaunchGuards` SKU (`trading_enabled` stays live), buy/sell **500 bps**, large `initial_balances` to `test1`. Token **treasury is a distinct LocalTerra sink** (test2), not the e2e/swarm trader — otherwise extra-debit nets to 1:1 ([#625](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/625) **M625-2**). A “safe launch” preset will brick swarm/E2E.
