...
Issue **#595 is implemented** (v1 multi-msg settlement). An on-chain `invoice-payer` adapter and indexer exact-out `amount_out=` are **follow-ups**, not v1 blockers. `#593` Create Token / Manage Save consume this card ([`AGENTS_FRONTEND_CREATE_TOKEN.md`](./AGENTS_FRONTEND_CREATE_TOKEN.md)). Desktop Create Token width is layout-only ([#669](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/669), [`AGENTS_FRONTEND_CREATE_TOKEN_LAYOUT.md`](./AGENTS_FRONTEND_CREATE_TOKEN_LAYOUT.md)) — do not fork this Pay card to “fix” screenspace.
