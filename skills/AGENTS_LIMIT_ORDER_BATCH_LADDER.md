...
| [`limitOrderLadder.ts`](../frontend-dapp/src/utils/limitOrderLadder.ts) | Client ladder preview + `sumLadderAmountsRaw` (must match `expand_limit_ladder` amounts; prices stay **human** in the preview — #529 scales at submit) |
...
6. **Gas preflight** for ladder uses `estimateLimitOrderBatchPlaceSequenceUlunaFeesTotal(rungCount, maxAdjustSteps)` — keep aligned with `getGasLimitForTx` for `place_limit_order_batch` / `place_limit_order_ladder`. The envelope is base + per-rung + `25_000 × sum(rung max_adjust_steps)` because skipped rungs may retry a bounded head walk before a thread cursor is available ([#1329](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1329), [`AGENTS_HYBRID_LIMIT_GAS.md`](./AGENTS_HYBRID_LIMIT_GAS.md)).
...
15. **Mixed-decimal limit prices ([#529](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/529))** — on-chain ladder `start_price` / `end_price` / rung `price` are **raw** token1-units/token0-units. Preview + crossing stay human; `placeLimitOrderLadderWithAllowance` / batch items pass `limitPriceScale` so UST1/USTR (~79 human → ~7.9e13 raw) is in the **L20** human band. Playbook: [`AGENTS_LIMIT_PRICE_DECIMALS.md`](./AGENTS_LIMIT_PRICE_DECIMALS.md).
...
16. **Named min remaining at place ([#1219](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1219) / L24)** — post–maker-fee remaining must be **≥ LIMIT_ORDER_DUST_FLUSH_THRESHOLD (10)**. Same constant as match-time dust flush (L16 / #264). Named LimitOrderAmountTooSmall (message includes 10); all-or-nothing — do not skip+refund like LimitInsertStepsExceeded. Maker fee stays floor. Ladder expand rejects equal-split rungs (including zeros from total < count) before batch execute. dApp disables Place below 10 raw units. This size floor does not reject remaining × price < 1: the unfillable in-band MIN_LIMIT_PRICE head remains open under [#1225](https://git.cl8y.com/code/cl8y-dex-terraclassic/issues/1225). UpdateLimitOrderPrice does not re-check size; see [limit-orders.md § Unfillable in-band floor orders](../docs/limit-orders.md#unfillable-in-band-floor-orders-forgejo-1225) and security invariant [L26](../docs/contracts-security-audit.md).
...
## Open adjacent blocker: economic fillability at the price floor (#1225)

The ≥ 10 remaining-size rule below does not guarantee floor(fill × price) ≥ 1. Keep #1225 open until in-band unfillable rows are rejected or removed from the active book, including legacy rows under default clean thresholds. Require pair execute / simulation / indexer parity and a long-prefix scan test before treating matching or ladder work as complete. See [limit-orders.md](../docs/limit-orders.md#unfillable-in-band-floor-orders-forgejo-1225), invariant [L26](../docs/contracts-security-audit.md), pair [orderbook.rs](../smartcontracts/contracts/pair/src/orderbook.rs), indexer [db_orderbook_sim.rs](../indexer/src/api/db_orderbook_sim.rs), and the [greedy book-first playbook](./AGENTS_GREEDY_BOOK_FIRST.md) for the #718 gate.
...
# Ladder create options when disconnected (#494)
cd frontend-dapp && npm test -- LimitOrderLadderPanel.disconnect LimitOrdersPage
...
- Anti-cognitive-overload retail copy (ladder progressive disclosure): [`AGENTS_FRONTEND_COPY_COGNITIVE_LOAD.md`](./AGENTS_FRONTEND_COPY_COGNITIVE_LOAD.md) ([#489](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/work_items/489))
