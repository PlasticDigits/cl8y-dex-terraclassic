...
| [GitLab **#670**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/670) / [`AGENTS_FRONTEND_TOKEN_MIGRATE.md`](./AGENTS_FRONTEND_TOKEN_MIGRATE.md) **M670** | Retail why-copy on `/token/migrate` (Unlock {X} + examples). Copy only — adopt payload unchanged |
| [GitLab **#670**](https://gitlab.com/PlasticDigits/cl8y-dex-terraclassic/-/issues/670) / [`AGENTS_FRONTEND_TOKEN_MIGRATE.md`](./AGENTS_FRONTEND_TOKEN_MIGRATE.md) **M670** | Retail why-copy on `/token/migrate` (Unlock {X} + examples). Copy only — adopt payload unchanged |
...
| [`AGENTS_LOCALNET_SWARM_TAX.md`](./AGENTS_LOCALNET_SWARM_TAX.md) | LocalTerra gem exclude + tax workers (**S621**, #621) |
...
4. **O601-4 — catalog filter.** Rogue `--admin` instantiate has `GetLauncherOrigin.launcher == null`. dApp (#593) / indexer (#594) must require CMM admin + origin. Adopted tokens (#626) attest with `GetMigrateOrigin` + CMM — do **not** fake `launcher_tx`.
...
7. **T592-7 — classification.** Provide (`TransferFrom`) and limit `PlaceLimitOrder*` `Send` are 1:1. Pair→EOA `Transfer` (swap receive / withdraw / limit refund) uses **buy tax** — same CosmWasm primitive; documented, not a pair wasm change. Retail e2e-tx asserts cancel pair debit = remaining and user credit net (**E622-6** / [`AGENTS_E2E_COMMUNITY_TAX_TX.md`](./AGENTS_E2E_COMMUNITY_TAX_TX.md)). Official router hops tax the original trader (**T592-13**). Manager-directory wallets (including hop `trader`) skip **buy, sell, and transfer** tax (**#609** / **E609-1**). `config.manager` is the same skip without a directory row (**#633** / **R633-1** / [`AGENTS_COMMUNITY_TAX_AUTOREGISTER.md`](./AGENTS_COMMUNITY_TAX_AUTOREGISTER.md)). Launch guards still use the economic kind (**E609-2** / **T592-11**). New CL8Y factory pairs that hold this token are registered (factory reply / dApp Create Pair / AutoLP bind / Manage catch-up).
