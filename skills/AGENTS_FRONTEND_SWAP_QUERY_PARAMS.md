...
| **Q711-6** | Optional pay amount: `exactAmount` / `amount` / `value` / `amountIn` when `isPositiveDecimalAmount` and ≤ 24 chars. No reverse quote (`exactField=output`). Never auto-submit, auto-connect, or skip #678 / 5–30–99 / blacklist / pause / freeze. Ignore `slippage`, `expertMode`, `recipient`, `pool_only`, `hybrid_optimize`. |
| **Q711-7** | Apply once per search string after `allTokens` is ready. Query wins over `defaultRetailSwapTokenPair`. Picker / flip afterward is not re-forced. Search-only changes on `/` apply via `useSearchParams` (Outlet `key={pathname}` does not remount). |
| **Q711-8** | Create Pair still has **no** `?a=&b=` (**C542-11**). Trade stays `/trade/:pairAddr`. A Swap deep link to UST1 is AMM, not mint/redeem (**U1**). No lecture banner when a link is ignored (**#489**). |
...
- **Don’t** rewrite the URL on every picker change (out of scope).
...
- [`AGENTS_FRONTEND_CREATE_PAIR_PICKER.md`](./AGENTS_FRONTEND_CREATE_PAIR_PICKER.md) — no `/create?a=&b=`
